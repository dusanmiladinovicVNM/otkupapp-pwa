...
| Otkup / dokumenta | `frmOtkup`+`modOtkup`, `frmDokumenta`+`modDokumenta`. **Pregled dokumenata (read-only):** JEDAN motor `modDokumenta.GetDokumentiByTip(tip, samoStornirani, dOd, dDo)` opslužuje oba panela u `frmDokumenta` — „Pregled storniranih" (`GetStorniraniByTip`) i „Pregled dokumenata" (`GetAktivniByTip`/`GetAktivniGrupisano`, dugme u frame-u „Lista zbirnih"): otkupni listovi, otpremnice, zbirne, prijemnice, fakture, keš/banka uplate i isplate (`Novac`, tip u koloni „Klasa / Tip") i 4 tipa reversa, 12 unifikovanih kolona + lanac Zbirna/Otpremnica/Faktura. **NE praviti novi per-tip pregled** — dodaj tip u `StorniraniTipovi` + mapiranje kolona u `GetDokumentiByTip`. Palete imaju svoj ekran (`frmPalete`), fakture `frmFakturisanje`, izvodi `frmBankaImport` |
