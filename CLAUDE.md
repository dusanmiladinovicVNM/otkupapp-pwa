...
| Stampa dokumenata (izlaz) | `modDocStyle` je JEDINA kapija: `DocPrintWs` / `DocExportPdf` → `DocEnsurePrintReady` (provera pred stampu). Stil + geometrija: `DocPageSetupThirdA4` (1/3-A4 obrasci — **Zoom=100, bez skaliranja**, redovi padaju na perforacije 99/198mm) i `DocPageSetupFitWide` (tabelarni — sve kolone na jednu stranu po sirini). **Ta dva se ne smeju pomesati** — namera se pamti pri pripremi obrasca. **PageSetup je driver-zavisan:** drajver podrazumevanog stampaca (stampac za etikete na istom racunaru) obara papir i prebacuje skaliranje na „no scaling“, i to se pamti U SHEETU → dokument izadje prelomljen bez ijedne izmene u kodu. Zato se posle upisa PROVERAVA sta je drajver primio (folbek: rucni `.Zoom` iz `Range.Width`), a kad drajver odbija A4 operater dobija upozorenje sa imenom stampaca. Dijagnostika: `modSetup.DebugStampaPageSetup`. **Ne dodavati nove `PrintOut`/`ExportAsFixedFormat` pozive mimo kapije.** |
