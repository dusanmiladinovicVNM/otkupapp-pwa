...
' Identitet zbirne za join zbirna<->prijemnica.
' Vlasnik = VozacID + KupacID -- ISTA definicija koju koristi
' modStorno.RequireJedanVlasnikPoBroju (AUD-052 / RF-05: broj se generise po
' vozacu, a dokument pripada kupcu). NE uvoditi drugu definiciju vlasnika.
...
Public Function BuildManjakDict() As Object
    ...
    '   <vlasnikKljuc>              -> Array(ZbirnaKg, PrijemnicaKg, CntPrijem)
    '   "#V|" & broj                -> Long: koliko RAZLICITIH aktivnih vlasnika
    '                                  nosi taj BrojZbirne (1 = broj je pouzdan)
    ...
    '   "#C|" & broj / "#K|" & broj -> Long/Double: prijem agregiran po SAMOM broju
    '                                  (bez vlasnika). Sme se koristiti iskljucivo
    '                                  kad je "#V|" = 1, tj. kad je dokazano da broj
    '                                  ima jednog vlasnika; pokriva i starije
    '                                  prijemnice kojima vlasnik nije popunjen.
    ' Prefiks "#" se ne moze pojaviti u BrojZbirne (`x/ddmmyy[-rb]`), pa nema
    ' kolizije sa vlasnik kljucevima.
    ...
    ' broj -> Dictionary(vlasnikKljuc -> vozacID); sluzi za brojanje vlasnika i
    ' za razresenje po vozacu (OM putanja nema KupacID na otpremnici).
    ...
    For z = 1 To UBound(zbrData, 1)
        ...
        Dim zVoz As String, zKup As String
        zVoz = ""
        zKup = ""
        ...
        If Not dict.Exists(vk) Then dict.Add vk, Array(0#, 0#, 0&)
        Dim vals As Variant
        vals = dict(vk)
    ...
    For p = 1 To UBound(prijData, 1)
        ...
        Dim pVoz As String, pKup As String
        pVoz = ""
        pKup = ""
        ...
        ' Agregat po samom broju (validan samo kad broj ima jednog vlasnika).
        If Not dict.Exists("#C|" & pZbr) Then dict.Add "#C|" & pZbr, 0&
        dict("#C|" & pZbr) = CLng(dict("#C|" & pZbr)) + 1
        If Not dict.Exists("#K|" & pZbr) Then dict.Add "#K|" & pZbr, 0#
        dict("#K|" & pZbr) = CDbl(dict("#K|" & pZbr)) + pKg
        ...
        Dim pvk As String
        pvk = ZbirnaVlasnikKljuc(pZbr, pVoz, pKup)
        If dict.Exists(pvk) Then
            vals = dict(pvk)
            vals(1) = vals(1) + pKg
