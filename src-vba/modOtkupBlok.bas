...
Public Sub OtkupBlok_OnButton(ByVal action As String)
    ...
    Select Case action
        ...
        Case "KOOPRANG": ShowKoopRang
        Case "KOOPRANGCLOSE": HideKoopRang
    End Select
    Exit Sub
EH:
...
Private Sub TogglePanel()
    ...
    If mVisible Then
        LoadOtpremnice
        LoadBlokovi
        HideKoopRang               ' overlay ranga ostaje skriven dok se ne klikne dugme
...
Private Sub BuildPanel()
    ...
    WireBtn mBtnKoopRang, "KOOPRANG"
    WireBtn mBtnRangClose, "KOOPRANGCLOSE"
End Sub
...
Private Sub StornoSelectedBlok()
    ...
    ' Autohladnjaca: zapamti kontekst lanca PRE storna. Ako je stanica hladnjaca i
    ' prijemnica lanca (ista BrojZbirne) bila paletizovana, posle storna se nudi
    ' ISPRAVKA (prefill otkupa + auto-relink osirocenih paleta pri Unosu) ili
    ' DUPLI UNOS (odmah skini fantomske paleta-stavke).
...
' Autohladnjaca posle storna bloka sa paletizovanom prijemnicom -- tri opcije:
'   DA     = ISPRAVKA: pending relink + prefill otkupa iz storniranog; operater menja
'            gresku i klikne Unos (btnUnos: AutoChain bez paletizacije + relink paleta).
'   NE     = DUPLI UNOS: fantomske paleta-stavke se odmah skidaju (detach); paleta
'            koja ostane prazna se stornira.
'   OTKAZI = nista: palete ostaju osirocene (rucno kroz Osiroceni dokumenti -> Palete).
Private Sub OfferHladnjacaIspravka(ByVal brDok As String, ByVal prijBroj As String, ByVal palInfo As String)
    ...
    ElseIf ans = vbNo Then
        Dim di As String
        If DetachOsirocenePaletaStavke_TX(prijBroj, di) > 0 Then
            MsgBox di, vbInformation, APP_NAME
        Else
            MsgBox "Nista nije skinuto." & IIf(Len(di) > 0, " " & di, ""), vbExclamation, APP_NAME
        End If
    Else
        MsgBox "Palete su osirocene (i dalje broje robu). Resi rucno: " & _
...
' Popuni levu (otkup) formu iz (upravo storniranog) otkupa, da operater menja samo
' gresku. Klasa I -> osnovna polja; ako postoji red Klase II -> ukljuci chkDveKlase
' i popuni II. Broj otkupa se NE preuzima (predlog sledeceg dolazi iz stanica-Change).
' Kolicina: u BRUTO modu uzmi BrutoKg (original bruto), inace Kolicina (neto). Cena se
' postavlja POSLEDNJA (auto-fill iz _Change eventova bi je inace pregazio).
...
Private Sub PrefillOtkupFromStornirano(ByVal brDok As String)
    ...
    ' Datum PRE stanice (predlog broja i zbirna zavise od datuma).
    If cDat > 0 Then
...
' ============================================================
' KOOPERANT: ukupan iznos otkupnih listova (tekuca god.) + rang
' ============================================================

' Public hook (frmOtkup.cmbKooperant_Change): osvezi inline sumu za izabranog
' kooperanta. No-op ako panel nije izgradjen/otvoren (label postoji samo u panelu).
Public Sub OtkupBlok_RefreshKoopTotal()
    On Error GoTo EH
    If Not mBuilt Then Exit Sub
    If Not mVisible Then Exit Sub
    RefreshKoopTotalInline
    Exit Sub
EH:
    LogErr "modOtkupBlok.OtkupBlok_RefreshKoopTotal"
End Sub

Private Sub RefreshKoopTotalInline()
    On Error GoTo EH
    If mLblKoopTotal Is Nothing Then Exit Sub

    Dim koopID As String: koopID = CurrentKoopIDFromForm()
    mCurKoopID = koopID
    If Len(koopID) = 0 Then
        mLblKoopTotal.caption = ""     ' slobodan tekst / bez izbora -> prazno (bez skeniranja)
        Exit Sub
    End If
    ...
    Exit Sub
EH:
    LogErr "modOtkupBlok.RefreshKoopTotalInline"
End Sub

' KooperantID trenutnog izbora u cmbKooperant (bez auto-kreiranja): bound ID iz
' liste; slobodan tekst -> "" (pa se inline suma ne prikazuje dok se ne izabere).
Private Function CurrentKoopIDFromForm() As String
    On Error Resume Next
    Dim cmb As MSForms.ComboBox
    Set cmb = mForm.Controls("cmbKooperant")
    If cmb Is Nothing Then Exit Function
    CurrentKoopIDFromForm = GetComboID(cmb)
End Function

' Ukupan iznos izdatih otkupnih listova (Sum Kolicina*Cena = "Ukupna vrednost",
' Cena je BRUTO sa PDV nadoknadom) za kooperanta u datoj godini; bez storniranih.
Private Function KoopPrometYear(ByVal koopID As String, ByVal yr As Integer) As Double
    On Error GoTo EH
    ...
    For i = 1 To UBound(data, 1)
        If Trim$(CStr(data(i, cKoop))) = koopID Then
            If cDat = 0 Or RowYear(data(i, cDat)) = yr Then
                s = s + NumVal(data(i, cKol)) * NumVal(data(i, cCena))
            End If
        End If
    Next i
    KoopPrometYear = s
    Exit Function
EH:
    LogErr "modOtkupBlok.KoopPrometYear"
End Function
...
' Overlay rang: prikazi (populate + u prvi plan preko grida).
Private Sub ShowKoopRang()
    On Error GoTo EH
    ...
    On Error Resume Next
    mLstRang.ZOrder 0
    mLblRangTitle.ZOrder 0
    mBtnRangClose.ZOrder 0
    On Error GoTo 0
    Exit Sub
EH:
    LogErr "modOtkupBlok.ShowKoopRang"
End Sub

Private Sub HideKoopRang()
    On Error Resume Next
    If Not mLstRang Is Nothing Then mLstRang.Visible = False
    If Not mLblRangTitle Is Nothing Then mLblRangTitle.Visible = False
    If Not mBtnRangClose Is Nothing Then mBtnRangClose.Visible = False
End Sub
...
Public Sub OtkupBlok_Release()
    ...
    Set mLblKoopTotal = Nothing
    Set mBtnKoopRang = Nothing
    Set mLblRangTitle = Nothing
    Set mLstRang = Nothing
    Set mBtnRangClose = Nothing
    mCurKoopID = ""
    mBuilt = False
    mVisible = False
    mOrigWidth = 0
