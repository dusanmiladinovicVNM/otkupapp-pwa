...
Private Sub Test_DuplicateFakturaIsBlocked()
    ...
    zbrFix = ZbrZateceniRed(testDate, TEST_VOZ_ID, brojZbirne, TEST_KUP_ID, _
...
Private Sub Test_InvalidZbirnaInvalidClassDoesNotAppend()
    ...
    Dim result As String
    ' KANONSKI pisac, ne fixture: tvrdnja je o KAPIJI (nevalidna klasa se
    ' odbija), a fixture ne proverava nista -- merio bi samo sebe.
    result = ZbrKanonNacrt( _
        NextTestDate(), TEST_VOZ_ID, _
        TEST_PREFIX & "-BAD-ZBR-" & NewScenarioCode("BADCLASS"), _
        TEST_KUP_ID, "BAD", 100#, 1)
...
Private Sub Test_ZBR_ImportDvaUredjajaNeStapaDokumente()
    ...
    ' zaglavlje OSTAVLJA stavku bez dokumenta -- tacno onu korupciju koju strog
    ' citalac (StavkeZbirneRedovi) posle prijavljuje, i to u TUDJEM testu.
    ...
    '
    ' Osa je ZbirnaID (S4-2a). Do S5-3 je ovde stajala GeneracijaID, koju je
    ' uvoz posebno pisao (ApplyNovaGeneracijaID) -- taj pisac je obrisan, a
    ' kanonski pisac svakom pozivu daje NOV ZbirnaID po konstrukciji. Tvrdnja
    ' se zato ne brise nego seli na osu koja stvarno nosi identitet.
    AssertTrue (UCase$(idA) <> UCase$(idB)), _
        "A21/KR-001: drugi uredjaj NE nasledjuje identitet prvog"
...
'
' Poredjenje sadrzaja je stajalo pod "If IsoUDatum(...) And IsDate(...)", pa je
' nevalidan datum TIHO ispadao iz poredjenja. Ako se sve ostalo poklopi, red bi
' dobio Duplicate -- a Duplicate je TERMINALAN (import uzima samo Pending), pa bi
' pokvaren red zauvek nestao, a master ostao na staroj verziji.
'
' ValidatePWAZbirna tu ne pomaze: ona se zove tek za NOV red, POSLE ove grane.
' Zato kapija mora da stoji bas u poredjenju.
Private Sub Test_ZBR_IstiCridNevalidanDatumNijeDuplikat()
    Dim tx As clsTransaction

    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBRDAT")
    ...
    zbrID = modMasterSync.TestHook_ImportZbirnaRowPWA( _
                ...
                100#, broj, crid)
    AssertTrue Len(zbrID) > 0, "ZBR datum preduslov: zbirna je uvezena"
    If Len(zbrID) = 0 Then GoTo Kraj

    ' Sve isto OSIM datuma, koji nije datum. Bez kapije bi ovo bilo "nema
    ' razlike" -> Duplicate -> terminalno.
    Dim r As String
    r = modMasterSync.TestHook_PwaZbirnaRazlika( _
            zbrID, zbrCrid, TEST_VOZ_ID, TEST_KUP_ID, "nije-datum", broj, crid)

    AssertTrue Len(r) > 0, "ZBR datum: nevalidan datum NIJE duplikat"
    AssertTrue InStr(1, r, "Datum", vbTextCompare) > 0, _
               "ZBR datum: razlog imenuje datum (bilo: " & r & ")"

    ' Kontrola: sa ISPRAVNIM datumom isti red je i dalje uredan NO-OP -- inace bi
    ' kapija koja odbija SVE prosla gornju tvrdnju iz pogresnog razloga.
    AssertEquals "", modMasterSync.TestHook_PwaZbirnaRazlika( _
                         zbrID, zbrCrid, TEST_VOZ_ID, TEST_KUP_ID, danas, broj, crid), _
                 "ZBR datum: sa ispravnim datumom isti red ostaje NO-OP"
...
EH:
    ...
    tx.RollbackTx
    On Error GoTo 0
    LogFatal "Test_ZBR_IstiCridNevalidanDatumNijeDuplikat", eN, eD
End Sub
...
' kojim je putem stigao. Bez njih uvoz nema po cemu da prepozna da je isti zapis
' vec video -- IsDuplicateZbirnaInMaster cita bas tu kolonu -- pa bi svaka PWA
' zbirna dolazila ponovo pri SVAKOM ciklusu, i svaki put pravila nov dokument.
'
' Pisu se u kanonskom piscu, ne naknadnim UpdateCell-om iz modMasterSync:
' tblZbirna sme da ima jednog pisca (A11). Zato se i mere kroz produkcioni uvoz,
' a ne pozivom pisca sa rucno sklopljenim zaglavljem.
Private Sub Test_ZBR_UvozPamtiPoreklo()
    Dim tx As clsTransaction

    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBRPOR")
    ...
    zbrID = modMasterSync.TestHook_ImportZbirnaRowPWA( _
                zbrCrid, TEST_VOZ_ID, TEST_KUP_ID, danas, TEST_VRSTA, TEST_SORTA, _
                100#, modBrojevi.FormatBroj(TEST_VOZ_ID, danas, 1), crid)

    AssertTrue Len(zbrID) > 0, "ZBR poreklo: uvoz je prosao"
    If Len(zbrID) = 0 Then GoTo Kraj

    AssertEquals zbrCrid, _
                 Trim$(NzToText(LookupValue(TBL_ZBIRNA, COL_ZBR_ID, zbrID, _
                                            COL_ZBR_CLIENT_RECORD_ID))), _
...
Private Function ZbrOtpremnicaNaDan(ByVal oznaka As String, ByVal datum As Date, _
    ...
    predaje.Add PredajaRed(2, otkID, TEST_VOZ_ID, "PRED-" & oznaka, _
    ...
    If modMasterSync.TestHook_CreateOtpremniceIzPredaje(predaje, ishodi, greske) <> 1 Then
        Err.Raise vbObjectError + 9431, "ZbrOtpremnicaNaDan", _
                  "Predaja nije napravila otpremnicu: " & greske
    End If

    ZbrOtpremnicaNaDan = modDokumenta.OtpremnicaZaOtkup(otkID)
End Function

' Zaglavlje zbirne na zadat dan.
Private Function ZbrHeaderNaDan(ByVal broj As String, ByVal datum As Date) As Object
    Dim h As Object
    Set h = CreateObject("Scripting.Dictionary")
    h.Add "Datum", datum
    h.Add "VozacID", TEST_VOZ_ID
    h.Add "KupacID", TEST_KUP_ID
    h.Add "BrojZbirne", broj
    Set ZbrHeaderNaDan = h
End Function
...
Private Sub Test_ManjakPreviewJeZbirnaMinusPrijem()
    ...
    zbrI = ZbrZateceniRed(testDate, TEST_VOZ_ID, brojZbirne, TEST_KUP_ID, _
                         "Test Hladnjaca", "Test Pogon", TEST_VRSTA, TEST_SORTA, _
                         100#, TEST_TIP_AMB, 10, KLASA_I)
    zbrII = ZbrZateceniRed(testDate, TEST_VOZ_ID, brojZbirne, TEST_KUP_ID, _
                          "Test Hladnjaca", "Test Pogon", TEST_VRSTA, TEST_SORTA, _
...
Private Sub Test_StornoPoBrojuOdbijaDvaVlasnika()
    ...
    AssertTrue Len(ZbrZateceniRed(testDate, TEST_VOZ_ID, brojZbirne, TEST_KUP_ID, _
                                 "Test Hladnjaca", "Test Pogon", TEST_VRSTA, TEST_SORTA, _
...
Private Sub Test_ZBR_PaletaNasledjujeGeneracijuPrijemnice()
    ...
    zbrA = ZbrZateceniRed(testDate, TEST_VOZ_ID, brojA, TEST_KUP_ID, _
                         "Test Hladnjaca", "Test Pogon", TEST_VRSTA, TEST_SORTA, _
                         100#, TEST_TIP_AMB, 10, KLASA_I)
...
' 14.09.2026).
'
' Do S4-2c/2a je ovo merio stari pisac, pa je uz tvrdnju o broju stajala i
' tvrdnja "dvoklasna zbirna je dva reda". Ta druga je NAMERNO nestala sa piscem:
' u kanonu je dvoklasna zbirna JEDAN header i dve stavke, pa tvrdnja vise nema
' predmet. Tvrdnja o broju je ostala, i meri je kanonski nacrt
' (ZbrNapraviDraft -> modBrojevi.RequireBrojSlobodanUNizu).
Private Sub Test_ZBR_StorniranBrojIstogVozacaOdbijen()
    ...
    Dim prvi As String
    prvi = ZbrKanonNacrt(d, TEST_VOZ_ID, broj, TEST_KUP_ID, KLASA_I, 100#, 10)
    AssertTrue Len(prvi) > 0, "ZBR broj: prvi nacrt je upisan"
    If Len(prvi) = 0 Then Exit Sub

    AssertEquals CStr(pre + 1), CStr(CountRows(TBL_ZBIRNA)), _
                 "ZBR broj: kanonska zbirna je JEDAN red zaglavlja"

    MarkTestRowStornirano TBL_ZBIRNA, COL_ZBR_ID, prvi

    pre = CountRows(TBL_ZBIRNA)
    AssertEquals "", ZbrKanonNacrt(d, TEST_VOZ_ID, broj, TEST_KUP_ID, KLASA_I, 100#, 10), _
    ...
    AssertTrue Len(ZbrKanonNacrt(d, TEST_VOZ_ID, broj & "-2", TEST_KUP_ID, _
                                 KLASA_I, 100#, 10)) > 0, _
...
' ZAGLAVLJE SE GRADI PO IMENU KOLONE, NE PO POZICIJI.
'
' Do S4-2c/2a je ovo merilo stari pisac (BuildZbirnaRowData). Pisac je obrisan,
' ali tvrdnja nije nestala nego se preselila: BuildZbirnaHeaderRowData gradi red
' kroz SetRowValueByColumn, pa svaka vrednost mora zavrsiti u SVOJOJ koloni.
' Pozicijski Array(...) bi to garantovao samo dok je redosled kolona tacno onakav
' kakav kod pretpostavlja.
'
' Drugi deo tvrdnje je KANONSKI i nov: zaglavlje NE nosi kolicinu, klasu ni gajbe
' (one su stavke), a vrstu, sortu i tip ambalaze nacrt dobija tek od prvog izvora
' (ZBR-KANON-04). Prazno ovde nije propust nego ugovor -- i bas to razlikuje
' kanonski red od zatecenog.
Private Sub Test_ZbirnaRowDataColumnMapped()
    ...
    Dim zbrID As String
    zbrID = ZbrKanonNacrt(testDate, TEST_VOZ_ID, brojZbirne, TEST_KUP_ID, _
                          KLASA_II, 123.45, 7, "Test Hladnjaca", "Test Pogon")
    ...
    AssertEquals "Test Hladnjaca", ZbrPolje(zbrID, COL_ZBR_HLADNJACA), _
                 "Zbirna mapiranje: Hladnjaca"
    AssertEquals "Test Pogon", ZbrPolje(zbrID, COL_ZBR_POGON), "Zbirna mapiranje: Pogon"

    ' Kolicina, klasa i gajbe su STAVKE -- zaglavlje ih ne nosi.
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_KOLICINA), _
                 "Zbirna mapiranje: zaglavlje BEZ kolicine"
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_KLASA), "Zbirna mapiranje: zaglavlje BEZ klase"
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_KOL_AMB), "Zbirna mapiranje: zaglavlje BEZ gajbi"

    ' Cinjenice robe donosi prvi izvor, pa ih nacrt jos nema (ZBR-KANON-04).
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_VRSTA), "Zbirna mapiranje: nacrt BEZ vrste"
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_SORTA), "Zbirna mapiranje: nacrt BEZ sorte"
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_TIP_AMB), _
                 "Zbirna mapiranje: nacrt BEZ tipa ambalaze"

    ' Kilaza je na STAVCI, i to u svojoj koloni.
    AssertTrue Abs(ZbrKg(zbrID, KLASA_II) - 123.45) < 0.001, _
               "Zbirna mapiranje: kilaza je na stavci"
...
Private Function AutoOtpFixture(ByVal datum As Date, ByVal stanicaID As String, _
                                ...
                                Optional ByVal kulturaID As String = "", _
                                Optional ByVal crid As String = "") As String
    Dim h As Object
    Set h = CreateObject("Scripting.Dictionary")
    h.Add "Datum", datum
    h.Add "KooperantID", TEST_KOOP_ID
...
' OTKUP SA CRID-OM, PREDAT VOZACU -> CRID upotrebljiv kao izvor zbirne.
'
...
Private Function ZbrPwaIzvorCrid(ByVal oznaka As String, ByVal datum As Date, _
    ...
    Dim otkID As String
    otkID = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-ZI-" & oznaka, _
                           KLASA_I, 400#, 250#, 20#, TEST_TIP_AMB, "", crid)

    Dim predaje As Collection
    Set predaje = New Collection
    predaje.Add PredajaRed(2, otkID, vozacID, "PRED-ZI-" & oznaka, _
                           PredajaIsoDatum(datum))
...
'
' Manifest i CRID su opcioni u potpisu SAMO zbog citljivosti testova koji mere
' jednoclanu predaju -- tada je manifest bas taj jedan clan.
Private Function PredajaRed(ByVal redIdx As Long, ByVal otkupID As String, _
                            ...
                            ByVal predatoAt As String, _
                            Optional ByVal crid As String = "", _
                            Optional ByVal manifest As String = "") As Variant
    Dim c As String, m As String
    c = crid
    If Len(c) = 0 Then c = "CRID-" & otkupID
    m = manifest
    If Len(m) = 0 Then m = c

    PredajaRed = Array(redIdx, otkupID, vozacID, predajaID, predatoAt, m, c)
End Function
...
'
' Prvi krug je sprecio da ZAKASNELI blok napravi DRUGU otpremnicu. Ali nije
' sprecio ono gore: da PRVI ciklus IZDA nepotpun dokument. GAS obradjuje redove
' pojedinacno, pa od klika A+B+C ume da stigne samo A+B -- i master je to video
' kao zavrsen utovar.
'
' Izdata otpremnica se ne dopunjuje (A13), pa je C zauvek ostajao napolju. Test
' to zove pravim imenom: dokument koji tvrdi manje nego sto je natovareno.
'
' Resenje nije jos jedna heuristika nego MANIFEST: red nosi spisak svih clanova
' tog klika, pa master zna kad je utovar CEO. Nepotpun utovar ne dobija ni
' dokument ni status -- redovi ostaju Pending i vracaju se sledecim ciklusom.
Private Sub Test_OTP_NepotpunUtovarNeDobijaDokument()
    Dim tx As clsTransaction

    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("PREDNP")
    ...
    Dim otkA As String, otkB As String
    otkA = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-NPA-" & scenario, _
                          KLASA_I, 400#, 250#, 20#, TEST_TIP_AMB)
    otkB = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-NPB-" & scenario, _
                          KLASA_II, 300#, 150#, 15#, TEST_TIP_AMB)

    Dim predajaID As String, cridA As String, cridB As String, cridC As String
    predajaID = "PRED-NP-" & scenario
    cridA = "CRID-NPA-" & scenario
    cridB = "CRID-NPB-" & scenario
    cridC = "CRID-NPC-" & scenario

    ' Manifest kaze TRI clana; stigla su dva.
    Dim manifest As String
    manifest = cridA & "," & cridB & "," & cridC
    ...
    AssertEquals "", modDokumenta.OtpremnicaZaOtkup(otkA), _
                 "NEPOTPUN: nijedan blok nije vezan dok utovar nije ceo"
    AssertEquals "", greske, "NEPOTPUN: cekanje NIJE greska"

    ' Redovi ostaju bez ishoda -> Pending -> sledeci ciklus ih opet donese.
    AssertFalse ishodi.Exists(2&), "NEPOTPUN: red ostaje Pending, bez statusa"

    ' --- SLEDECI CIKLUS: stigao i treci clan --------------------------------
    Dim otkC As String
    otkC = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-NPC-" & scenario, _
                          KLASA_I, 200#, 250#, 10#, TEST_TIP_AMB)
    ...
    predaje.Add PredajaRed(3, otkB, TEST_VOZ_ID, predajaID, PredajaIsoDatum(datum), _
                           cridB, manifest)
    predaje.Add PredajaRed(4, otkC, TEST_VOZ_ID, predajaID, PredajaIsoDatum(datum), _
                           cridC, manifest)

    AssertEquals "1", _
                 CStr(modMasterSync.TestHook_CreateOtpremniceIzPredaje(predaje, ishodi, greske)), _
                 "NEPOTPUN: kad je utovar CEO, nastaje JEDAN dokument"

    Dim otp As String
    otp = modDokumenta.OtpremnicaZaOtkup(otkA)
    AssertTrue Len(otp) > 0, "NEPOTPUN: blok A je vezan"
    If Len(otp) = 0 Then GoTo Kraj

    AssertEquals "3", CStr(modDokumenta.IzvoriOtpremnice(otp).count), _
                 "NEPOTPUN: dokument nosi SVA TRI clana utovara"
...
EH:
    ...
    tx.RollbackTx
    On Error GoTo 0
    LogFatal "Test_OTP_NepotpunUtovarNeDobijaDokument", eN, eD
End Sub

' IDENTITET UTOVARA PREZIVLJAVA ISPRAVKU (review #388, drugi krug P1).
'
' Ispravka pravi NOV dokument i stornira stari. Dok PredajaID nije isao sa njim,
' nova verzija je ostajala bez identiteta utovara -- a OtpremnicaPoPredaji
' namerno gleda samo AKTIVNE. Posledica: cim otpremnica prodje kroz ispravku,
' zakasneli blok istog utovara opet moze da napravi SVOJ dokument. Ista greska
' koju je prvi krug zatvorio, samo kroz drugi lifecycle.
'
' Test meri oba koraka: da nova verzija nosi identitet, i da zakasneli blok
' POSLE ispravke i dalje staje -- i to imenujuci NOVU otpremnicu.
Private Sub Test_OTP_IspravkaCuvaIdentitetUtovara()
    Dim tx As clsTransaction

    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("PREDISP")
    ...
    otkA = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-ISA-" & scenario, _
                          KLASA_I, 400#, 250#, 20#, TEST_TIP_AMB)
    otkC = AutoOtpFixture(datum, TEST_ST_ID, TEST_PREFIX & "-OTK-ISC-" & scenario, _
                          KLASA_II, 300#, 150#, 15#, TEST_TIP_AMB)

    Dim predajaID As String, cridA As String
    predajaID = "PRED-IS-" & scenario
    cridA = "CRID-ISA-" & scenario
    ...
    AssertEquals "1", _
                 CStr(modMasterSync.TestHook_CreateOtpremniceIzPredaje(predaje, ishodi, greske)), _
                 "PREDAJA isp preduslov: utovar je postao otpremnica"

    Dim otpPrva As String
    otpPrva = modDokumenta.OtpremnicaZaOtkup(otkA)
    AssertTrue Len(otpPrva) > 0, "PREDAJA isp preduslov: blok je vezan"
    If Len(otpPrva) = 0 Then GoTo Kraj

    ' --- ISPRAVKA: nova verzija istog fizickog utovara ----------------------
    Dim g As String, otpNova As String
    otpNova = modDokumenta.IspravkaOtpremnice_TX(otpPrva, g)
    AssertTrue Len(otpNova) > 0, "PREDAJA isp: ispravka je napravila novu verziju (" & g & ")"
    If Len(otpNova) = 0 Then GoTo Kraj

    AssertEquals predajaID, OtpPolje(otpNova, COL_OTP_PREDAJA_ID), _
                 "PREDAJA isp: nova verzija NOSI identitet utovara"

    ' --- ZAKASNELI BLOK POSLE ISPRAVKE --------------------------------------
    '
    ' Bez prenosa identiteta bi ovde nastao TRECI dokument za isti utovar.
    Set predaje = New Collection
    predaje.Add PredajaRed(5, otkC, TEST_VOZ_ID, predajaID, PredajaIsoDatum(datum), _
                           "CRID-ISC-" & scenario, "CRID-ISC-" & scenario)

    AssertEquals "0", _
                 CStr(modMasterSync.TestHook_CreateOtpremniceIzPredaje(predaje, ishodi, greske)), _
                 "PREDAJA isp: zakasneo blok NE pravi dokument ni posle ispravke"
    AssertTrue InStr(1, greske, otpNova, vbTextCompare) > 0, _
               "PREDAJA isp: razlog imenuje NOVU verziju (bilo: " & greske & ")"
...
EH:
    ...
    tx.RollbackTx
    On Error GoTo 0
    LogFatal "Test_OTP_IspravkaCuvaIdentitetUtovara", eN, eD
End Sub
...
Private Sub Test_OTP_PredajaJeJedanUtovar()
    ...
    ' Manifest je ISTI za sve redove jednog klika -- to i jeste njegova poenta.
    Dim predajaID As String, cridS As String, cridN As String, manifest As String
    predajaID = "PRED-" & scenario
    cridS = "CRID-P1S-" & scenario
    cridN = "CRID-P1N-" & scenario
    manifest = cridS & "," & cridN

    Dim predaje As Collection
    Set predaje = New Collection
    predaje.Add PredajaRed(2, otkStari, TEST_VOZ_ID, predajaID, PredajaIsoDatum(danas), _
                           cridS, manifest)
    predaje.Add PredajaRed(3, otkNovi, TEST_VOZ_ID, predajaID, PredajaIsoDatum(danas), _
                           cridN, manifest)
...
Private Sub Test_OTP_PredajaMesanihVrstaSeOdbija()
    ...
    Dim cridMA As String, cridMB As String, manifestM As String
    cridMA = "CRID-MA-" & scenario
    cridMB = "CRID-MB-" & scenario
    manifestM = cridMA & "," & cridMB

    Dim predaje As Collection
    Set predaje = New Collection
    predaje.Add PredajaRed(2, otkA, TEST_VOZ_ID, "PRED-M-" & scenario, _
                           PredajaIsoDatum(datum), cridMA, manifestM)
    predaje.Add PredajaRed(3, otkB, TEST_VOZ_ID, "PRED-M-" & scenario, _
                           PredajaIsoDatum(datum), cridMB, manifestM)
...
Private Sub Test_HLD_AutoLanacOtpremnica()
    ...
    ' --- 5) hladnjaca BEZ ogledala: lanac ga napravi, ne staje -----------
    ' Ogledalo nije podatak o coveku nego proxy za stanicu, pa ga lanac trazi
    ' kanonskim idempotentnim upisom. Van malina rezima ovo je JEDINI put koji ga
    ' za hladnjacu pravi -- bez toga bi lanac tamo uvek stajao.
    Dim bezOgledala As String, otp5 As String
    AssertTrue Not modMalina.IsManagedStationMirror(TEST_HLAD2_ST_ID), _
               "Lanac: druga hladnjacka stanica pocinje bez ogledala"
    bezOgledala = CreateOtkup_TX(OtkHeaderNaStanici(TEST_PREFIX & "-OTK-HLDD-" & scenario, TEST_HLAD2_ST_ID), _
                                 OtkStavke(10#, 100#, 1, 0#, 0#, 0))
    izvestaj = ""
    otp5 = modAutoHladnjaca.AutoLanacHladnjaca(bezOgledala, izvestaj)
    AssertTrue Len(otp5) > 0, "Lanac: ogledalo nastaje kroz lanac (" & izvestaj & ")"
    AssertTrue modMalina.IsManagedStationMirror(TEST_HLAD2_ST_ID), _
               "Lanac: posle lanca stanica ima svoje ogledalo"
    If Len(otp5) > 0 Then
        AssertEquals TEST_HLAD2_ST_ID, OtpPolje(otp5, COL_OTP_VOZAC), _
                     "Lanac: vozac je ogledalo bas te stanice"
    End If
    ...
    ' --- 8) ROUTER PO STANICI: odluka PRE nego sto blok postoji -----------
    ' Ekran ovim grana pre potvrde prekoracenja: hladnjacki blok ne podleze
    ' ogranicenju tudjeg rucnog nacrta, pa mu se to pitanje i ne postavlja.
    ' (Da se pitalo, operater bi na "Ne" izgubio ceo upis zbog dokumenta sa
    ' kojim blok nema veze.)
    Dim stGreska As String
    stGreska = "x"
    AssertTrue modAutoHladnjaca.LanacVaziZaStanicu(TEST_HLAD_ST_ID, stGreska), _
               "Lanac: hladnjacka stanica ide u lanac (router po stanici)"
    AssertEquals "", stGreska, "Lanac: hladnjacka stanica je pouzdan odgovor"

    stGreska = "x"
    AssertTrue Not modAutoHladnjaca.LanacVaziZaStanicu(TEST_ST_ID, stGreska), _
               "Lanac: obicna stanica ne ide u lanac (router po stanici)"
    AssertEquals "", stGreska, "Lanac: obicna stanica je pouzdan odgovor"

    stGreska = ""
    AssertTrue Not modAutoHladnjaca.LanacVaziZaStanicu("ST-NEMA-" & scenario, stGreska), _
               "Lanac: nepoznata stanica ne ide u lanac"
    AssertTrue Len(stGreska) > 0, _
               "Lanac: nepoznata stanica vraca RAZLOG i u routeru po stanici"

    ' --- 9) HLADNJACKI BLOK SE NE VEZUJE RUCNO ----------------------------
    ' Oporavak posle pale automatike je "otkloni uzrok pa PONOVI lanac", a ne
    ' "veze ga za prvu otvorenu otpremnicu" -- inace se obavezan lanac zaobilazi
    ' jednim klikom iz liste "Bez otpremnice". Kapija je na granici radnje.
    Dim rucni As String, hldSlobodan As String, obicanSlobodan As String, razlog As String
    prev = modOtkupUI.ActiveMode
    modOtkupUI.ActiveMode = "F1"
    rucni = CreateOtpremnicaDraft_TX(OtpHeaderNaStanici(TEST_PREFIX & "-OTP-HLDR-" & scenario, TEST_ST_ID), _
                                     OtpOcek(100#, 10#, 0#, 0#), g)
    AssertTrue Len(rucni) > 0, "Lanac: rucni nacrt za vezivanje napravljen (" & g & ")"
    If Len(rucni) > 0 Then
        AssertEquals "", modScrDokumenti.AktivirajOtpremnicu(rucni), _
                     "Lanac: rucni nacrt je aktivan (vezivanje)"

        hldSlobodan = CreateOtkup_TX(OtkHeaderNaStanici(TEST_PREFIX & "-OTK-HLDI-" & scenario, TEST_HLAD2_ST_ID), _
                                     OtkStavke(8#, 100#, 1, 0#, 0#, 0))
        razlog = modScrDokumenti.VeziZaAktivnu(hldSlobodan)
        AssertTrue Len(razlog) > 0, "Lanac: hladnjacki blok se ne vezuje rucno"
        AssertEquals "", modDokumenta.OtpremnicaZaOtkup(hldSlobodan), _
                     "Lanac: odbijen blok nije zavrsio u rucnom nacrtu"

        ' Kontrola: obican blok se vezuje normalno -- kapija gadja bas hladnjacu.
        obicanSlobodan = CreateOtkup_TX(OtkHeader(TEST_PREFIX & "-OTK-HLDJ-" & scenario), _
                                        OtkStavke(9#, 100#, 1, 0#, 0#, 0))
        AssertEquals "", modScrDokumenti.VeziZaAktivnu(obicanSlobodan), _
                     "Lanac: obican blok se i dalje vezuje rucno"

        ' Oporavak: ponavljanje lanca nad tim istim blokom uspeva.
        izvestaj = ""
        AssertTrue Len(modAutoHladnjaca.AutoLanacHladnjaca(hldSlobodan, izvestaj)) > 0, _
                   "Lanac: odbijen blok se oporavlja ponavljanjem lanca (" & izvestaj & ")"
    End If
    RadniStoPocetno
    modOtkupUI.ActiveMode = prev
...
' ZATECENI OBLIK ZAGLAVLJA ZBIRNE -- FIXTURE, NE PISAC (S4-2c/2a).
'
' Stari pisac (SaveZbirna / _TX / Multi_TX) je obrisan u ovom rezu. Testovi koji
' su ga zvali uglavnom nisu merili NJEGA nego nesto nizvodno: manjak, generaciju
' palete, otvorene fakture, storno po broju. Njima treba RED, ne pisac.
'
' Fixture pravi ZATECENI oblik: kolicina, klasa i gajbe NA ZAGLAVLJU, plus
' stavka (da strog citalac ne odbije ceo registar). Taj oblik kanonski pisac vise
' NE pravi -- namerno ostavlja te kolone prazne -- a citaoci koji ih jos citaju
' (popis: grupa zbr_linija) nestaju u S3e-2. Tada nestaje i ovaj fixture. Do tada
' bi seed u kanonskom obliku tim citaocima davao NULU, pa bi test "prolazio" ne
' merivsi nista.
'
' Generacija se pecati ISTIM scope-om koji je pecatio stari pisac (broj + vozac +
' kupac) -- testovi generacije mere bas taj scope.
'
' STA FIXTURE NE RADI: ne proverava NISTA. Testovi koji su merili KAPIJE starog
' pisca (nevalidna klasa, zauzet broj, tudji vlasnik broja) presli su na kanonski
' pisac; da su ostali ovde, tvrdnja bi im postala placebo.
Private Function ZbrZateceniRed(ByVal datum As Date, ByVal vozacID As String, _
    ...
    Const SRC As String = "ZbrZateceniRed"

    Dim zbirnaID As String
    zbirnaID = GetNextID(TBL_ZBIRNA, COL_ZBR_ID, "ZBR-")
    If Len(zbirnaID) = 0 Then
        Err.Raise vbObjectError + 9310, SRC, "GetNextID nije vratio ZbirnaID."
    End If
    ...
    SetRequiredField rowData, TBL_ZBIRNA, COL_ZBR_VOZAC, vozacID
    SetRequiredField rowData, TBL_ZBIRNA, COL_ZBR_BROJ, brojZbirne
    SetRequiredField rowData, TBL_ZBIRNA, COL_ZBR_KUPAC, kupacID
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_HLADNJACA, hladnjaca
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_POGON, pogon
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_VRSTA, vrstaVoca
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_SORTA, sortaVoca
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_KOLICINA, ukupnoKol
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_TIP_AMB, tipAmb
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_KOL_AMB, ukupnoAmb
    SetOptionalField rowData, TBL_ZBIRNA, COL_ZBR_KLASA, klasa
    SetOptionalField rowData, TBL_ZBIRNA, COL_STORNIRANO, ""

    Dim r As Long
    r = AppendRow(TBL_ZBIRNA, rowData)
    If r <= 0 Then
        Err.Raise vbObjectError + 9311, SRC, "AppendRow za tblZbirna nije uspeo."
    End If

    ApplyGeneracijaID TBL_ZBIRNA, r, COL_ZBR_BROJ, brojZbirne, _
                      COL_ZBR_VOZAC, vozacID, COL_ZBR_KUPAC, kupacID

    ZbrZateceniRed = zbirnaID
    ZbrStavkaUzZateceni zbirnaID, klasa, ukupnoKol, ukupnoAmb
End Function

' KANONSKI NACRT ZBIRNE -- za testove koji mere KAPIJE pisca, ne oblik reda.
Private Function ZbrKanonNacrt(ByVal datum As Date, ByVal vozacID As String, _
                               ByVal brojZbirne As String, ByVal kupacID As String, _
                               ByVal klasa As String, ByVal kol As Double, _
                               ByVal amb As Double, _
                               Optional ByVal hladnjaca As String = "", _
                               Optional ByVal pogon As String = "") As String
    Dim h As Object
    Set h = CreateObject("Scripting.Dictionary")
    h.Add "Datum", datum
    ...
    h.Add "Hladnjaca", hladnjaca
    h.Add "Pogon", pogon

    Dim g As String
    ZbrKanonNacrt = CreateZbirnaDraft_TX(h, ZbrOcek(klasa, kol, amb), g)
End Function

Private Sub ZbrStavkaUzZateceni(ByVal zbirnaID As String, ByVal klasa As String, _
    ...
    ZbrDodajStavku zbirnaID & "-S1", zbirnaID, _
                   ...
                   klasa, kol, amb, "ZbrStavkaUzZateceni"
End Sub
...
' OBA KANONSKA ULAZA IMAJU ISTU DEFINICIJU VALJANOG IZVORA (review #372, P1).
'
' Do ovog reza je ista DRAFT otpremnica bila odbijena na putu nacrta i primljena
' na jednopoteznom -- pa je zbirna mogla da nastane IZDATA iz robe koja nije
' otisla. Test meri BAS taj par: isti izvor, dva ulaza, isti ishod.
Private Sub Test_ZBR_ObaUlazaTrazeIzdatIzvor()
    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBROU")

    Dim gg As String, nacrtOtp As String
    nacrtOtp = CreateOtpremnicaDraft_TX( _
                   OtpHeader(TEST_PREFIX & "-OTP-OU-" & scenario), _
                   OtpOcek(200#, 10#, 0#, 0#), gg)
    AssertTrue Len(nacrtOtp) > 0, "ZBR oba ulaza: nacrt otpremnice napravljen"
    If Len(nacrtOtp) = 0 Then Exit Sub

    ' --- jedan potez ---
    Dim g As String, rez As String
    rez = CreateZbirnaIzIzvora_TX(Pr3Header(TEST_PREFIX & "-ZBR-OU1-" & scenario), _
                                  Pr3Izvor(nacrtOtp, ""), g)
    AssertEquals "", rez, "ZBR oba ulaza: jedan potez ODBIJA nacrt otpremnice"
    AssertTrue InStr(1, g, "nije izdata", vbTextCompare) > 0, _
               "ZBR oba ulaza: jedan potez imenuje razlog (bilo: " & g & ")"
    ...
    Exit Sub
EH:
    LogFatal "Test_ZBR_ObaUlazaTrazeIzdatIzvor", Err.Number, Err.description
End Sub

' PRVI IZVOR DEFINISE VRSTU, SORTU I TIP AMBALAZE (review #372, P1).
'
' Nacrt ih ne moze primiti kroz zaglavlje -- HdrProveriKljuceve ih izricito ne
' dozvoljava, jer su cinjenica ROBE. Zato nacrt krece prazan, prvi izvor ih
' upisuje, a izdata zbirna ih NOSI. Bez toga bi svaka zbirna zavrsila bez vrste
' i sorte, a da nijedna kapija ne pisne.
Private Sub Test_ZBR_PrviIzvorDefiniseCinjenice()
    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBRCI")
    ...
    AssertEquals "", ZbrPolje(zbrID, COL_ZBR_VRSTA), _
                 "ZBR cinjenice: nacrt krece BEZ vrste"

    AssertTrue modDokumenta.DodajZbirnaIzvor_TX(zbrID, otp, g), _
               "ZBR cinjenice: izvor dodat (" & g & ")"

    AssertEquals TEST_VRSTA, ZbrPolje(zbrID, COL_ZBR_VRSTA), _
                 "ZBR cinjenice: prvi izvor je definisao vrstu"
    AssertEquals TEST_SORTA, ZbrPolje(zbrID, COL_ZBR_SORTA), _
                 "ZBR cinjenice: prvi izvor je definisao sortu"
    AssertEquals TEST_TIP_AMB, ZbrPolje(zbrID, COL_ZBR_TIP_AMB), _
                 "ZBR cinjenice: prvi izvor je definisao tip ambalaze"

    AssertTrue modDokumenta.IzdajZbirnu_TX(zbrID, g), _
               "ZBR cinjenice: zbirna se izdaje (" & g & ")"
    AssertEquals TEST_VRSTA, ZbrPolje(zbrID, COL_ZBR_VRSTA), _
                 "ZBR cinjenice: izdata zbirna NOSI vrstu"

    Exit Sub
EH:
    LogFatal "Test_ZBR_PrviIzvorDefiniseCinjenice", Err.Number, Err.description
End Sub

' PISAC I CITALAC TRAZE ISTO (review #372, P1).
'
' RequireCeoBroj meri samo celobrojnost, pa je -10 gajbi prolazilo kroz pisca i
' padalo tek kod strogog citaoca. Dokument koji pisac napravi a citalac odbija je
' najgora vrsta kvara: nastaje bez greske, a rusi svaki kasniji prikaz.
Private Sub Test_ZBR_NegativnaAmbalazaSeNeUpisuje()
    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBRNA")

    Dim pre As Long
    pre = Pr3BrojRedova(TBL_ZBIRNA)

    Dim g As String, zbrID As String
    zbrID = CreateZbirnaDraft_TX(Pr3Header(TEST_PREFIX & "-ZBR-NA-" & scenario), _
                                 ZbrOcek(KLASA_I, 400#, -10#), g)

    AssertEquals "", zbrID, "ZBR negativna ambalaza: nacrt je odbijen"
    AssertTrue InStr(1, g, "negativna", vbTextCompare) > 0, _
               "ZBR negativna ambalaza: kapija imenuje razlog (bilo: " & g & ")"
    AssertEquals CStr(pre), CStr(Pr3BrojRedova(TBL_ZBIRNA)), _
                 "ZBR negativna ambalaza: zaglavlje nije ostalo"

    Exit Sub
EH:
    LogFatal "Test_ZBR_NegativnaAmbalazaSeNeUpisuje", Err.Number, Err.description
End Sub

' RED BEZ IDENTITETA JE GORI OD PADA (review #372, P1).
'
' Stavka nacrta i red clanstva su do ovog reza legali bez provere PK-a, pa bi
' prazan NewEntityID prosao kroz commit. Takav red nijedna kasnija radnja ne moze
' da pogodi. Seam NewEntityIDPadniTest je jedini nacin da se pokaze da kapija
' stvarno grize.
Private Sub Test_ZBR_PrazanIDDetetaNeProlazi()
    Dim prevMode As Boolean
    On Error GoTo EH

    Dim scenario As String
    scenario = NewScenarioCode("ZBRPI")
    ...
    ' --- stavka nacrta: zaglavlje prodje, stavka pada ---
    Dim pre As Long
    pre = Pr3BrojRedova(TBL_ZBIRNA)

    Dim g As String, zbrID As String
    modDataAccess.NewEntityIDPadniTest True, 1
    zbrID = CreateZbirnaDraft_TX(Pr3Header(TEST_PREFIX & "-ZBR-PI-" & scenario), _
                                 ZbrOcek(KLASA_I, 400#, 20#), g)
    modDataAccess.NewEntityIDPadniTest False

    AssertEquals "", zbrID, "ZBR prazan ID: nacrt sa praznim ID-em stavke je odbijen"
    AssertTrue InStr(1, g, "ZbirnaStavkaID", vbTextCompare) > 0, _
               "ZBR prazan ID: kapija imenuje stavku (bilo: " & g & ")"
    AssertEquals CStr(pre), CStr(Pr3BrojRedova(TBL_ZBIRNA)), _
                 "ZBR prazan ID: zaglavlje nije ostalo"

    ' --- red clanstva ---
    Dim dobar As String
    dobar = CreateZbirnaDraft_TX(Pr3Header(TEST_PREFIX & "-ZBR-PI2-" & scenario), _
                                 ZbrOcek(KLASA_I, 400#, 20#), g)
    If Len(dobar) > 0 Then
        Dim preIzv As Long
        preIzv = Pr3BrojRedova(TBL_ZBIRNA_IZVORI)

        modDataAccess.NewEntityIDPadniTest True
        AssertTrue Not modDokumenta.DodajZbirnaIzvor_TX(dobar, otp, g), _
                   "ZBR prazan ID: clanstvo sa praznim ID-em je odbijeno"
        modDataAccess.NewEntityIDPadniTest False

        AssertTrue InStr(1, g, "ZbirnaIzvorID", vbTextCompare) > 0, _
                   "ZBR prazan ID: kapija imenuje clanstvo (bilo: " & g & ")"
        AssertEquals CStr(preIzv), CStr(Pr3BrojRedova(TBL_ZBIRNA_IZVORI)), _
                     "ZBR prazan ID: red clanstva nije ostao"
    End If
...
EH:
    modDataAccess.NewEntityIDPadniTest False
    SetTestMode prevMode
    LogFatal "Test_ZBR_PrazanIDDetetaNeProlazi", Err.Number, Err.description
End Sub
...
Private Sub Test_BKTX_ZbirnaTudjegVlasnikaOdbijena()
    ...
    ' I kroz pisca.
    AssertEquals "", ZbrKanonNacrt(d, TEST_VOZ_ID, brojStanice, TEST_KUP_ID, _
                                   KLASA_I, 1000#, 100), _
                 "BKTX zbirna: pisac odbija broj tudjeg vlasnika"

    ' Kontrola: vozacev sopstveni broj prolazi kroz istog pisca.
    AssertTrue Len(ZbrKanonNacrt(d, TEST_VOZ_ID, _
                                 modBrojevi.FormatBroj(TEST_VOZ_ID, d, 1), _
                                 TEST_KUP_ID, KLASA_I, 1000#, 100)) > 0, _
...
Private Sub Test_BKTX_UvozZbirneOdbijaTudjBroj()
    ...
    '
    ' Od S5-3 se zbirna sastavlja od OTPREMNICA, pa kontrolni slucaj mora da
    ' ima pravi izvor. Odbijeni slucajevi iznad ga ne trebaju -- kapija broja
    ...
    AssertTrue Len(TestHook_ImportZbirnaRowPWA("CRID-BKTXZOK-" & scenario, TEST_VOZ_ID, _
                         TEST_KUP_ID, d, TEST_VRSTA, TEST_SORTA, 100#, _
                         modBrojevi.FormatBroj(TEST_VOZ_ID, d, 1), bktxCrid)) > 0, _
