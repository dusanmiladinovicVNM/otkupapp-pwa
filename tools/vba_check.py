...
Provere:
 ...
 14. SLOJ_UPIS   -- ekranski modul (`modScr*`, `modOtkupUI`) zove `AppendRow` /
                     `UpdateCell` / `GetNextID`. Upis i dodela identiteta ne
                     pripadaju sloju prikaza.
 15. SLOJ_UZVODNO -- `modDataAccess` pominje `modScr*` / `modApp*` / `modDom*` /
                     `frm*`. Zavisnost ide samo nanize; ovo bi bio ciklus.
 16. REPO_TX     -- `modRepo*` zove `BeginTx` / `AddTableSnapshot`. Transakcioni
                     scope pripada pozivaocu (ADR-0003 tacka B).
 17. REPO_API    -- `modRepo*` izlaze genericki `Public` API (`UpdateColumn`,
                     `ByVal colName`). To je `UpdateCell` sa prefiksom.

Provere 14-17 uvedene su nad izvorom koji ih VEC postuje (nula nalaza), pa nemaju
...
def self_test() -> int:
    ...
    ukupno = (len(SELF_TEST_CASES) + len(SELF_TEST_POZIVI) + len(ZAKLONJENO_CASES)
              ...
              + len(DUPLI_LOKAL_CASES) + len(SLOJ_CASES) + 4)
    for line in palo:
