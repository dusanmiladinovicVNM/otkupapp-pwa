...
async function loadOtkupPregled() {
    document.getElementById('pregledList').innerHTML =
    ...
    const json = await safeAsync(async () => {
        return await apiFetch(
            'action=getOtkupi&otkupacID=' + encodeURIComponent(CONFIG.OTKUPAC_ID)
        );
    ...
    document.getElementById('statPregledKoop').textContent =
        new Set(all.map(r => r.kooperantID)).size;

    const list = document.getElementById('pregledList');

    if (all.length === 0) {
        list.innerHTML =
            '<p style="text-align:center;color:var(--text-muted);padding:40px;">Nema otkupa</p>';
        return;
    }
    ...
    list.innerHTML = all.map(r => {
        ...
        return `
            <div class="queue-item" style="border-left-color:${bc};">
                <div class="qi-header">
                    <span class="qi-koop">${escapeHtml(r.kooperantName)}</span>
                    <span class="qi-time">${escapeHtml(r.datum)}</span>
                </div>
                <div class="qi-detail">
                    ${escapeHtml(r.vrstaVoca)} ${escapeHtml(r.sortaVoca || '')} ${escapeHtml(r.klasa)} |
                    ${r.kolicina} kg × ${r.cena} = <strong>${v} RSD</strong>
