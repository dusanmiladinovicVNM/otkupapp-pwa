...
- Otkup-list number (`txtBrojDokumenta`) follows the otpremnica with **no panel-specific generator**: the panel sets `txtDatum` **before** `cmbOtkupnoMesto`, so `frmOtkup`'s own `cmbOtkupnoMesto_Change` → `RefreshBrojDokumentaSuggestion` (canonical `SuggestNextBroj`) yields `OM/ddmmyy[-N]` from the selected OM + the otpremnica date — instead of today's date. Normal (panel-closed) entry already drives the number from the selected OM + `txtDatum`, so that path is unchanged.
