...
- Otkup-list number (`txtBrojDokumenta`) is generated with `GenerateBrojDokumenta` for the **otpremnica's date** and the **OM taken from the `cmbOtkupnoMesto` field** (so it matches the row that gets saved), instead of always using today's date; the running `OM/ddmmyy` → `-2`/`-3`… sequence is recomputed after each Unos.
