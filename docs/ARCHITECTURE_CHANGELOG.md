...
- **OM issues empty packaging to a kooperant** (`frmDokumenta`, OM-Ulaz frame). A runtime toggle `tglIzdKoop` (added via `Me.Controls.Add` — `.frx` untouched) switches the frame's ambalaža direction: default **Prijem na OM** (`Stanica` `Ulaz`, unchanged) vs **Izdavanje kooperantu** which books `Kooperant` `Ulaz` (`DOK_TIP_OM_IZLAZ_KOOP`, no vozač) so the kooperant balance reflects empties received (later netted by the otkup `Kooperant` `Izlaz`). New constant `DOK_TIP_OM_IZLAZ_KOOP`; `SaveOMUlaz_TX` gained an `izdavanjeKoop` flag; single-entry (OM-side `Izlaz` deferred to the broader double-entry topic). Storno of this type follows the existing OM-Ulaz gap (not in the storno combo).
