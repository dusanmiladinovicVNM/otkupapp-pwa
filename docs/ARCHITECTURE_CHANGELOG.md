...
|---|---|---|---|---|
| v6.29 | 2026-06-20 | Driver (`Vozac`) packaging balance fix: the ledger `Smer` is entity-relative, so the vozač saldo/report double-counted a complete otpremnica→prijemnica route (loaded N at the station + delivered N to the buyer → Saldo −2N, e.g. −200 for 100 gajbica). New `VozacAmbEffectiveSmer` reads the driver as a moving warehouse — otpremnica loads (`Stanica` `Izlaz` → driver `Ulaz`), prijemnica unloads (`Kupac` `Izlaz` → driver `Izlaz`) — inverting `Stanica`-side rows so a full route nets to 0; `Kupac` (destination) rows and all entity reports keep the raw `Smer`. Also excludes `DokumentTip = "Otkup"` (procurement) from the vozač balance — otkup is tagged with a vozač (auto-hladnjača forces the mirror vozač onto every hladnjača otkup) and double-charged the same crates already counted on the otpremnica. Read-only `modAmbalaza`/`modIzvestaj` change. | Yes | read-only; no schema/data migration; re-import `modAmbalaza.bas` + `modIzvestaj.bas` |
...
Fixed the driver (`Vozac`) packaging balance so a complete otpremnica→prijemnica route for the same vozač nets to **0** instead of showing a phantom double `Izlaz`. The ledger `Smer` is stored relative to the **entitet** (Kooperant / Stanica / Kupac); the vozač saldo/report previously summed that raw `Smer`, so a driver who loaded N crates at the station (otpremnica = `Izlaz` rel. Stanica) and delivered the same N to the buyer (prijemnica = `Izlaz` rel. Kupac) was debited 2×N (100 + 100 → Saldo −200). Surfaced via a real otkup-in-own-hladnjača case where the buyer is the firm itself and the "vozač" is the station. A second, related double-count was then found and fixed: the **otkup** leg (procurement, `Kooperant` `Izlaz`) is also tagged with a vozač (`modOtkup:448`; auto-hladnjača forces the mirror vozač onto every hladnjača otkup), charging the same crates again on top of the otpremnica — so the vozač balance now **excludes `DokumentTip = "Otkup"`** (driver custody = otpremnica load − prijemnica deliver). Read-only fix — no ledger writes, schema or data change.

### Fixed

- New canonical helper `modAmbalaza.VozacAmbEffectiveSmer(smer, entitetTip)`: the driver is a moving warehouse — an **otpremnica loads** him and a **prijemnica unloads** him — so it inverts `Stanica`-side rows (`Izlaz` from the station = driver `Ulaz`/load) and keeps `Kupac`-side rows raw (`Izlaz` to the buyer = driver `Izlaz`/unload). A complete route nets to 0; an open otpremnica shows a positive saldo = crates still on the driver. Invalid `Smer` still fails fast (helper returns it unchanged → caller raises).
...
- The rule keys on `EntitetTip`: `Kupac` = destination (raw `Smer`), everything else = source/`Stanica` (inverted). Matches every current booking (otpremnica = `Stanica`, prijemnica / kupci-otpremnica = `Kupac`; otkup/`Kooperant` is excluded anyway). A future flow that books a `Kupac` row as a source — or a non-`Kupac` as a destination — would need the rule revisited.
