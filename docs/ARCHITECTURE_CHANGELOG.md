...
**Status:** Active changelog — v6.42 Banka nalozi za plaćanje (`frmBankaExportPregled`/`modBankaExportPregled`/`modPrint`/`modConfig`/`modPodesavanja`): (1) **„Generiši CSV naloge"** — CSV naloga za prenos za uvoz u e-banking; platilac `SELLER_NAME`/`SELLER_ACCOUNT`, primalac kooperant+TR, **iznos po bloku = `clsBlokIsplata.IsplatitiIznos`** (operater „Isplatiti" unos ili otvoreni iznos), **poziv na broj = broj otkupnog bloka** (jaki auto-map ključ pri uvozu izvoda), šifra/svrha `CFG_BANKA_NALOG_SIFRA`(default 221)/`CFG_BANKA_NALOG_SVRHA` — nova Podešavanja grupa „Banka / nalozi" (tblSEFConfig); fajl u `CSV_DIR_BANKA_NALOZI` (UTF-8, `;`, decimalna tačka); **bez upisa u `tblNovac`** (knjiži se tek uvozom izvoda); `WriteAllTextUtf8` sada Public (reuse iz parser modula). (2) **„PDF specifikacija"** (umesto TSV clipboard) — `PrintIsplataSpecifikacija` → `modPrint.EnsureIsplataSpecSablon`/`FillIsplataSpecSablon` (`WS_ISPLATA_SPEC_SABLON`, house-style A4 portrait, 9 kolona sa „Za isplatu" + UKUPNO), izlaz po `CFG_ISPLATA_SPEC_PRINT_MODE` (default PDF, folder `Specifikacije\`); isti izbor blokova/iznosa kao CSV (deljeni `CollectIsplataBlokovi` u formi). (3) **Runtime kontrole u formi** (`Controls.Add`, `.frx` se ne dira): combo „Kooperant" (dropdown+unos, `fmMatchEntryComplete`, substring filter nad `kooperantNaziv`; `m_FullBlokovi` čuva punu listu → prune override-a i combo imena idu protiv pune liste, filter ne briše „Isplatiti" unose; kooperant filter je čist view-filter `ApplyKooperantFilter` bez rebuild-a tabela), combo „Sa računa" (`CFG_BANKA_NALOG_RACUNI` `;`-lista, prikaz sa imenom banke preko `BankaNazivZaRacun` NBS-prefiks mape; izabrani račun → `GenerisiNalogeCSV`/`PrintIsplataSpecifikacija` `platilacRacun` param, fallback `SELLER_ACCOUNT`), i dva dugmeta za **ručno vezivanje virman avansa** na blokove (`mBtnAvansBlok` fokusiran blok / `mBtnAvansSel` čekirani) → postojeći `ApplyAvansToOtkup_TX` (dotad zakačen SAMO na snimanje novog otkupa u `modOtkup`; za već otvorene blokove nije bilo UI akcije). Perf: `RenderListbox` preko `.List=arr` (1 upis umesto AddItem+po-ćelija), `BuildBlokIsplataList` KooperantID preko `BuildLookupDict` (O(n) umesto O(n^2)). v6.41 Agrohemija (`frmAgrohemija`/`modAgrohemija`/`modConfig`): (1) izdavanje robe **opciono bez parcele** kad je `PRACENJE_PARCELA` OFF (`IsPracenjeParcela()` reuse iz `frmOtkup`; smart-doza se preskače, `lstParcele` disabled; dug se sada prikazuje i za kooperanta bez parcela); (2) **početni dug kooperanta (migracija)** — dugme „Pocetni dug" + `BookPocetniDug()` knjiže `MAG_IZLAZ` red `Vrednost=iznos` na rezervisani virtuelni artikal `ART_POCETNI_DUG` (`SaveMagacin` dobio `Optional allowNoStock`); artikal izuzet iz combo-lista i `GetMagacinStanje`; **known limitation:** PWA `ExportMagacinKoop` ga još ne izuzima (KI-006/ROADMAP). v6.40 release rutina: `tools/release.sh|ps1` spaja pull→bump `APP_VERSION`→commit→tag→push→stamp u jednu komandu (ostaju 3 Excel klika); GAS `installMonitoringTriggers` sad uključuje i `rebuildMonitoringFleet` (auto rebuild Fleet taba na sat); `docs/RELEASE_PROCEDURE.md` dobio „Jedna komanda" + opcioni „Potpis VBA projekta (opciono)". v6.39 auto-verzija: `stamp-build` i `modBuildInfo` dobijaju `BUILD_VERSION` (`git describe --tags --always`; na tagu `vba-v2.2.1`, posle N commita se sam diže `vba-v2.2.1-3-gabc1234`); `modMonitoring`/`modLicense` šalju `buildVersion`; GAS `Events` + `Fleet` dobijaju kolonu `BuildVersion`. Jedini ručni „bump" je `git tag`; `APP_VERSION` ostaje gruba baza. v6.38 verzionisanje/fleet: novi modul `modBuildInfo` (`BUILD_SHA`/`BUILD_DATE`, stamp pri buildu preko `tools/stamp-build.sh|ps1`); `modMonitoring` i `modLicense` šalju `buildSha`/`buildDate`; GAS `Monitoring.gs` — `Events` dobija kolone `BuildSha`/`BuildDate` (`ensureHeader_` rekonsilijacija, stari redovi ostaju poravnati) + novi `Fleet` tab sa `rebuildMonitoringFleet()`/`getMonitoringFleet` za pregled „ko ima koju verziju"; release procedura R1–R3 u `docs/RELEASE_PROCEDURE.md`. v6.37 panel „Otkupni blokovi" (`modOtkupBlok`): „Cena po otpremnici" je sada **default** za nove blokove (pre-fill `txtCena`); **ručni override u `txtCena` se poštuje i nikad se ne pregazi** (uklonjen `ApplyCenaToOtpremnica`, reverz v6.27 „one price applies to all"). v6.36 prijemnica auto-štampa (`CFG_PRIJEMNICA_PRINT_MODE`) gejtovana na **default hladnjaču** (`frmDokumenta.btnUnosPrij`: štampa samo kad je `kupac == MALINA_DEFAULT_KUPAC`); eksterni kupci se ne štampaju automatski. v6.35 panel „Otkupni blokovi" (`modOtkupBlok`): količina (kg) se u panelu i obe liste (otpremnice + blokovi) prikazuje sa **fiksne 2 decimale** (`FmtKgDec` → `#,##0.00`, ista konvencija kao `frmOtkup.UpdateUkupnoKg`). v6.34 otkupni list: „Saldo ambalaze" je sada kumulativni entitetski saldo kooperanta (Početno stanje pre bloka + Izdato − Primljeno; novi `modAmbalaza.GetKooperantAmbOpening`). v6.33 dnevna/periodična specifikacija (datum od–do) + kolona „Otkupno mesto" u panelu „Otkupni blokovi" (`modOtkupBlok`). v6.32 otkupni list: ambalaža kao uokvirena tabelica (Primljena/Izdata/Saldo). v6.31 daje punu podršku za **Klasu II kroz ceo lanac** (zasebne gajbe #3 — ledger, otpremnica/zbirna/prijemnica, paletizacija), dozvoljava **unos samo Klase II bez Klase I** (#2), uvodi **bruto unos sa obaveznim gajbama** (#1) i **otpremnicu koja čuva neto + `BrutoKg`** (#5, panel poredi neto↔neto); MALINA posivljava Zbirna sekciju u `frmDokumenta`; auto-cena Klase II; PR57 numeracija prijemnice (`GenerateBrojPrijemnice`); fix regresije izdate ambalaže kod unosa samo Klase II
...
|---|---|---|---|---|
| v6.49 | 2026-08-14 | **Test platforma: kapije, identitet run-a, jedinstven lifecycle** (`tests/suite_manifest.json`, `tools/vba_gate.py`, `tools/release_gate.py`, `tools/run_vba.py`, `tools/release.ps1|sh`, `.claude/hooks/vba-test.sh`, novi `modTestRunner`/`modTestAssert`/`clsTestContext`/`clsTestResult`). Povod: `vba-v2.40.0` je tagovan uz posteno zapisanu napomenu da behavior runner nije ni pokrenut -- uzrok nije nepaznja nego **redosled** (release.ps1 je bump/commit/tag/push radio PRE nego sto operateru kaze da uradi Import i Compile). **(1) Release Gate.** Redosled je sada: pull -> bump `APP_VERSION` u radno stablo (bez commita, da kapija vrti BAS ono sto ce biti tagovano) -> `release_gate.py` (static / fixture / behavior / green / compile / external) -> tek onda commit, **anotiran** tag sa verdiktom kapije u poruci, push. Pala kapija = nema commita/taga/push-a i bump se vraca. `--waive <kapija> --reason "..."` je jedini izlaz i zapisuje se u tag i u `tests/last_release.json`; waiver bez razloga se odbija, `NOT_RUN` blokira isto kao `FAIL`. **(2) Identitet run-a.** Kanonski (CRLF-neutralan, `modBuildInfo` izuzet) SHA256 nad `src-vba` + last-green marker u `.git/agrix-vba-last-green` koji pamti i KOJU kapiju je prosao (`dev` marker ne zadovoljava release). Stop hook presao sa `git diff HEAD~1` na taj hash -- commit nad `docs/`, rebase i amend vise ne mogu da ponište obavezu testa. `last_run.json` nosi git sha, hash izvora, hash fixture-a, platformu, locale, seed. **(3) Verdikt razdvojen** na `STATIC/COMPILE/SCHEMA/BEHAVIOR/COUNTS/CLEANUP/EXTERNAL` umesto jednog `ZELENO` u kome je `COMPILE NEJASNO` nestajalo. **(4) Manifest** kao jedini izvor istine o suite-ovima (katalog izvaden iz `run_vba.py`); `--manifest-check` proverava oba smera i pri prvom pokretanju nasao **10 ulaznih tacaka koje niko ne poziva ni ne pokrece** (SEF sub-suite-ovi + `RunHttpUtilsSmokeSuite`) -- upisane u `unlisted` sa razlogom po komadu. `min_asserts` + `TR_Report` hvataju tih pad pokrivenosti (189 -> 120 provera je crveno). **(5) Lifecycle.** `modTestRunner` (Begin/End Suite, eksplicitan `TR_NotRun`), `modTestAssert` (jedan assertion API sa `AssertRaised`), `clsTestContext` (snapshot/restore Excel state-a kroz `Class_Terminate` -- VBA nema `finally`). `modLicenseTests` migriran kao kanarinac; ostalih 10 prikljuceno kroz jedan `TR_Report` poziv. **(6) Zatvorene rupe:** dva tiha `Exit Sub` u `RunBankaImportTestSuite`, `RunNovacSmokeSuite` iz blind u gate, `make_fixture.strip_vba` fail-closed (dokazuje da nije ostao nijedan donor modul tipa 1/2/3), ORPHAN tvrd pad nad sopstvenim fixture-om. **(7) Novo:** `--shuffle --seed`, `--repeat`, per-suite `timeout_s`, JUnit XML, leak detektor (prijava, ne kapija -- nije dokazan u oba smera). | da (`docs/TEST_PLATFORM.md`, `docs/RELEASE_PROCEDURE.md`, `.claude/rules/testovi.md`, `CLAUDE.md`) | **Sve VBA izmene su samo staticki proverene** (`vba_check`, dokazano u oba smera); nijedna nije izvrsena nad Excelom -- prvi Windows `run_vba.py --gate pr` je dokaz. Otvoren dug je popisan u `docs/TEST_PLATFORM.md` sec.11. |
| v6.48 | 2026-08-11 | **RF-10 / AUD-026 — banka export naloga (fail-closed saldo)** (`modBankaExportPregled`, `frmBankaExportPregled`, `modTestBanka`). **Treći i poslednji paket M6.** Bez izmene šeme, `.frx` netaknut, bez novih form `WithEvents`, bez novih `Poruka()` ključeva. **(1) Stale override clamp (FM-0020 #1).** Per-blok „Isplatiti" override živi u formi (`m_OverrideAmounts`, `OtkupID → iznos`) i **preživljava reload** liste; `PruneStaleOverrides` je brisao samo unos za blok koji je iz liste nestao, a iznos nije poredio ni sa čim. Posle delimične isplate, vezanog avansa ili storna zaostao override je zato mogao biti **veći od trenutno otvorenog** — i takav je ulazio u prikaz i u CSV nalog. Logika je izmeštena u nov `ClampOverridesToOpen(overrideDict, blokovi)` (testabilan seam van forme, uz `BuildOpenAmountDict` kao zajednički indeks `OtkupID → OtvorenIznos`): blok kog više nema ili je bez otvorenog iznosa **gubi** override, override veći od otvorenog se **spušta** na otvoreno, a **manji ostaje netaknut** (namerna delimična isplata je operaterska odluka, ne greška). `PruneStaleOverrides` je sada `Function` koja vraća broj usklađenih unosa, pa `LoadBlokovi` to javlja u statusnoj liniji — tiho spuštanje iznosa bi operater lako promašio. **(2) Finalna saldo revalidacija (FM-0020 #2 / FM-0021 #1).** `GenerisiNalogeCSV` je pred upis proveravao **samo** `HasTekuciRacun` i `IsplatitiIznos > 0`, dakle ništa što se tiče salda — svaka promena podataka između prikaza i klika na „Generiši" (uvoz izvoda, vezan avans, storno u drugom delu aplikacije) mogla je da završi kao **preplata u nalogu za banku**. Sada se pred upis čita **svež** saldo (`BuildBlokIsplataList()` → `BuildOpenAmountDict`) i svaki nalog poredi sa trenutno otvorenim iznosom kroz nov `ValidateNalogSaldo(blokovi, openByOtkup)`; ijedan nalog preko otvorenog → **fajl se ne piše**, a razlog (koji blok, koliko traži vs otvoreno) se vraća kroz nov `Optional ByRef outOdbijeno` i forma ga prikazuje pa odmah osveži pregled (klamp tada spusti zaostale override-e). **Fail-closed** na tri načina: blok kog više nema među otvorenima se računa kao otvoreno = 0; greška pri očitavanju završi u `EH`, dakle takođe bez fajla; a provera gleda **tačno** one blokove koje writer i upisuje (sa TR, iznos > 0), da blok koji uopšte ne ide u fajl ne obori generisanje. Poređenje je u **cent-domenu** preko postojećeg `modNovac.ZaokruziNovac` (half-up), **bez epsilon tolerancije** — raniji prag `> otvoreno + 0.01` je propuštao preplatu od punog centa (`600.01` nije veće od `600.00 + 0.01`), a baš taj cent banka isplati; isto pravilo važi za klamp, finalnu kapiju, operaterski unos (`txtIsplatiti_Exit`) i `CsvIznos` (normalizuje PRE `Format$`), pa se ono što kapija odobri i ono što writer upiše ne mogu raziići. NE uvoditi novu granicu ni nov helper. **(3) Brojanje stvarno primenjenih avansa (FM-0020 #2, oslonac na RF-02/AUD-010).** `ApplyAvansToOtkup_TX` vraća `True` i za **no-op** (transakcija je uspela, ali nije bilo slobodnog avansa niti otvorenog iznosa), pa je batch „Primeni avans (sel.)" brojao svaki `True` kao primenjen avans i prijavljivao naduvan rezultat. `PrimeniAvansTX` sada čita **stvarno proknjižen iznos** iz `ByRef` parametra koji je RF-02 već uveo: batch razdvaja **primenjeno / bez promene / greška** i prikazuje zbir, po-blok akcija ne prijavljuje uspeh kad nije knjižen nijedan dinar, a override se skida **samo** kad se otvoreno bloka zaista promenilo. **Testovi:** `modTestBanka` proširen sa RF-09 na celu banku — nove grupe `T12` (klamp: 1000 na otvoreno 600 → 600; nestao i zatvoren blok → brisanje; manji override netaknut; drugi prolaz no-op; bez liste blokova sve pada) i `T13` (nalog preko otvorenog odbijen **sa imenom bloka** u poruci; nestao blok fail-closed; blok bez TR ne obara naloge; razlika ispod tolerancije prolazi; bez mape otvorenih se odbija). Oba su čisti seam testovi bez tabela, van transakcije kao `T01`; suite ostaje tvrd gate. **Review runda 1 (pre merge-a):** (R1) poredjenje iznosa je bilo na epsilon pragu `> otvoreno + 0.01` (preuzet iz zatecene provere unosa) — taj prag propusta preplatu od **punog centa** (`600.01` nije vece od `600.00 + 0.01`), a bas taj cent banka isplati; sva tri mesta (unos u formi, klamp, finalna kapija) sada normalizuju oba iznosa postojecim `ZaokruziNovac` (half-up) i porede **tacno**, `CsvIznos` normalizuje **pre** `Format$` (inace bi writer i validator mogli da se raziidju), a provera „iznos > 0" ide nad zaokruzenom vrednoscu da sub-cent unos ne prodje validaciju pa tiho ispadne iz fajla. Uvedene granicne provere `600.00/600.01/600.005/600.004`. (R2) test je gadjao samo validator, pa bi uklanjanje njegovog poziva proslo nezapazeno — sadrzaj CSV-a se sada gradi iskljucivo kroz nov `BuildNalogCsvPayload` koji **u sebi nosi** kapiju (jedina putanja do `WriteAllTextUtf8`), a nov T14 tvrdi da je payload pri preplati prazan. (R3) `Err.description` se u svim `EH` granama ovog ekrana hvata **pre** `LogErr` (BUG-1/AUD-054) — ranije je operater na pad generisanja naloga mogao dobiti praznu poruku umesto razloga. (R4) sub-cent ostatak je mogao da napravi nalog na **nula dinara**: `GetOpenOtkupi` racuna otvoreno sirovo (`kolicina * cena - isplaceno`, bez `ZaokruziNovac`), pa je legitiman ostatak tipa `0.004` RSD prolazio kroz sva tri praga `> 0` (forma `CollectIsplataBlokovi`, validator, payload writer), a `CsvIznos` bi ga upisao kao `;0.00;` — nije preplata, ali je nevalidan nalog koji banci moze da obori uvoz CELOG paketa. Iznos se sada normalizuje **pre** praga na sva tri mesta (forma pamti zaokruzen `IsplatitiIznos`, validator i writer koriste isti `trazeno = ZaokruziNovac(...)`), pa takav blok jednostavno ne dobija nalog. T14 dopunjen slucajem `regularan nalog + blok 0.004` → tacno jedan data red i payload bez `;0.00;`. (R5) saldo-mapa nije bila fail-closed na **korumpiranom kanonskom ključu**: `BuildOpenAmountDict` je duple `OtkupID`-eve pisala assignment-om (svesna odluka iz prve verzije „da korumpiran podatak ne obori pregled"), a `GetOpenOtkupi` ne agregira po `OtkupID` nego vraća red po red — pa bi kod dva reda sa istim `OtkupID`, **različitim** `BrojDokumenta` i različitim otvorenim iznosom poslednji upis odlučio saldo, i otvoreno bloka B bi odobrilo nalog za blok A (`traženo 1.000 <= 2.000` prolazi iako je na A otvoreno 500). Na finansijskoj kapiji je to fail-**open**, pa je vraćeno na tvrd pad: dupli i prazan `OtkupID` sada dižu tipizirane greške (`ERR_ISPLATA_DUPLI_OTKUPID` / `ERR_ISPLATA_PRAZAN_OTKUPID`). Posledica je namerna na obe strane — pregled prijavi razlog u statusu umesto da „nekako" izabere koji duplikat važi, a `GenerisiNalogeCSV` padne u svom `EH` **pre** nego što payload uopšte nastane (mapa se gradi kao argument), pa nema fajla za banku; razlog se sada vraća kroz `outOdbijeno` umesto generičnog „pogledajte log". Duplikat inače hvata i `RunProductionHealthCheck`, ali health-check nije preduslov svakog klika na „Generiši naloge". Nov T15 gađa konfliktne vrednosti (500 vs 2.000 pod istim `OtkupID`), propagaciju kroz klamp i prazan ključ; zdrava lista mora i dalje da prođe. (R6) **cross-flow rupa iste invarijante:** R5 je zatvorio CSV, ali ne i drugu mutacionu akciju istog ekrana. `ClampOverridesToOpen` je izlazio pre izgradnje mape kad override-a nema (`overrideDict.count = 0`) — a to je stanje pri **prvom** otvaranju forme — pa strogа provera kanonskog ključa nije ni radila i korumpiran `OtkupID` je mogao normalno da stoji na ekranu; mapa se sada gradi **pre svih ranih izlaza**. Važnije: `modNovac.ApplyAvansToOtkup` je target red čitao kao `FindRows(...)(1)`, dakle **prvi red pobeđuje**, bez `count > 1` provere — kod dupliranog `OtkupID`-a vrednost i vlasnik se čitaju sa jednog reda, a avans se u ledgeru vezuje samo za dvosmislen `OtkupID`. Guard je zato dodat u **core** (`vbObjectError + 1045`, pre svakog čitanja target reda i pre ijednog upisa), ne u formu — UI nikad ne sme biti jedina bezbednosna granica, a `RunProductionHealthCheck` nije preduslov klika na „Primeni avans". Nov **T16** je tabelarni (ne dictionary) i gađa slučaj koji je ranije **tiho** prolazio — **isti** kooperant na oba duplirana reda (različit kooperant je već obarao zatečeni owner guard AUD-010, pa se i on proverava da ne oslabi): akcija mora biti odbijena, `appliedAmount = 0`, nijedan `tblNovac` red ne sme dobiti sporan `OtkupID`, avans ostaje nevezan, oba `tblOtkup` reda netaknuta — uz kontrolu da jedinstven `OtkupID` i dalje prolazi. Provereno je i da duplikat ne nastaje ni u jednom legitimnom toku: `tblOtkup` se puni samo na dva mesta (`modOtkup`, `modMasterSync`), oba preko `GetNextID`, a storno postojeće redove samo označava. (R7) **najopasniji sloj iste invarijante — pogrešan primalac novca:** stroga saldo-mapa iz R5 vidi samo **otvorenu** kolekciju, a identitet kooperanta je dolazio iz `BuildLookupDict(TBL_OTKUP, ...)` — „prvi pojav pobeđuje" nad **sirovom** tabelom, dok `GetOpenOtkupi` preskače stornirane i već isplaćene redove. Kod dupliranog `OtkupID`-a te dve semantike se razilaze: red A (kooperant K-A) isplaćen i zato nevidljiv u otvorenoj listi, red B (kooperant K-B) otvoren — u otvorenoj kolekciji postoji **jedan** `OtkupID`, pa duplikat-kapija nema šta da prijavi, ali vlasnik dolazi sa reda A i blok B dobija naziv i **tekući račun kooperanta A**. Nalog bi otišao **pogrešnom primaocu**, što je teže od pogrešnog prikaza. Zato se identitet primaoca više ne rešava „prvim pogotkom": nov `BuildOtkupOwnerIndex` u jednom prolazu kroz sirovu tabelu pamti vlasnika **samo za jednoznačne** `OtkupID`-eve i zasebno evidentira duplikate, a `BuildBlokIsplataList` pada (`ERR_ISPLATA_DUPLI_OTKUPID`) čim je duplirani ID **otvoren** — provera ide pre view-filtera, da se ekran ponaša isto kao finalna kapija koja listu gradi bez filtera. Istorijski duplikat kome nijedan red nije otvoren namerno **ne** obara ekran. Nov T17 daje kooperantima očigledno različite račune i dokazuje da nikakav payload ne nastane, uz kontrolu da zatvoren duplikat prolazi; T17 se u suite-u pokreće **pre** T16, jer T16 namerno ostavlja dva otvorena reda sa istim `OtkupID`. Uz to: neuspeo rebuild više ne ostavlja **prethodnu** listu na ekranu (`LoadBlokovi` EH čisti prikaz) — writeri jesu fail-closed, ali prikaz ne sme da tvrdi više od njih. (R8) **poslednji tihi gubitak obaveze:** RF-10 je uveo `ERR_ISPLATA_PRAZAN_OTKUPID`, ali taj guard nikad nije mogao da opali — `BuildOtkupOwnerIndex` prazan `OtkupID` preskače (ispravno, to je samo indeks), pa vlasnik nije nađen i red je završavao u zatečenom tihom `GoTo NextRow`: **otvorena obaveza je nestajala sa ekrana bez ijedne poruke**, dok bi ostali nalozi uredno otišli u fajl. Isto je važilo za otvoren blok sa praznim `KooperantID` (FM-0021 #5). To je suprotno pravilu koje ovaj paket uvodi, pa je identitet sada fail-closed po redosledu: `OtkupID` postoji → jedinstven je → kooperant postoji → tek onda TR/naziv/filteri. Prazan kooperant nije legitimno stanje — `SaveOtkupMulti_TX` ga odbija pri upisu (greška 1810), a `OtkupID` dolazi iz `GetNextID`; dakle nijedan zdrav tok ne može da napravi takav red. **FM-0021 #5 je time zatvoren strože nego što je katalog predlagao** (predlog je bio „prebroj preskočene i prikaži”, ovde je tvrd pad). Nova `ERR_ISPLATA_PRAZAN_KOOPERANTID`. Novi T18/T19 (tabelarni): otvoren blok bez `OtkupID` odnosno bez kooperanta mora da padne tipizirano i da imenuje dokument, uz kontrolu da **zatvoren** red bez `OtkupID` ne obara pregled (granica iz R7: korupcija koja ne može proizvesti nalog ne blokira ekran). Uz to su T16–T19 prebačeni u **izolovane transakcije** (`BeginIsolatedTx`) — svi čitaju CELU tabelu, pa je seed jednog obarao kontrolne slučajeve drugog; izolacija je uklonila i zavisnost od redosleda pokretanja. (R9) **dva nalaza:** (a) *test-bug, acceptance blocker* — seed redovi u T18/T19 su imali **7 kolona i 6 vrednosti**, pa bi `BitAppend` pukao na `Subscript out of range` i prekinuo ceo suite pre nego što T18 uopšte proveri produkcioni guard; dodat je nedostajući `Date`, a uz to je uveden mehanički audit arnosti svih `BitAppend` poziva (13 poziva, 0 neslaganja). (b) *produkcioni* — pogrešan primalac je bio zatvoren na `OtkupID`, ali ne i **jedan FK korak dalje**: `BuildKooperantTekuciRacunCache` je bio „prvi pojav pobeđuje”, pa dva reda u `tblKooperanti` sa istim `KooperantID` i **različitim** `TekuciRacun` znače da nalog ide na račun onog reda koji je slučajno prvi — a `OtkupID` pritom može biti savršeno jedinstven, dakle sve kapije iz R5–R8 prolaze. `RunProductionHealthCheck` to ne amortizuje: njegova duplicate-key provera pokriva `OtkupID`/`OtpremnicaID`/`FakturaID`/`NovacID`/`ParcelaID`…, ali **ne** `KooperantID`. Cache sada vraća i skup duplikata, a `BuildBlokIsplataList` diže novu `ERR_ISPLATA_DUPLI_KOOPERANTID` čim duplirani kooperant stoji iza **otvorenog** bloka; neiskorišćen duplikat ne obara ekran (ista granica kao R7/R8). Kad je jedinstvenost dokazana, „prvi pogodak” u `GetKooperantNaziv` postaje **jedini** pogodak, pa i naziv primaoca postaje pouzdan bez dodatnog indeksa. Kooperant BEZ master reda ostaje zatečena putanja (nema TR → blok se vidi, ne izvozi se, broji se u „Bez TR”) — to je već fail-safe i nije dvosmislen identitet. Usput su **kooperant-kapije pomerene ISPRED view-filtera**, gde su već stajale `OtkupID` provere: inače bi filtriran pregled „radio”, a `GenerisiNalogeCSV` (koji listu gradi bez filtera) uvek padao. Nov T20: kontrola da neiskorišćen duplikat prolazi, pa tvrd pad kad duplikat stoji iza otvorenog bloka, pa dokaz da cela CSV putanja vraća prazan rezultat i ne pravi fajl. **Prvi operaterski run (PASS=186, FAIL=1) je oborio jedan TEST vektor, ne produkciju:** `600.005` se u `Double`-u čuva kao `600.00499999999999545…`, dakle **ispod** pola pare, pa ga half-up `ZaokruziNovac` spušta na `600.00` — a da li će se prevrnuti zavisi i od preciznosti međurezultata (VBA drži 80-bit u x87 registru, dok simulacija u 64-bit koraku prevrne). Vektor je zato uklonjen kao inherentno dvosmislen i zamenjen sa `600.006` (jednoznačan na svakoj putanji). Invarijanta nije bila prekršena: `600.005` prolazi kapiju, ali u fajl ide `600.00`, što nije preplata nad otvorenih `600.00`. Umesto tvrdnje o SMERU zaokruživanja, T14 sada tvrdi ono što je zaista bitno — za graničnu vrednost iznos u fajlu **nikad** ne prelazi otvoreno. **Van obima (svesno):** PDF specifikacija isplata ne dobija istu kapiju (prikazni dokument, ne nalog banci), export ledger/rezervacija bloka (FM-0021 #3) i agregacija klasa po `BrDok` (FM-0021 #2) ostaju P2 u katalogu. | ne (bez arhitektonske promene) | AUD-026 zatvoren; **M6 kompletan** (RF-22 → RF-09 → RF-10) |
| v6.47 | 2026-08-09 | **RF-09 / AUD-007 + AUD-014 + AUD-025 — banka import i mapiranje** (`modParse`, `modBankaImport`, `modBankaMapiranje`, `frmBankaImport`, nov `modTestBanka`). Drugi paket M6. Bez izmene šeme, `.frx` netaknut, bez novih form `WithEvents`, bez novih `Poruka()` ključeva. **(1) P0 datum (AUD-007).** `TryParseDateValue` je `DateSerial(Y, m, d)` koristio bez provere, a `DateSerial` nemoguć datum ne odbija nego ga **prelije** (`30.02.2026` → `02.03.2026`, dan 32 → sledeći mesec, mesec 13 → sledeća godina) — parse je vraćao `True` i pomeren datum. Sada idu opsezi (`d` 1–31, `m` 1–12, `Y` 1900–2199) pa **round-trip** (`Day/Month/Year` rezultata == uneto), inače `False` bez diranja `result`-a (isti obrazac stroge provere kao `modMasterSync.IsParsableMasterSyncDate`). Uz to je uvoz izvoda dobio **Level 0** proveru u `ParseBankaIzvodForImport` (pre postojeća 4 nivoa integriteta): datum izvoda i datum **svake** transakcije moraju biti stvarni datumi, inače uvoz tog PDF-a pada (`vbObjectError + 1009/1010`), a `ClassifyBankaImportError` je dobio `DATUM TRANSAKCIJE` → `parse error` (fajl ide u Error folder). Ekspozicija je bila drugačija nego što je audit tvrdio: parseri banaka datum drže kao string i ne zovu `TryParseDateValue` — pozivaoci su bila polja datuma u formama; bank-import putanja je **tek sada** vezana za istu proveru. **(2) Dedupe + broj računa (AUD-025 / FM-0022 #1).** `IsDuplicateBankaImport` je dobio **obavezan** `brojRacuna` (novi `RequireColumnIndex` na `COL_BIM_BROJ_RACUNA`, čitanje po imenu) i poredi ga pre broja dokumenta u **obe** grane (`BankaReferenz` i fallback datum/iznos/partner). Broj izvoda je jedinstven **po računu** firme, pa je transakcija sa drugog računa ranije tiho ispadala iz staging-a. Poziv iz `SaveBankaImportRows` prosleđuje staging kolonu 3; dva test poziva u `modTestStorno` (T24) ažurirana. **(3) 3+ kandidata bloka (AUD-025 / FM-0023 #14).** `GetOtkupCandidatesForKooperantBlock` je alocirao `ReDim result(1 To 2)` i na trećem pogotku radio `Exit For` sa `count = 3` — kopiranje u `finalResult` je onda padalo na „Subscript out of range", a taj pad je iz `AutoMapAllBankaImport_TX` rollback-ovao **ceo** batch (uključujući već mapirane redove). Sada je bafer veličine ulaza, granica je imenovana (`MAX_BLOK_KANDIDATA = 2`), a prekoračenje diže **tipiziranu** grešku `ERR_BMAP_MANUAL_REQUIRED` (`vbObjectError + 2950`, Public zbog testova). Nov `AutoMapBankaImportRowBatch` omotava per-red poziv u obe batch petlje i guta **isključivo** taj kod (greška nastaje pre ijednog upisa za red, pa nema parcijalnog stanja), upiše `Error` status kad je `markErrorOnFail`, javi `BANKA_AUTOMAP_MANUAL_REQUIRED`/WARN i nastavi; svaka druga greška se re-raise-uje i batch pada kao i pre. `AutoMapAllBankaImport_TX` je dobio `Optional ByRef manualRequiredCount`. **(4) Smer guard (AUD-025 / FM-0023 #8).** Nov `RequireBimSmer(bim, expectedSmer, src)`: odbija red bez čistog smera (oba iznosa ili nijedan) i neslaganje tipa i smera — `MapBankaImportAsKupac` traži uplatu, `MapBankaImportAsKooperant`/`…BlockCore` isplatu, `MapBankaImportAsOM` samo čist smer (OM promet ide u oba pravca). Auto putanja već grana po smeru, pa joj se ponašanje ne menja; ručna (`cmbMapTip` je slobodan izbor) je do sada mogla da knjiži isplatu kao uplatu kupca. Tihi `If isplataUkupno <= 0 Then Exit Function` u `…BlockCore` (iz UI-a je izgledao kao „ništa se nije desilo") zamenjen je ovom kapijom. **(5) `_Activate` više ne knjiži (AUD-014 / FM-0024 #1).** Odluka o jakim ključevima izdvojena je u read-only `ResolveStrongKupac`/`ResolveStrongKooperant` (jedan izvor istine za auto putanju i za prikaz), pa nov `CountStrongKeyReadyBankaImport` može da **prebroji** šta bi se mapiralo bez ijednog upisa. `frmBankaImport.UserForm_Activate` sada zove samo brojanje; knjiženje je iza **runtime dugmeta** „Mapiraj jake ključeve (N)" (`Controls.Add` + `clsUiSink` sink, `.frx` netaknut, `ReleaseUiSinks` na `QueryClose`/povratku) uz `vbYesNo` potvrdu i prikaz rezultata. Maska `On Error Resume Next` oko auto-map poziva je uklonjena, a `AutoMapStrongKeysBankaImport_TX` posle rollback-a **re-raise-uje** umesto da vrati 0. **(6) Ručni Kupac ≠ uvek avans (FM-0024 #2/#26).** `cmbFaktura` je bila mrtva kontrola (samo `Clear`), pa je ručno mapiranje uvek slalo prazan `FakturaID` → `NOV_KUPCI_AVANS` i onda kad je faktura postojala. Sada se puni nestorniranim fakturama tog kupca sa otvorenim saldom (`GetUplataForFaktura`), izbor ide u `MapBankaImportAsKupac_TX`, prazan izbor traži potvrdu, a veza faktura↔kupac se proverava i na upisu (`RequireFakturaZaKupca` — FM-0023 #2, bilo latentno dok UI nije mogao da pošalje `FakturaID`). **(7) Preview = komanda (FM-0024 #3/#11).** Nov `EffectiveBlockNo` (ručno izabran blok u `cmbOtkupBlok` ima prednost nad pozivom na broj iz izvoda) koristi se u sve tri grane `BuildOutgoingPreview`, a nova sekcija „RUČNO" u preview-u prikazuje šta bi uradilo dugme „Ručno mapiraj red" iz **istih** kontrola, uključujući odbijanje po smeru i po broju kandidata. Tekst kandidata bloka izdvojen u `CandidatesPreviewText` + `SafeBlockCandidates` (bio prepisan četiri puta; resolver sada može da digne grešku, pa je preview mora prikazati a ne pasti na njoj). Rezultati ručnih akcija se više ne ignorišu (`ReportManualResult`). **(8) Ručni izlaz za blok preko granice.** `MAX_BLOK_KANDIDATA` važi za **automatsku** raspodelu; `GetOtkupCandidatesForKooperantBlock` je dobio `allowOverMax`, a `MapBankaImportAsKooperantBlockManual(_TX)` `allowManyCandidates` koji forma prosleđuje tek pošto operater potvrdi prikazanu podelu. Bez toga bi red označen „za ručno" bio **trajno nezavršiv**: ručni wrapper zove isti core, core isti resolver, resolver istu grešku. Raspodela je izdvojena u `PlanBlokRaspodela` (greedy, veći otvoreni prvi) koji koriste i writer i preview (`SplitPreviewText`), pa prikazana i proknjižena podela ne mogu da se raziđu; sortiranje kandidata je sada `SortKandidatiPoOtvorenomDesc` za N redova (bio swap koji radi samo za tačno 2). **(9) Jedan ugovor o smeru.** Nov `ClassifyBimSmer` (`UPLATA`/`ISPLATA`/`NEJASAN`) koriste i `RequireBimSmer` i preview u formi — ranije je preview proveravao samo „ima li uplate/isplate", pa je red sa **oba** iznosa u prikazu izgledao validno a writer bi ga odbio; OM grana preview-a smer uopšte nije proveravala. **(10) Fail-open mesta zatvorena.** `CountStrongKeyReadyBankaImport` više ne vraća `0` na grešku nego je propagira (forma prikazuje „jaki ključevi: GREŠKA" i gasi dugme) — tiho `0` je schema/read problem prikazivalo kao „nema šta da se mapira", što je suprotno cilju AUD-014; u batch omotaču je upis poslovnog statusa (`Error`) izvučen ispod `On Error Resume Next` (monitoring ostaje best-effort), pa neuspeo upis obara batch umesto da izveštaj kaže „za ručno" a red ostane otvoren. **(11) Locale-nezavisan datum + typed staging.** `TryParseDateValue` je prvo zvao `IsDate`/`CDate` (locale-zavisno: na `MM/DD` mašini `01.02.2026` → 2. januar), a deterministički `d.m.yyyy` je bio tek fallback — iako upravo taj format daju parseri izvoda. Redosled je obrnut: nov `TryParseBankaDateDMY` (bez `CDate`, sa round-trip i opsegom) ide **prvi**, `IsDate` ostaje fallback za ostale zapise. Uz to je validirani `Date` ranije bio **odbačen**: `ParseBankaIzvodForImport` je u rezultat vraćao sirovi tekst, `SaveBankaImportRows` ga upisivao kroz `CStr`, a mapiranje ponovo tumačilo kroz `CDate`. Sada rezultat nosi typed `Date`, staging ga upisuje bez `CStr` (`ToBimDatum`), pa se datum jednom pročita i više ne tumači (FM-0022 #17 / FM-0023 #23 na ovoj putanji). Dedupe poredi datume semantički (`SameBimDatum`/`TryBimDatum`: typed → DMY → `IsDate`, poređenje po danu, string fallback), da zatečeni string-staging ne bi promašio duplikat pri ponovnom uvozu. **(11b) Fallback zatvoren za `d.m.g` oblik.** Prvo stvarno pokretanje `RunBankaImportTestSuite` (`PASS=98 FAIL=1`) oborilo je `T01 mesec 13 odbijen`: `TryParseBankaDateDMY` je ispravno odbio `01.13.2026`, ali je zatim `IsDate`/`CDate` fallback tu vrednost prihvatio tako što **zameni dan i mesec** (13. januar) — isto locale preuređivanje zbog kog deterministički parser i postoji. Nov `LooksLikeDmyTriple` čini DMY parser **konačnim** za `d.m.g`/`d/m/g` oblik: ako ga on odbije, `CDate` se ne poziva. Fallback ostaje samo za zapise drugog oblika (`gggg-mm-dd`, već-`Date`); posledica je i da dotted-ISO (`2026.02.01`) više ne prolazi. **(12) Stabilan identitet kupca/OM-a (FM-0024 #7).** `LoadManualTargets` je punio `cmbPartner` sa `GetLookupList(..., "Naziv")` (koji **spaja** istoimene partnere u jednu stavku), a knjiženje je radilo `LookupValue` po nazivu → **prvi** pogodak: dva kupca istog naziva operater nije mogao da razlikuje, a uplata/avans su mogli da odu na pogrešnog. Sada Kupac i OM idu kroz postojeći `FillComboDisplayID`/`GetComboID` (bound ID kolona, isti obrazac kao `frmFakturisanje`/`frmDokumenta`), a sve grane forme čitaju izbor kroz jedan `SelectedPartnerID()`. **(13) Raspodela uplate kupca (preplata).** `MapBankaImportAsKupac` je ceo iznos izvoda knjižio na izabranu fakturu kao `NOV_KUPCI_UPLATA` — faktura je mogla ostati **preplaćena**, a stvaran avans neevidentiran (owner/storno guard to ne hvata). Sada: `otvoreno = iznos − aktivne uplate` (`GetOtvorenoNaFakturi`, isti helper puni i listu faktura u formi), `naFakturu = Min(uplata, otvoreno)`, ostatak `NOV_KUPCI_AVANS` — oba reda u istoj TX, `Obradjeno="Da"` tek kad je ceo iznos proknjižen (neuspeo avansni red diže grešku, fail-closed); već plaćena faktura se odbija (saldo se mogao promeniti između prikaza i klika). Preview prikazuje istu podelu. **(14) Bezbedan izlaz za dvosmislen blok.** Potvrda za 3+ kandidata je `vbYesNoCancel`: „NE" knjiži ceo iznos kao `NOV_VIRMAN_AVANS_KOOP` (ništa se ne vezuje za otkup dok je poreklo nejasno; avans se kasnije precizno vezuje postojećim `ApplyAvansToOtkup_TX`), pa greedy podela nije jedina ponuđena opcija. **(15) Javni API modula = samo `_TX` (FM-0023 #6).** Ne-TX mutatori (`MapBankaImportAsKupac/Kooperant/OM/KooperantBlock(Manual)`, `AutoMapBankaImportRow`, `SkipBankaImportRow`) prebačeni su u `Private`: od RF-09 jedan poziv može da napravi **više** redova u `tblNovac` (uplata na fakturu + avans višak; raspodela po bloku), pa bez transakcije pad drugog upisa ostavlja prvi trajno upisan, stavku izvoda otvorenu, a ponovni pokušaj duplira prvi deo — transakcija je time deo ugovora, ne stvar discipline pozivaoca. Jedini spoljni pozivaoci bili su testovi; oni sada idu kroz `_TX`, a `gBankaSilentBatch` je proširen i na greške u `_TX` omotačima (batch/test režim ne sme da blokira dijalogom). **(16) Staging writer je fail-closed i atomičan.** Javni ulaz je `SaveBankaImportRows_TX` (snapshot `tblBankaImport`), a `SaveBankaImportRowsCore` je `Private`: core dodaje red po red, pa bi bez transakcije u batch-u „prvi validan, drugi nevalidan" prvi red ostao upisan — polovično uvezen izvod koji više nije ni duplikat ni ceo. Produkcioni put (`ImportOnePdfIntoBankaImport_Core`) zove core direktno jer već stoji unutar `ImportBankaInbox_TX`, pa nema ugnežđene transakcije. Uz to: `ToBimDatum` je pri neuspešnom parsiranju vraćao **original**, pa je javni `SaveBankaImportRows` i dalje mogao da upiše tekst koji nije datum (ili datum van opsega) ako ga neko pozove mimo parsera. Zamenjen je `RequireBimDatum`-om koji vraća `Date` ili diže grešku (`ERR_BIM_SAVE_BASE + 7/8`), uz nov `modParse.IsPoslovnaGodina` da opseg ima jedno mesto istine. **(17) Vidljiv disambiguator u listi partnera (FM-0024 #7, drugi deo).** `FillComboDisplayID` drži ID u skrivenoj koloni (`0 pt`) — kod bira tačan ID, ali dva partnera istog naziva operateru izgledaju identično, pa je izbor pogađanje (najgore kod avansa, gde ni faktura ne pomaže da se identitet prepozna). Nov `ComboDisplayWithID`/`ShowIDInComboDisplay` u `modComboBinding` dopunjava prikaz ID-em (`Agro Trade  [KUP-284]`); skrivena kolona ostaje netaknuta, pa `GetComboID` radi kao pre. **(18) AUD-054 na RF-09 import lancu.** `ImportBankaInbox_WithDrivePull` (javni ulaz preko `ImportBankaInbox`) i `PullBankPdfsFromDriveProduction` su imali `LogErr SRC` + `Err.Raise Err.Number, SRC, Err.description`: `LogErr` interno prolazi kroz `On Error Resume Next`/`On Error GoTo 0` i **resetuje** `Err`, pa je uvoz koji je uredno rollback-ovan (`ImportBankaInbox_TX`) na vrhu lanca gubio broj i opis uzroka — direktna kontradikcija paketu koji zatvara tiho padanje uvoza. Oba mesta sada hvataju `errNum/errDesc/errSrc` pre logovanja. Regresiju čuva nov `tools/check-banka-eh.py` (statički, bez Excela): traži `Err.Raise Err.Number` i čitanje `Err.*` posle `LogErr` u banka modulima — pri uvođenju je odmah našao još dva takva mesta u `frmBankaImport` (`UserForm_Activate`, `ConfirmManyCandidatesSplit`), oba popravljena. **(19) Pali batch ≠ „0 mapirano" (dorada u rundi 6).** Prva verzija ove izmene **nije radila**: `Err.Raise` je stajao pod još aktivnim `On Error Resume Next` (uključen zbog best-effort logovanja/monitoringa u istom EH bloku), pa se raise tiho gutao i funkcija se normalno vraćala sa `0` — tačno ponašanje koje je trebalo da ukloni. Dodat `On Error GoTo 0` pre raise-a (isti redosled koji `AutoMapStrongKeysBankaImport_TX` ima od početka), a `tools/check-banka-eh.py` je proširen provere (c): svaki `Err.Raise` pod aktivnim `Resume Next`. Checker je pre proširenja davao lažno zeleno na živom blocker-u; posle proširenja ga reprodukuje, a širok skan celog `src-vba/` pokazuje da je to bilo **jedino** takvo mesto u projektu.  `AutoMapAllBankaImport_TX` je posle rollback-a vraćao `0` bez greške, pa je `btnAutoSve_Click` odmah prikazivao „Automatski mapirano: 0" — uspešan oblik rezultata nad poništenim batch-om. Sada propagira grešku (isti ugovor kao `AutoMapStrongKeysBankaImport_TX`), a klik-handler je dobio EH koji poruku razlikuje. **(20) `SafeBlockCandidates` više ne izjednačava sve greške.** Helper je svaku grešku pretvarao u tekst i time otvarao „ručno uz potvrdu" tok — schema/read problem je vodio u dijalog koji nudi potvrdu podele, kao da je reč o 3+ kandidata. Sada se **samo** `ERR_BMAP_MANUAL_REQUIRED` mapira u taj tok, sve ostalo se propagira; `UpdateAutoPreview` je dobio EH koji razlog prikazuje u panelu umesto da klik na listu otvori VBA runtime dijalog. **(21) AUTO i RUČNI izvor bloka su razdvojeni.** `EffectiveBlockNo` je uveden za RUČNI put (combo ima prednost nad pozivom na broj), ali ga je koristio i `BuildOutgoingPreview`, tj. AUTO deo preview-a — a AUTO writer (`MapBankaImportAsKooperantBlock`) ne vidi nijednu kontrolu forme i uvek ide po `PozivNaBroj`. Uz `PozivNaBroj = A` i ručno izabran combo `B`, preview je prikazivao blok **B** (i `cmbOtkupBlok_Change` je odmah osvežavao AUTO prikaz), a „Automatski mapiraj red" knjižio **A**. Sada: nov `AutoBlockNoForBim` u `modBankaMapiranje` je **jedini** izvor za auto put i zove ga i writer i `BuildOutgoingPreview` (sve tri grane), a form-lokalni helper je preimenovan u `EffectiveManualBlockNo` i koristi ga samo ručni preview. Test T09 dokazuje ugovor kroz produkcionu putanju: dva bloka istog kooperanta, izvod pokazuje na prvi → auto knjiženje pogađa blok iz poziva na broj, drugi ostaje netaknut. **(22) „Jedan red ne obara batch" prošireno na celu klasu (review 8).** Batch omotač je gutao samo `ERR_BMAP_MANUAL_REQUIRED`, pa je red koji se AUTO razreši na **već plaćenu fakturu** dizao `ERR_BMAP_BASE + 53` i rollback-ovao ceo „Auto sve" — ista rupa koju AUD-025 zatvara, samo kroz drugu grešku. Nov `IsManualRequiredBankaError` definiše klasu „operater odlučuje" (`+50` 3+ kandidata, `+51` faktura drugog kupca, `+52` stornirana, `+53` već plaćena) — sve se dižu **pre** ijednog upisa za taj red, pa gutanje ne ostavlja parcijalno stanje. `+54` (avansni deo nije proknjižen) je namerno IZVAN klase: nastaje posle prvog `SaveNovac`-a i mora da obori batch. **(23) Ručni kooperant sa praznim izborom bloka (review 8).** Lista blokova se puni ali se ne auto-selektuje, pa je prazan `cmbOtkupBlok` **default** slučaj — a on je išao u zasebnu granu (`MapBankaImportAsKooperantBlock_TX`, bez potvrde podele), gde blok sa 3+ stavki završava generičkom greškom, iako preview (koji već računa efektivni blok) nudi podelu. Tj. „3+ blok ima izlaz" je radilo samo ako operater ručno klikne blok. Grana je uklonjena: ručni put uvek koristi `EffectiveManualBlockNo` (isti blok koji preview prikazuje) i uvek ide kroz `ConfirmManyCandidatesSplit`; za prazan blok ponašanje je nepromenjeno (nema kandidata → ceo iznos kao avans). **(24) Sitnije (review 8).** `AutoMapAllBankaImport_TX` sada postavlja `gBankaSilentBatch` (strong pass je to radio, „Auto sve" nije — pa je per-red `MsgBox` blokirao prolaz), uz vraćanje zastavice i na EH putanji pre propagacije greške. `CountStrongKeyReadyBankaImport` čita ključeve iz već učitanog reda (`ResolveStrongKupacByKeys`/`ResolveStrongKooperantByKeys`) umesto da po redu ponovo čita celu `tblBankaImport` — hint se računa pri svakom osvežavanju liste, pa je to bio perf regres na velikom izvodu. **Testovi:** nov `modTestBanka.RunBankaImportTestSuite` — tvrda kapija sa 11 grupa provera (podiže grešku kad ijedna padne): nemoguć datum odbijen a validan tačno parsiran; ista transakcija na dva računa prolazi obe; 3+ kandidata daje `ERR_BMAP_MANUAL_REQUIRED` a batch mapira zdrav red; pogrešan smer odbijen bez knjiženja; `01.02.2026` = 1. februar i u stagingu je `vbDate`; uplata preko otvorenog iznosa fakture daje dva reda čiji je zbir jednak iznosu iz izvoda, a već plaćena faktura se odbija), sve u jednoj transakciji koja se uvek rollback-uje (prefiks `BIT-`). | Yes — AR §19 (AUD status) uz sledeći snapshot | `.frx` netaknut; bez novih `Poruka()` ključeva; **ponašanje je strože nego pre** — nevalidan datum obara uvoz izvoda, pogrešan smer i anomalan blok se odbijaju umesto da prođu/pucaju |
| v6.46 | 2026-08-08 | **RF-22 / AUD-032 — SEF UX i lifecycle** (`modSEFService`, `modSEFStatusSync`, `modSEFClient`, `modSEFValidator`, `frmSEF`, `modConfig`, `modPoruke`, `modSEFTests`). Prvi paket M6. Bez izmene šeme, `.frx` netaknut, bez novih form `WithEvents`; SEF submit/validate/JSON logika (RF-21 teren) nije dirana. **(1) Ugovor o ishodu slanja (FM-0037 #1).** `SendInvoiceToSEF_TX` je vraćao `submissionID` u **sve četiri** grane TX 2, pa je `frmSEF` bezuslovno prikazivao „Faktura poslata" i za REJECTED i za TECH_FAILED. Uvedene su tri **čiste** funkcije (bez I/O, pa su testabilne offline): `IsSuccessfulSEFSendState` (uspeh = `SEF_SENT`/`SEF_ACCEPTED`), `SEFSendFailureErrNumber` (REJECTED → `ERR_SEF_REJECTED`, ostalo → nov `ERR_SEF_SEND_FAILED`) i `SEFSendOutcomeMessage` (poruka po stanju, kroz `modPoruke`). Neuspeh sada **diže tipiziranu grešku umesto povratne vrednosti**. Mesto raise-a je bitno: **posle `tx.CommitTx` i posle `On Error GoTo 0`** — stanje fakture je već trajno, a raise ne ulazi u `EH` servisa, pa ne okida `SEF_SEND_EXCEPTION_AFTER_LOCAL_SENDING`/CRITICAL niti pokušava rollback commitovane transakcije. `frmSEF.btnPosalji_Click` poziv drži pod `On Error Resume Next`, čita `Err.Number`/`Err.Description` u lokalne pre `Err.Clear` i grana: uspeh → `SEFSendOutcomeMessage` (`vbInformation`), ta dva koda → ishod kao `vbExclamation`, sve ostalo → `Err.Raise` u postojeći `EH` (`vbCritical`). Namerno **nije** korišćen `On Error GoTo 0` + `Resume` unutar handlera. **(2) Prazan/nepoznat status ≠ SENT (FM-0038).** `ParseStatusResponse` `Case Else` je imao `FirstNonEmpty(statusValue, "SENT")` — odgovor bez `Status` polja je nepotvrđenu fakturu upisivao kao poslatu. Fallback je sada `SEF_STATUS_UNKNOWN` (`"UNKNOWN_STATUS"`, nova konstanta u `modConfig`, uz postojeći `WF_SEF_UNKNOWN`); nepoznat ali neprazan status se čuva **doslovno** (SEF sme da doda status koji mi ne poznajemo — to nije razlog da se prepiše). Odluka „šta je poznat status" izdvojena je u `IsKnownSEFRefreshStatus`, a „gde ide workflow" u `SEFUnknownStatusTargetState`: `SEF_SENDING` → `SEF_UNKNOWN` (jedini slučaj koji **mora** da se pomeri, inače ostaje recovery petlja), sva ostala stanja → prazan povratak = ne diraj workflow, upiši samo refresh polja. Event postaje `SYNC_FAILED`, `SEFLastErrorCode = UNKNOWN_STATUS`, monitoring `SEF_STATUS_UNKNOWN`/WARN/`MANUAL_REVIEW`/`needsManualReview=True` (ranije INFO `SEF_STATUS_UPDATE`). **`SEF_UNKNOWN` je bio slepo crevo:** `modSEFValidator` je dozvoljavao `SEF_SENDING → SEF_UNKNOWN`, ali `SEF_UNKNOWN` kao `oldState` nije imao `Case` → padao je u `Case Else` („Unknown current workflow state"), pa bi svaka sledeća tranzicija pukla. Dodate su `SEF_UNKNOWN → SENT/ACCEPTED/REJECTED/TECH_FAILED` i `frmSEF` drži „Osveži status" aktivnim za to stanje, pa ručna provera ima izlaz. **(3) Refresh/recovery više ne lažu uspeh (FM-0037 #13).** `RefreshSEFStatus_TX` je završavao sa `= True` bez obzira na ishod; sada vraća `refreshOk` — True samo za upotrebljiv status, False za pad API-ja i za nepoznat status. `RecoverStuckSEFSendingInvoice` je `Function As Boolean` (`IsSEFRecoveryComplete` = „više nije `SEF_SENDING`") i grana na **tri** ishoda umesto bezuslovnog „Recovered" event-a: nije izašla iz SENDING → `SYNC_FAILED`; izašla uz upotrebljiv status → `STATE_CHANGED` „Recovered"; izašla bez upotrebljivog statusa → `SYNC_FAILED` „moved out for manual review". Zatvorena su **oba** uzroka večne petlje iz nalaza: (a) remote `STORNO/CANCELLED` dok je lokalno `SEF_SENDING` je menjao samo refresh polja → faktura ostaje `SENDING` → svaki startup piše lažan „Recovered"; sada se pomera na `SEF_SENT` (tranzicija dozvoljena, faktura je očigledno stigla do SEF-a, a `RefreshPendingOutbound` je posle toga preskače kao terminalnu). (b) pad refresh-a nad `SEF_SENDING` je ciljao `SEF_SYNC_ERROR` — **zabranjena tranzicija** → izuzetak → faktura trajno zaglavljena i greška pri svakom startup-u; nov `SEFRefreshFailureTargetState` za `SEF_SENDING` cilja `SEF_UNKNOWN`, za ostala stanja ostaje `SEF_SYNC_ERROR`. **Pouka:** „vrati `True` na kraju funkcije" je ista klasa greške kao `Boolean` guard sa `False` u `EH` (RF-08) — povratna vrednost mora da nosi ishod, inače i pozivalac i brojači vide uspeh koji se nije desio. **(4) `cmbFaktura_Change` → `ClearSEFInfo`.** Reuse postojećeg `ClearSEFInfo`; obična control-event procedura, **nijedna nova `WithEvents` deklaracija** (`SELF_UPDATE.md` zamka #11). **(5) Test makroi sa pravnim side-effect-om (FM-0037 #6).** `Test_CancelInvoiceOnSEF_TX`/`Test_StornoInvoiceOnSEF_TX` (hardkodovan `FAK-00007`, bez potvrde, vidljivi u Alt+F8 uz obične alatke) su **uklonjeni**, a ne guardovani: `modSEFTests` već ima `RunSEFCancelLiveSuite`/`RunSEFStornoLiveSuite` iza `SEF_TEST_ALLOW_LIVE` + `SEF_TEST_ALLOW_CANCEL_STORNO` + `ConfirmDangerousSEFMutation`, pa bi kapija na duplikatu značila dve putanje ka istom pravnom činu. Ostali dev makroi u `modSEFService`/`modSEFStatusSync` (send/retry/recover/refresh — takođe živi SEF) prebačeni su u `Private`: nestaju iz Alt+F8, a i dalje se pokreću iz VBE-a (F5). Caller-check nad celim `src-vba/` + `gas/`: nijedan poziv. **(6) Batch sažetak (FM-0037 #15).** `RecoverAllStuckSEFSendingInvoices` i `RefreshPendingOutboundInvoices_TX` su `Function As String` i vraćaju sažetak koji `frmSEF` prikazuje; uvedene su kategorije koje su ranije bile skrivene u „nije puklo": `NotRecovered` (poziv prošao, faktura i dalje `SEF_SENDING`) i `Unresolved` (refresh prošao, status neupotrebljiv), uz per-item `SEF_RECOVERY_INVOICE_NOT_RECOVERED`/WARN. `Sub` → `Function` ne lomi statement-pozivaoce (`modMain` startup recovery, `modSEFTests`). **(7) AUD-054 usput.** `ApplySEFStateOrRefreshOnly` EH je bio `LogErr` pa `Err.Raise Err.Number` (= `Err.Raise 0`, greška se guta); pošto je funkcija ionako u paketu, popravljen je po BUG-1 obrascu (capture pre `LogErr`, fallback `ERR_SEF_STATE`). To je jedini AUD-054 site u ovom modulu — ostatak (43) ostaje zaseban mehanički pass. **(8) Review runda 2 — tri P1 nalaza.** **(R1) Adapter za zvanicni SEF status.** Kod gadja `sales-invoice?invoiceId=`, a zvanicni `SalesInvoiceStatus` prihvatanje zove **`Approved`**, ne `Accepted`; enum nosi i `Seen`, `Sending`, `Paid`, `OverDue`, `Archived`, `Mistake`, `Deleted` i sam `Unknown`. Rucna lista u `ParseStatusResponse` je poznavala samo `ACCEPTED/REJECTED/SENT/NEW/DRAFT/CANCELLED/STORNO/ERROR`. Dok je `Case Else` bio `"SENT"`, drift se nije video — to je i bio AUD-032b; posle popravke bi **stvarno odobrena faktura zavrsavala kao „nepoznat status, rucna provera"**, tj. popravka bi napravila novu klasu lazne poruke. Uveden je `ClassifySEFExternalStatus` (`modSEFStatusSync`) sa sedam klasa (`SEF_CLS_*`, `modConfig`) i **jedan je izvor istine za sve potrosace**: `ParseStatusResponse` (flagovi `Accepted`/`Rejected`), `RefreshSEFStatus_TX` (workflow **i** monitoring grana — ranije dve odvojene liste statusa koje su vec pocele da se razilaze), `IsTerminalExternalRefreshStatus` (batch skip), `IsFinalSEFStatus`/`IsPendingSEFStatus`, `frmSEF` boja. Klasa `INFO` (`Paid/OverDue/Archived`) postoji zato sto ti statusi ne nose odluku kupca: izdavalac moze da obelezi placeno, rok moze da istekne — belezi se `SEFStatus`, ali se workflow **namerno ne pomera** (zakljucak „placeno znaci prihvaceno" bi bio nas izum, ne podatak od SEF-a). `SEFStatus` ostaje **doslovan** spoljni status, pa je `APPROVED` dodat u `ValidateFakturaCanBeStorniranoOnSEF` i u `frmSEF` uslov za „Storniraj" — inace bi prihvacena faktura postala nestorniva (regresija koju bi uveo sam adapter). **(R2) Planer tranzicije umesto ad-hoc ciljeva.** `SEF_SYNC_ERROR -> SEF_SYNC_ERROR` (self-transition) i `SEF_UNKNOWN -> SEF_SYNC_ERROR` state machine ne dozvoljava, pa je **drugi uzastopni pad SEF-a** zavrsavao izuzetkom i rollback-om — bas u trenutku kad SEF ionako ne odgovara; a terminalni status dok je faktura u `SEF_UNKNOWN` je menjao samo refresh polja, pa je `UNKNOWN` bio trajan. Umesto trece ad-hoc funkcije za ciljno stanje, odluka je izdvojena u cist planer `SEFRefreshTargetState(currentState, classification)` koji **sam pita state machine** kroz nov `IsSEFTransitionAllowed` (ne-bacajuci omotac nad `ValidateAllowedTransition`, jedini izvor istine ostaje ona). Time nedozvoljena tranzicija vise nije moguca **po konstrukciji**, a ne po iscrpnosti spiska uslova. Dvokorak koji state machine zahteva (`SEF_SYNC_ERROR -> SEF_SENT -> SEF_ACCEPTED`) planer izrazava vracanjem prvog koraka, a upisivac `ApplySEFRefreshOutcome` ga zove najvise dva puta (`MAX_HOPS`). `ApplySEFStateOrRefreshOnly` i `IsFinalLocalSEFWorkflowState` su uklonjeni: pravilo „ne vracaj finalna stanja u `SEF_SENT`" je **posledica** legalnosti tranzicije (`ACCEPTED -> SENT` je ionako zabranjen), pa je bilo duplirano znanje. **(R3) `IsSEFRecoveryComplete` fail-open.** Provera `<> SEF_SENDING` je prazan string, `BOGUS_STATE` i `SEF_READY` proglasavala oporavkom. To nije teorija: helper se hrani iz `GetFakturaSEFWorkflowState`, a citaci u `modSEFPersistance` nose AUD-054 obrazac (`LogErr` pa `Err.Raise Err.Number` = `Err.Raise 0`), pa schema/read greska moze da vrati prazno — i batch bi je prijavio kao uspesan oporavak. Sada je whitelist tacno onih stanja u koja `SEF_SENDING` sme da predje; sve ostalo je `False`. **Pouka (ista u sva tri nalaza):** kad odluka ima svoj izvor istine — zvanicni enum, state machine, lista dozvoljenih ishoda — kod ga mora **pitati**, a ne prepisivati u sopstvenu listu; svaka prepisana lista je kopija koja tiho zastari. **Testovi:** `Test_SEFOfficialStatusEnumClassified` (svaki status zvanicnog enum-a ima eksplicitnu klasu) i **`Test_SEFRefreshTransitionMatrix`** — orkestracioni test nad **svih 12 stanja x 7 klasa** koji tvrdi da planer vrati ili prazno ili tranziciju koju `IsSEFTransitionAllowed` dozvoljava, plus konkretne ishode. Matrica je odmah nasla regresiju u **prvoj verziji planera** (izgubljen dvokorak iz `SEF_SYNC_ERROR`, faktura bi ostala zauvek u tom stanju) — tacno klasa greske koju izolovani testovi pomocnih funkcija ne vide. **Ostaje otvoreno (van AUD-032):** `PAID/OVERDUE/ARCHIVED` fakture se i dalje ne mogu stornirati iz forme (zateceno ogranicenje liste); `WF_SEF_STORNO` i dalje niko ne postavlja (FM-0037 #11, P2). **(9) Review runda 3 — semantika dve klase.** **(R4) `MISTAKE` u `SEF_CLS_TERMINAL` je pretvarao neuspelo slanje u lokalno „poslato".** Zvanicni `Mistake` je **greska prilikom slanja dokumenta**, a ne kraj njegovog zivota (zvanicno uputstvo dozvoljava `Cancel` bas za `Draft`, `New`, `Mistake`). Posto planer za svaku terminalnu klasu bira `WF_SEF_SENT`, lanac je bio: `SEF_SENDING` + `MISTAKE` -> `SEF_SENT` -> recovery vraca `True` -> batch preskace kao terminalan (`IsTerminalExternalRefreshStatus`) -> `Cancel` zakljucan (validator znao samo `DRAFT/NEW/ERROR`) -> `Retry` nedostupan (workflow nije `SEF_TECH_FAILED`). Faktura koja NIJE poslata ostajala je oznacena kao poslata i **bez ijedne operativne putanje**. Uvedena je zasebna klasa `SEF_CLS_SEND_FAILED`: `MISTAKE` + `SENDING`/`UNKNOWN` -> `WF_SEF_TECH_FAILED`, izlazi iz terminal-skip grupe i iz `IsFinalSEFStatus`, telemetrija `SEF_STATUS_SEND_FAILED`/WARN/`RETRY`, u formi crvena umesto zute. Iz `SEF_SENT`/`SEF_SYNC_ERROR` planer vrati prazno (state machine nema put ka `TECH_FAILED`), pa se status samo zabelezi -- svesno, jer je alternativa izmisljanje tranzicije. **Pouka:** klasifikacija po *ishodu dokumenta* (poniste. vs greska) nije isto sto i klasifikacija po *daljem toku* — „nema vise automatskog napretka" je bio pogresan kriterijum, jer `Mistake` ima jasan sledeci korak. **(R5) `INFO` je ostavljao recovery zaglavljenim.** Klasa je bila tacno prepoznata, ali je planer za nju uvek vracao prazno, pa je `SENDING` + `PAID` ostajalo `SENDING`, a `UNKNOWN` + `ARCHIVED` ostajalo `UNKNOWN` — uz protivrecnu kombinaciju: refresh vraca `True`, UI javlja „status osvezen", recovery vraca `False`, i startup istu fakturu obradjuje pri svakom pokretanju. Prvobitno obrazlozenje („status ne dokazuje odluku kupca, pa ne pomeramo workflow") je bilo tacno kao tvrdnja, ali je odgovaralo na **pogresno pitanje**: `Paid`/`OverDue`/`Archived` ne dokazuju **prihvatanje**, ali nedvosmisleno dokazuju da je dokument **u SEF lifecycle-u**, dakle da nije vise „u slanju". `INFO` je zato u istoj grupi kao `PENDING`/`TERMINAL` (`desired = WF_SEF_SENT`); finalna lokalna stanja stiti provera legalnosti tranzicije, a prihvacenom je ne proglasava — za to je `SEF_CLS_ACCEPTED`, sto test izricito tvrdi (`SEFRefreshTargetState(SENDING, INFO) <> WF_SEF_ACCEPTED`). **Pouka:** „nemamo dovoljno informacija za zakljucak A" ne znaci „nemamo nikakvu informaciju" — konzervativnost je opravdana samo za onaj zakljucak koji zaista nije dokazan. **(R6, P2)** Telemetrija je spajala `TERMINAL` i `INFO` u `SEF_STATUS_TERMINAL`, pa su placene/dospele/arhivirane fakture izgledale ugasene; razdvojeno u `SEF_STATUS_INFO`. **Capability ugovor.** `Cancel`/`Storno` su bila **dva rucna spiska** — jedan u validatoru (fail-closed kapija), drugi u `frmSEF` (enable dugmeta) — pa je forma mogla da nudi akciju koju kapija odbija i obrnuto; taj isti drift je i sakrio `MISTAKE`. Uvedeni `CanCancelSEFStatus`/`CanStornoSEFStatus` u `modSEFValidator`, koje koriste **oba** mesta. `Test_SEFStatusCapabilities` proverava stvarnu sposobnost, ne naziv klase: `Mistake` -> cancel da/storno ne, `Approved` -> storno da/cancel ne, prazan status -> ni jedno (fail-closed), nijedan status ne nudi obe akcije, i `SEF_SENT` nema retry putanju (razlog zasto `Mistake` ne sme tamo). **Otvoreno, van AUD-032:** posle `MISTAKE` -> `SEF_TECH_FAILED` retry pravi **nov** `requestId` (`ShouldReuseLastSubmission` trazi prethodnu submisiju u `FAILED`/`CREATED`), pa je preporucen redosled „Otkazi pa Retry"; retry bez otkazivanja zavrsava kao 409 -> `CONFLICT` -> manual review (AUD-030), dakle fail-closed, ne tiho. Dedup ugovor za retry je RF-21 teren (FM-0037 #2). **(10) Review runda 4 — `MISTAKE` lifecycle izvrsiv kroz produkcionu putanju.** **(R7) `SEF_SENT` + `MISTAKE` nije prelazio u `SEF_TECH_FAILED`.** Runda 3 je rutirala `MISTAKE` u `WF_SEF_TECH_FAILED`, ali state machine ga je dozvoljavao samo iz `SEF_SENDING`/`SEF_UNKNOWN` — a **standardna** sekvenca je uspesan submit -> `SEF_SENT` -> refresh vrati `MISTAKE`. Planer je vracao prazno, faktura ostajala „uspesno poslata", i (novo od runde 3, jer `MISTAKE` vise nije terminalan) batch ju je osvezavao u nedogled. Dodate tranzicije `SEF_SENT -> SEF_TECH_FAILED` i `SEF_SYNC_ERROR -> SEF_TECH_FAILED`. **Pouka o testu:** matrica je ovu kombinaciju „pokrivala" invarijantom „ili prazno ili legalno" — a prazno je bilo legalno, pa je propustila. Invarijantni test dokazuje da kod **ne radi nesto pogresno**, ali ne i da **radi ono sto treba**; za to je potreban eksplicitan ocekivan ishod po kombinaciji (dodat za obe). **(R8) Retry bi i posle toga bio odbijen kao duplikat — poslovna odluka.** Lanac potvrdjen u kodu: uspesan submit upisuje `SubmissionStatus = SENT`; status refresh namerno ne dira submission red; `ShouldReuseLastSubmission` trazi `FAILED`/`CREATED`; `HasSuccessfulSEFSubmission` (fail-closed guard, AUD-031d) dize `ERR_SEF_DUPLICATE`. UI je dakle nudio dugme koje kapija svakako odbija. Izabrana opcija **(b) — ne nuditi Retry**, jer da li SEF prihvata ponovni POST istog `requestId` za dokument u statusu `Mistake` **nije proverivo staticki** (isti zakljucak kao FM-0037 #2), a slanje fakture je pravni cin. Uveden `CanSendSEFInvoice(workflowState, sefStatus)`: sendable workflow **I** odsustvo zivog dokumenta na SEF-u (blokira klase `ACCEPTED`/`PENDING`/`INFO`/`TERMINAL`/`SEND_FAILED`; `REJECTED` dozvoljen — postojeci resubmit tok; `ERROR`/`UNKNOWN`/prazno dozvoljeno — tehnicki pad bez dokumenta, tu retry radi kao i pre). Koriste ga **i** `ValidateFakturaForSEF` (raise sa razlogom i sledecim korakom, PRE generickog duplicate-a) **i** `frmSEF`. **(R9, P2) Uspesan Cancel je ostavljao „Retry" upaljen** (`CancelInvoiceOnSEF_TX` menja samo refresh polja, dugme gledalo samo workflow) — resava ista kapija: posle cancel-a `SEFStatus = CANCELLED` -> klasa `TERMINAL` -> slanje blokirano. **Namerno NIJE uvedeno `WF_SEF_CANCELLED`** (druga ponudjena opcija): trazilo bi izmenu state machine, migraciju zatecenih redova i novo terminalno stanje u svim izvestajima; zajednicka kapija resava isti problem bez ijednog od toga. Slab assert u live cancel testu (`Len(afterWorkflow) > 0`, kroz koji je `SEF_TECH_FAILED` prolazio) zamenjen tvrdnjom da otkazana faktura ne sme biti ponudjena za slanje. „Osvezi status" omogucen i za `SEF_TECH_FAILED` **kad `SEFDocumentId` postoji** (obican tehnicki pad ga nema, pa bi refresh pukao). **Pouka (treci put u ovom paketu):** dve liste za istu odluku uvek se raziđu — cancel/storno (runda 3), send (runda 4), status (runda 2). Svaka je resena istim potezom: jedna funkcija koju koriste i kapija i UI. **Otvoreno za vlasnika:** ako demo SEF potvrdi retry istog `requestId` za `Mistake`, treba transakciono prebaciti submission red u retry-eligible stanje i skinuti `SEND_FAILED` iz blokirane liste — jedna izmena na jednom mestu. **(11) Review runda 5 — trajna cinjenica umesto promenljivog statusa.** **(R10)** `CanSendSEFInvoice(workflow, sefStatus)` je postojanje SEF dokumenta zakljucivao **iz statusa**, a `SEFStatus` se prepisuje pri svakom padu refresh-a (`FAILED`/`HTTP_ERROR` -> klasa `UNKNOWN` -> „nema dokumenta"). Sekvenca je realna i uvedena je **bas rundom 4**, koja je omogucila „Osvezi status" iz `SEF_TECH_FAILED` kad docId postoji: submit uspe -> `SEF_SENT` + `SEFDocumentId` + submission `SENT` -> refresh vrati `MISTAKE` -> `SEF_TECH_FAILED` (slanje blokirano) -> **sledeci refresh padne** -> `SEFStatus = HTTP_ERROR` -> guard vrati `True` -> forma pali „Retry" -> klik pada na `HasSuccessfulSEFSubmission`. Isto i posle uspesnog Cancel-a. Potpis prosiren na `CanSendSEFInvoice(workflowState, sefStatus, sefDocumentId)`, a redosled provere obrnut: **prvo trajna cinjenica** (neprazan `SEFDocumentId` -> blokada, bez obzira na status), status je tek rezervna odbrana ako je docId izgubljen. `REJECTED` sazet na „sendable samo iz `SEF_READY`" (pripremljen resubmit tok). Provereno da normalne retry putanje prezive: ni tehnicki pad ni 409 `CONFLICT` ne upisuju `SEFDocumentId` u `tblFakture` (pisu ga samo success grane), a `PrepareRejectedInvoiceForResubmit` ga brise. **(R11, P2)** Poruka je za SVAKI blokiran status savetovala Cancel, koji nije dozvoljen za `Approved`/`Sent`/`Paid`/`Archived`/vec `Cancelled`; uveden `SEFSendBlockedNextStep` koji sledeci korak izvodi iz istih capability funkcija (`CanCancelSEFStatus` -> Cancel, `CanStornoSEFStatus` -> Storno, inace refresh/portal). **Pouka (poslednja i najvaznija u paketu):** kad guard odgovara na pitanje „postoji li X", mora da gleda **trajan zapis** o X-u, a ne izvedeno/promenljivo polje koje se prepisuje pri gresci. Isti obrazac kao AUD-032c (`IsSEFRecoveryComplete` je citao stanje koje na read gresci moze biti prazno) i kao razlog zasto `RequireOtkupAktivanZaStampu` (RF-08) cita SIROVU tabelu umesto filtriranog niza. Tri runde su isle na „dve liste za istu odluku", ova na „pogresan izvor za istu cinjenicu". **(12) Review runda 6 — dokumentovani resubmit tok je bio mrtav.** **(R12)** Faktura koju SEF odbije **tek na refresh-u** nije mogla kroz sopstveni korektivni tok: uspesan submit upise submission red kao `SENT`; refresh vrati `REJECTED` i pomeri workflow, ali submission red NAMERNO ne dira (zakomentarisan `SaveSEFSubmissionResult_Row`, da se podaci o originalnom submit pozivu ne prepisu podacima iz status upita); `PrepareRejectedInvoiceForResubmit` ocisti `SEFDocumentId` i link, ali ne i stari `SENT` red; sledece slanje prodje `CanSendSEFInvoice` pa padne na `HasSuccessfulSEFSubmission` (`ERR_SEF_DUPLICATE`). Moj komentar uz `CanSendSEFInvoice` je pritom tvrdio da je `REJECTED` „namerno dozvoljen jer je to postojeci resubmit tok" — **tvrdnja o putanji koja ne radi**. Nov `DischargeSentSEFSubmissions_Row` razduzuje submisije u statusu `SENT` -> `SEF_SUB_REJECTED` unutar iste TX (dodat snapshot `tblSEFSubmission`), i to upisuje **tacno ono sto bi `SaveSEFSubmissionResult_Row` upisao za `response.Rejected`** — ne izmislja se stanje, primenjuje se postojece mapiranje sistema. `SEF_SUB_ACCEPTED` se ne dira (prihvacena submisija uz odbijen workflow = neuskladjen podatak -> guard s pravom blokira). Razmatrana alternativa „odblokirati zakomentarisani `SaveSEFSubmissionResult_Row` u refresh-u" je odbacena: prepisala bi `HttpStatus`/`ResponseBody`/`ApiStatus` originalne predaje podacima status upita i unistila audit trag. **Testabilnost je iznudila refaktor:** `clsTransaction.BeginTx` PUCA na ugnezdjenu transakciju (`Err.Raise 5, "Transaction already active"`), pa se tok koji zivi unutar `_TX` procedure **ne moze** pokriti testom — telo je izdvojeno u `PrepareRejectedInvoiceForResubmit_Row` po obrascu koji projekat vec nosi (`CreateFaktura`/`_TX`, `SaveMagacinCore`/`SaveMagacin`); javni potpis i pozivaoci nepromenjeni. Nov integracioni test seed-uje realno stanje, tvrdi da guard PRE pripreme blokira a POSLE ne, uz `SEF_READY` + obrisan docId + sendable po zajednickoj kapiji, i uvek rollback-uje. **Posledica koju treba znati:** `RunSEFTestSuite` vise nije „bez izmene tabela" — zapisano je u zaglavlju suite-a, jer bi stara tvrdnja postala netacna (ista klasa greske koju trijaza vec belezi za `SEF_TEST_LOG`). **Pouka:** cist capability test je dokazao da je funkcija „sendable", a ne da se faktura moze poslati — dve razlicite tvrdnje. Kad odluka zavisi od stanja u tabelama, bar jedan test mora da prodje kroz stvarne tabele i stvarnu kapiju. **(13) Review runda 7.** **(R13) Rollback testa nije vracao crash-recovery Journal.** `AppendRow` odmah pise CSV red preko `modJournaling.WriteJournalRow`, a `tx.RollbackTx` vraca samo Excel tabele -- posle svakog `RunSEFTestSuite` ostajali bi lazni journal zapisi (test faktura, test submisija, test event) i sledeci start bi mogao da javi lazno upozorenje o gubitku podataka. Projekat ima namenski `modJournaling.SetTestModeQuiet` (uveden uz RF-27 `modAgrohemijaTests`, isti seed+rollback ciklus) -- propustio sam da ga upotrebim. Test ga sada pali i **vraca ZATECENO stanje** (`IsTestModeQuiet` pre, ne bezuslovno `False`), i u uspesnoj putanji i u `EH`, uz `StopAutoSaveTimer`. **Pouka:** kad se test oslanja na rollback, mora se proveriti STA JOS pise pri upisu -- transakcija pokriva ListObject, ali ne i CSV journal, ni AutoSave raspored, ni log sheet. **(14) Review runda 8 — pred merge.** **(R15) AUD-054 na direktnoj RF-22 putanji.** Paket tvrdi fail-closed kapije, a planer na kraju zove writer koji gresku NIJE propagirao: `UpdateFakturaSEFState_Row`, `UpdateFakturaSEFRefreshFields_Row`, `UpdateSEFLastSyncAt_Row`, `ClearFakturaLastSubmission_Row`, `HasSuccessfulSEFSubmission` (sam duplicate guard) i validatori su imali `LogErr` pa `Err.Raise Err.Number` = `Err.Raise 0`. Popravljena **sva 22 mesta u `modSEFPersistance` (14) i `modSEFValidator` (8)**, ne samo imenovana -- mesavina popravljenih i nepopravljenih EH blokova u istom modulu je gora za dijagnostiku od doslednog modula. AUD-054 ostatak: 21 (bio 43). **(R16) Uspesan storno nije pomerao lokalni workflow.** `StornoInvoiceOnSEF_TX` je i na `Success=True` zvao samo `UpdateFakturaSEFRefreshFields_Row` -> trajna kontradikcija `SEFWorkflowState = SEF_SENT/SEF_ACCEPTED` uz `SEFStatus = STORNO`, koju batch jos i preskace kao terminalnu. Kontradikciju je **ucvrstio moj planer**, koji je celu `TERMINAL` klasu vodio u `WF_SEF_SENT`. `STORNO` izdvojen u `SEF_CLS_STORNO` (jedini spoljni terminal sa lokalnim parnjakom; tranzicije `SENT/ACCEPTED -> STORNO` su postojale, ali ih niko nije koristio -- FM-0037 #11), planer ga vodi u `WF_SEF_STORNO` uz most preko `SEF_SENT`. `Cancelled`/`Deleted` ostaju **external-terminal-only**, sada eksplicitno: lokalni state machine nema `WF_SEF_CANCELLED`, a uvodjenje bi trazilo izmenu state machine + migraciju zatecenih redova + izmene u izvestajima. Storno i cancel idu kroz **zajednicki** `ApplySEFExternalOutcome_Row` (isti writer koji koristi refresh) -- „sta spoljni status znaci za workflow" je time na jednom mestu za sve tri putanje. **(R17)** `PrepareRejectedInvoiceForResubmit_Row` je upisivao interni marker `SEF_READY` u kolonu `SEFStatus` (po definiciji: poslednji SPOLJNI status), cime bi se izgubio audit podatak da je SEF odbio fakturu; `sefStatus` se vise ne prosledjuje. **(R18)** `DischargeSEFSubmission_Row` proverava i remote identitet: kad i submission red i faktura nose `SEFDocumentId`, moraju se poklopiti, inace zastareo `SEFSubmissionIDLast` moze da oznaci pogresan pokusaj kao odbijen -- a to je zapis o predaji poreskom organu. Neslaganje puca (ne pretpostavlja se koji je tacan). **(R19)** Test-gap koji je i propustio storno nalaz kroz sedam rundi: matrica dokazuje da state machine DOZVOLJAVA `-> STORNO`, ali ne i da produkcioni put to RADI. Dodat `Test_SEFStornoMovesLocalWorkflow` nad stvarnim tabelama (iz `SEF_SENT` i iz `SEF_ACCEPTED`), kroz `ApplyStornoResultOnSEF_Row` -- telo TX-a izdvojeno istim `_Row` obrascem kao priprema resubmita. **Pouka:** invarijantni test dokazuje da kod ne radi nista zabranjeno; da radi ono sto treba dokazuje samo test nad stvarnim upisom. **(15) Review runda 9.** **(R20) Suite bi sigurno pao.** Runda 8 je `Storno` izdvojila u zasebnu klasu, ali je stari assert u `Test_SEFOfficialStatusEnumClassified` ostao `Storno -> TERMINAL`, dok novi u matrici tvrdi `-> STORNO`; isti runner zove oba i ima tvrd gate. Ispravljen assert i dva zastarela opisa. **Uzrok je sistemski:** promena semantike u produkciji **ne obara nista u repou** jer CI ne pokrece Excel -- jedini trag je operater koji pokrene suite. Zato je dodata `tools/check-sef-asserts.py`: PARSIRA produkcioni izvor (klasifikator, tabela tranzicija iz `ValidateAllowedTransition`, `WF_*` konstante) i uporedjuje ga sa svakim `AssertEquals` nad `ClassifySEFExternalStatus`/`SEFRefreshTargetState` u `modSEFTests`. Logika se NE kopira rucno -- provera se izvodi iz izvora, pa ne moze da zastari zajedno sa testom. Napomena o samoj skripti: prva verzija je prijavila 17 laznih neslaganja jer nije spajala VBA line-continuation pri parsiranju state machine-a (svaka `allowed()` je vracala False); ispravljeno pre commita -- alat za proveru i sam mora da se proveri. **(R21)** Lineage guard iz runde 8 nije bio izvrsen (test submission red nije imao `SEFDocumentId`): happy-path submisija sada nosi isti docId kao faktura, dodat scenario sa razlicitim docId (puca, submisija ostaje `SENT`, duplicate guard i dalje blokira) i kontrola da poklapanje razduzuje. **Pouka:** guard koji nijedan test ne izvrsi je isto sto i guard koji ne postoji -- a „test postoji" i „test prolazi kroz tu granu" su dve razlicite tvrdnje. **(16) Review runda 10 -- merge blocker.** **(R22)** `IsSEFRecoveryComplete` nije bio uskladjen sa `STORNO` semantikom uvedenom u rundi 8: zaglavljena `SEF_SENDING` faktura + spoljni `STORNO` -> planer je (ispravno, dvokorakom) odvede u `WF_SEF_STORNO`, ali whitelist recovery ishoda to stanje nije imao -> UI javlja „Recovery NIJE uspeo", batch broji `NotRecovered`, iako je faktura upravo uspesno izasla iz `SEF_SENDING` u ispravno terminalno stanje. **Nova verzija bas one greske koju AUD-032c uklanja** -- uvedena mojim sopstvenim dodavanjem `SEF_CLS_STORNO`, bez uskladjivanja downstream ugovora. Whitelist dopunjen, uz preformulisano obrazlozenje: spisak je izveden iz PLANERA (ishodi koje `SEFRefreshTargetState` moze da proizvede iz `SEF_SENDING`), ne iz tabele tranzicija -- `STORNO` nije direktan naslednik `SEF_SENDING` nego rezultat dvokoraka. Test pokriva ceo lanac, ne samo krajnju tacku. **Pouka (druga u istom paketu):** kad se doda nova klasa/stanje, mora se proci SVAKI downstream potrosac -- runda 9 je uhvatila zaboravljen assert, runda 10 zaboravljen ugovor. Zato `check-sef-asserts.py` postoji, ali on pokriva testove, ne i ovakve whitelist-e u produkciji. **(R23)** Sam `check-sef-asserts.py` nije bio onoliko nezavisan koliko je commit tvrdio: mapa `klasa -> ciljno stanje` je bila rucno prepisana u `plan()` -- alat protiv dupliranog znanja je i sam nosio duplirano znanje. Mapa se sada parsira iz `SEFRefreshTargetState`; rucno preslikana ostaju samo dva pravila (grana za `ERROR`/`UNKNOWN` i most preko `SEF_SENT`), sto je sada eksplicitno u zaglavlju skripte umesto ranije presiroke tvrdnje. Alat je proveren i negativnim testom (namerno pokvaren assert -> prijavljen, exit 1). **(R24)** Zaglavlje `RunSEFTestSuite` je tvrdilo „svi cisti osim jednog", a table-mutating testova je dva; ispravljeno i u suite-u i u release notes i u opisu PR-a (uz 8 -> 9 klasa, 12x8 -> 12x9 matrica). **(R14) Razduzenje suzeno.** `DischargeSentSEFSubmissions_Row(fakturaID)` (svi `SENT` redovi) zamenjen sa `DischargeSEFSubmission_Row(submissionID, fakturaID)`: `SEFSubmissionIDLast` se cita PRE brisanja linka i razduzuje se tacno ta submisija, uz proveru vlasnistva (raise ako pripada drugoj fakturi). Stariji `SENT` redovi se ne diraju. Da suzavanje ne bi vratilo mrtav tok u anomaliji, priprema je dobila **fail-closed post-proveru**: ako `HasSuccessfulSEFSubmission` i dalje vraca `True` (stariji `SENT`, ili prethodna `ACCEPTED`), priprema PADA sa `ERR_SEF_DUPLICATE` -- bolje glasan pad nego "pripremljeno" pa odbijanje na klik. Test pokriva i taj scenario. | Yes — AR §19 (AUD status) uz sledeći snapshot | **bez migracije**; koriste se postojeće SEF kolone `tblFakture`; `.frx` nedirnut i bez novih form `WithEvents`; izvori **ASCII-only**; **7 novih `Poruka()` ključeva** (`SEF_MSG_SEND_POSLATA`, `SEF_MSG_SEND_PRIHVACENA`, `SEF_MSG_SEND_ODBIJENA`, `SEF_MSG_SEND_TEH_GRESKA`, `SEF_MSG_SEND_NEPOZNAT_ISHOD`, `SEF_MSG_STATUS_NIJE_OSVEZEN`, `SEF_MSG_RECOVERY_NEUSPESAN`) → posle importa **obavezno `EnsurePoruke`**; re-import `modSEFService`/`modSEFStatusSync`/`modSEFClient`/`modSEFValidator`/`frmSEF`/`modConfig`/`modPoruke`/`modSEFTests`. Gates: nov **`RunSEFTestSuite`** — offline (nijedan poziv ka pravom SEF-u), **tvrd gate** sa `On Error GoTo 0` pre raise-a (RF-08 obrazac, da očekivana greška gate-a ne uđe u sopstveni `EH` i ne doda lažan failure); objedinjuje postojeće čiste testove (klasifikacija HTTP odgovora uklj. 409→CONFLICT, preciznost stavki, dozvoljene/zabranjene tranzicije) i tri nova seam testa: `Test_SEFSendOutcomeContract`, `Test_SEFStatusUnknownIsNotSent` (preko novog `TestProxyForParseStatusResponse`, po ugledu na `TestProxyForParseSubmitResponse`), `Test_SEFRecoveryOutcomeContract`. Suite je bio referenciran u `PLAN_SANACIJE` sekcija 5C ali **nije postojao** — svi živi suite-ovi su iza `SEF_TEST_ALLOW_*` kapija, pa se gate za SEF milestone do sada nije mogao pokrenuti bez pravog SEF poziva. `Test_LiveSendAndRefresh` je usklađen sa novim ugovorom (neuspeh mora da baci grešku i **ne sme** da vrati SubmissionID). **Operater-smoke:** odbijena faktura (poruka ≠ „poslata"), nepoznat status → ručna provera, recovery zaglavljenog `SENDING`, promena fakture u combou, Alt+F8 lista. **Van obima:** banka import/mapiranje (RF-09, nosi P0 AUD-007), banka export (RF-10), ostatak AUD-054. |
| v6.45 | 2026-08-08 | **RF-08 / AUD-011 + ostatak AUD-027 — faktura (vlasništvo, `Count=1`, transakcioni ulaz) i štampa (reprint storna, `.UnMerge`)** (`modFaktura`, `modPrint`, `modPoruke`, `modFakturaTests`). Poslednji paket M5 milestone-a — time je M5 (izveštaji + faktura) zatvoren. Bez izmene šeme, `.frx` netaknut. **(1) Vlasništvo prijemnice (FM-0034 #1):** `CreateFaktura` validation loop je verovao pozivaocu da su prosleđene prijemnice kupčeve — jedina provera je bio UI filter `GetPrijemniceByKupac` u `frmFakturisanje`, a UI nije sigurnosna granica (bilo koji drugi ulaz — test, budući PWA/SEF poziv, ručno sastavljen `stavke` payload — mogao je da fakturiše tuđu robu). Sada se za svaku stavku poredi `tblPrijemnica.KupacID` (po imenu kolone, uz `RequireColumnIndex` fail-fast guard) sa `kupacID` argumentom; neslaganje diže `vbObjectError + 1721` **unutar** `CreateFaktura_TX`, pa `RollbackTx` vraća sve. Poređenje je `Trim$` + case-sensitive, isto kao postojeći filter u formi. **Namerna posledica:** legacy prijemnica sa praznim `KupacID` se sada odbija (nije „ničija" — mora se dopuniti pre fakturisanja). **(2) `Count=1` guard (FM-0034 #2):** `rowPrijValidate = CLng(rows(1))` je uzimao prvi pogodak `FindRows`-a bez provere jedinstvenosti; dva aktivna reda sa istim `PrijemnicaID` su time fakturisala količinu/cenu pogrešnog reda — tiho i deterministički. Dodat je fail-closed `rows.count > 1` (`vbObjectError + 1707`, poruka nosi ID + `Count`), **isti obrazac** koji `RequireSingleFakturaRow` već nosi za `tblFakture` i `RequireSingleMasterSyncRow` (RF-28) za sync red. Nije uveden nov helper: `RequireSingleFakturaRow` je vezan za `TBL_FAKTURE`/`COL_FAK_ID`, a `rows.count = 0` grana je na tom mestu već postojala — dodata je samo `> 1` grana uz nju. **(3) `CreateFaktura` → `Private` (FM-0034 #3):** base funkcija radi bez transakcije, a upisuje na tri mesta (header `tblFakture`, stavke `tblFakturaStavke`, `Fakturisano`/`FakturaID` na `tblPrijemnica`) + zove `ApplyAvansToFaktura`; direktan poziv kod greške ostavlja pola posla. Jedini ulaz je sada `CreateFaktura_TX` (snapshot nad `tblFakture`/`tblFakturaStavke`/`tblPrijemnica`/`tblNovac`). Caller-compile provera nad celim `src-vba/`: nijedan modul/forma nije zvao base funkciju — `frmFakturisanje:703` i svih 5 mesta u `modBusinessFlowProTests` već idu kroz `_TX`, pa je promena potpisa bezbedna. **(4) Reprint storniranog otkupa (AUD-027 / FM-0031 #3):** `ReprintOtkupniListByOtkupID` je radio `d = ExcludeStornirano(d, TBL_OTKUP)` **pre** traženja reda, pa stornirani otkup nije pronađen → `brDok` ostaje prazan → raw fallback `If ids = "" Then ids = otkupID` ga ipak prosleđuje `OutputOtkupniList`-u. Poništen dokument je tako izlazio na papir bez ijedne oznake. Uvedena je **fail-closed kapija** `modPrint.RequireOtkupAktivanZaStampu(otkupID, sourceName)` koja čita **sirovu** `GetTableData(TBL_OTKUP)` (iz filtriranog niza se storno status po definiciji ne može utvrditi — to je i bio uzrok bug-a) i **prolazi tiho samo kad se otkup može dokazati kao jedinstven i aktivan**; sve ostalo diže grešku: nema kolone `OtkupID`/`Stornirano` (`RequireColumnIndex`, ne `GetColumnIndex`), prazna tabela, nema reda, **više redova sa istim `OtkupID`**, `Stornirano = "Da"`. `ReprintOtkupniListByOtkupID` je zove **pre svega ostalog**, pod `On Error GoTo BLOKADA`, pa svaki neuspeh provere završava u vidljivoj poruci (`Poruka("PRINT_ERR_OTKUP_BLOKIRAN")` + razlog) i odustajanju od štampe; razlog se hvata **pre** `LogErr`-a (BUG-1/AUD-054). **Review korekcija (bila prava rupa):** prva verzija je bila Boolean seam `OtkupStorniranZaStampu` sa `False` kao podrazumevanim ishodom **svake** greške — dakle fail-**open** baš u schema-corruption scenariju, i to onom najgorem: `ExcludeStornirano` bez kolone `Stornirano` takođe vraća **sirovu** tabelu, pa bi guard koji „ne zna" i filter koji „ne filtrira" zajedno pustili poništen dokument na papir. **Pouka:** kad `Boolean` helper nosi sigurnosnu odluku, njegov `EH` određuje smer otkaza — `Exit Function` posle `LogErr` je uvek „dozvoli". Sigurnosne provere zato treba pisati kao `Require*` koje **pucaju**, a ne kao `Boolean` koje ćute. Fallback za **aktivan** otkup bez `BrojDokumenta` je namerno zadržan (jedan red je bolji od nijednog); ukinut je samo put kojim je storno prolazio. **(5) `FillFakturaSablon` `.UnMerge` (AUD-027 / FM-0031 #19):** cleanup pre punjenja je radio `ClearContents` + `Borders.LineStyle = xlNone` + `Interior.ColorIndex = xlNone`, ali **ne** i `UnMerge`, dok red „UKUPNO:" nastaje kao `ws.Range(tot, tot.Offset(0, 4)).Merge` na poziciji `startCell.Offset(nStavke, 0)` — dakle **funkcija broja stavki**. Faktura sa 3 stavke odštampana posle fakture sa 1 stavkom je pisala preko merge-a zaostalog na offsetu 1. Dodat je `.UnMerge`, ali na **dinamičnom** opsegu. **Review korekcija:** zatečeni cleanup opseg (`startCell` .. `startCell.Offset(80, 5)`) i `NumberFormat` petlja (`Offset(0,1)`..`Offset(79,1)`) nose **fiksnu granicu od 80 redova**, a broj stavki nije ograničen — `frmFakturisanje` dozvoljava proizvoljan izbor prijemnica, a ni `CreateFaktura` ni renderer nemaju `nStavke <= 80` invarijantu. Deterministički lom je **81 → 82 stavke**: merge totala sa offseta 81 preživi cleanup, 82. stavka piše preko njega, funkcija završi u `EH` i vrati `Nothing` — **štampa tiho izostane**. Uz to broj prijemnice tipa `1/2026` od 81. stavke gubi `"@"` format i Excel ga može protumačiti kao datum. Umesto uvođenja poslovnog maksimuma (koji bi morao da se enforce-uje na tri mesta — UI, `CreateFaktura`, renderer), opseg sada prati stvaran obim: `cleanupRows = max(nStavke + 4, contentBottom - startCell.row)`. Isti opseg koriste `.UnMerge`, `ClearContents`, ivice/pozadina i `NumberFormat = "@"`. **Pouka:** `.UnMerge` je bio tačan alat, ali na netačnom opsegu — fiksna „dovoljno velika" granica u rendereru je invarijanta koju niko ne održava. **Review korekcija 2 (druga runda):** prva verzija dinamične granice čitala je `ws.UsedRange`, što je pogrešna semantika za ovu odluku — `UsedRange` obuhvata i **prazne ali formatirane** ćelije, a `EnsureFakturaSablon` formatira **ceo list** (`ws.cells.Font.name` / `.Size`). Dno je time moglo da bude poslednji red lista, pa bi svaki render radio `UnMerge`/`ClearContents`/`Borders`/`NumberFormat` nad ~6M ćelija u `A:F` — praktično zamrznut Excel i naduvan workbook. Uveden je seam `modPrint.SablonLastContentRow(ws, firstCol, lastCol)` = `Range.Find(What:="*", LookIn:=xlFormulas, SearchOrder:=xlByRows, SearchDirection:=xlPrevious)` nad `A:F`: gleda **samo sadržaj** i **samo kolone dokumenta**; fail-soft `0` vraća cleanup na `nStavke + 4`, opseg koji ovaj dokument ionako mora da pokrije. **Svesno NIJE dirano** treće predloženo mesto (`ws.cells.Font...` → samo korišćeni deo): taj font daje dosledan izgled i redovima stavki **ispod** izgrađenog rasporeda (`EnsureFakturaSablon` ne zna koliko će ih biti), pa bi sužavanje tražilo `LAYOUT_VER` bump i pogođen maksimalan broj redova — tj. povratak baš one fiksne invarijante koju smo upravo uklonili. Sa `Find`-om je pitanje da li je `UsedRange` naduvan **irelevantno** za cenu cleanup-a, pa je root-cause rizik zatvoren bez te izmene. **Pouka:** `UsedRange` odgovara na pitanje „dokle je list dodirnut", a ovde je pitanje „dokle je nešto napisano" — kad granica upravlja cenom operacije, semantika izvora granice je deo ispravnosti, ne detalj. **Namerno NIJE `ws.cells.UnMerge`**, iako to rade svi ostali `Fill*` šabloni (`:207/495/1000/1195/1502`): oni pre punjenja **ruše i grade ceo list**, pa je blanket `UnMerge` kod njih bez posledica; `FakturaSablon` je nasuprot tome **perzistentan** (`EnsureFakturaSablon` ga gradi jednom i preskače dok se `H1` `LAYOUT_VER` ne promeni) i u zaglavlju nosi sopstvene namerne merge-ove (`FakKupac`, `DocSellerHeader`, `DocTitleBlock`) koje bi blanket `UnMerge` trajno pokidao — sledeća faktura bi se štampala sa razbijenim zaglavljem. **Pouka:** „isti obrazac kao ostali `Fill*`" važi samo dok važi i njihova pretpostavka (list se svaki put gradi iznova); kod perzistentnog šablona obrazac se preuzima **opsegom koji se puni**, ne celim listom. | Yes — AR §19 (AUD status) uz sledeći snapshot | **bez migracije**; validacija pre upisa + priprema lista za štampu; `.frx` nedirnut i bez novih form `WithEvents`; izvori **ASCII-only**; **2 nova `Poruka()` ključa** (`PRINT_ERR_STORNIRAN_OTKUP`, `PRINT_ERR_OTKUP_BLOKIRAN`) → posle importa **`EnsurePoruke`**; re-import `modFaktura`/`modPrint`/`modPoruke`/`modFakturaTests`. **Strože ponašanje je namerno:** fakturisanje koje je ranije tiho prolazilo (prijemnica bez kupca / sa drugim kupcem, duplirani `PrijemnicaID`) sada pada vidljivo. Gates: `RunFakturaSmokeSuite` — **tvrd gate** (`Err.Raise` kad ijedna provera padne, po konvenciji RF-14/RF-06; ranije je samo prikazivao zbir, pa je automatizovani pozivalac mogao da ga prijavi kao PASS) + 4 nova testa: `Test_CreateFakturaBlocksPrijemnicaDrugogKupca`, `Test_CreateFakturaBlocksDuplicatePrijemnicaID`, `Test_ReprintBlocksStorniraniOtkup` (storno + **duplikat `OtkupID`** + **nepoznat `OtkupID`**; `tblOtkup` redovi u `clsTransaction` sa obaveznim rollback-om; gađa kapiju jer sama blokada otvara `MsgBox`) i `Test_FakturaSablonCleanupPratiBrojStavki` (**81 pa 82 stavke** — 82. red nije u zaostalom merge-u, stvarno je upisan, format mu je i dalje tekst, merge zaglavlja netaknut; **plus granica cleanup-a**: prazna formatirana ćelija na redu 5000 ne pomera `SablonLastContentRow`, i posle novog rendera i dalje ima svoju boju — dokaz da je cleanup nije dohvatio. Bez tabela, jer `FillFakturaSablon` prima stavke kao niz; šablon se na kraju **briše**, pa ga `EnsureFakturaSablon` regeneriše čist i test ne ostavlja ni stavke ni potpise ni `PrintArea`). `AppendTestPrijemnicaRow` je dobio obavezan `kupacID` parametar (test red bez kupca s pravom više ne prolazi). **Hard gate ima `On Error GoTo 0` pre sebe** — bez toga očekivana greška gate-a ulazi u `EH` suite-a, pa `LogFakturaFatal` doda lažan failure, `FinishFakturaSuite` se izvrši drugi put i prijavljen broj padova je za jedan veći od stvarnog (isti obrazac kao `RunMasterSyncSmokeSuite`). **Operater-smoke ostaje:** faktura sa 3 stavke posle fakture sa 1 stavkom. **Van obima (zapisano, ne dirano):** AUD-054 (SEF `Err.Raise 0`) — zaseban SEF-hardening; faktura-status/SEF lifecycle (RF-22); document-snapshot za reprint (TL-006). |
| v6.44 | 2026-08-07 | **RF-07 / AUD-024 + AUD-012 + deo AUD-027 — freshness izveštaja, vidljive greške, revers po tipu ambalaže** (`frmIzvestaj` kod, `modIzvestaj` seam-ovi, `modKarticaDetalji`, `modPoruke`). Nastavak RF-06: tamo *šta izveštaj računa*, ovde *šta izveštaj tvrdi da prikazuje* i *koji revers se odštampa*. Bez izmene šeme, bez ijednog novog upisa, `.frx` netaknut. **(1) Freshness (FM-0029 #1/#19):** status linija i naslovi štampe (`PrintIzvestaj`, `PrintKarticaPDF`, `PrintKarticaAmbalazePDF`) su čitali `txtDatumOd/txtDatumDo`, dok su podaci u listama generisani iz `m_curOd/m_curDo` zapamćenih pri kliku na „Prikaži" — promena datuma bez re-generisanja je davala **staru listu pod novim periodom**, i na ekranu i na papiru. Sve tri putanje sada idu kroz novi `PrikazanPeriod()` (isključivo `m_cur*`, prazno dok se izveštaj nije nijednom generisao); štampa pre prvog generisanja je blokirana porukom. Uz to su dodati `txtDatumOd_Change`/`txtDatumDo_Change` (obični control event handleri — **nijedna nova `WithEvents` deklaracija u formi**, `SELF_UPDATE.md` zamka #11) koji postavljaju `m_periodDirty`, pa status odmah nosi amber „NIJE OSVEZENO"; `btnUnos_Click` je jedino mesto koje flag čisti. **(2) Vidljiv `CleanFail` (FM-0029 #2):** `GenerateActivePage` je grešku lazy generisanja samo logovao (`LogErr` + `Resume`), pa je na ekranu ostajala **prethodna lista uz zelen status** „N redova ucitano" — tuđe brojke pod novim naslovom. Greška se sada pamti u `m_genFailMsg`/`m_genFailTab`, tab ostaje van `m_genTabs` (retry pri sledećem prelasku), a prijava ide **iz `CleanExit`, posle `EndTableCache` + `ScreenUpdating=True` i van aktivnog error handlera** (`MsgBox` pod `Resume` nije pouzdan): `ShowGenFailure` briše aktivnu listu, zove `KarticaDetalji_Clear`, boji status crveno i prikazuje opis greške. **(3) Tab-matrica (FM-0029 #3):** vidljivost stranica je bila hardkodirana u `UpdateReportMode` sa zbirnim tabovima 5/6/7 za **sve** tipove entiteta; RF-06 je te kombinacije doveo do `Empty`, ali su ostale u meniju. Odluka je izdvojena u testabilan seam `modIzvestaj.IzvestajTabDostupan(entitetTip, zbirni, pageIdx)` + `IZV_TAB_*` konstante, koji prati **stvarni dispatch** `Report*` funkcija (Vozač nema `IZV_TAB_PROSECNA_CENA` jer `ReportProsecnaCena` nema vozačku granu; Kooperant nema nijedan zbirni tab). `UpdateReportMode` sada samo pita matricu u petlji, a runtime tabovi (`m_otkPageIdx`/`m_ambPageIdx`, dinamički indeks preko statičkih) se pale eksplicitno i nisu deo matrice. Prazan skup tabova postavlja `m_noValidTabs` -> vidljiva poruka u statusu umesto prazne forme bez objašnjenja. Mapiranje UI labele u kod entiteta je izdvojeno u `IzvestajEntitetKod` (bio dupliran `Select Case` u `btnUnos_Click`). Uz to je iz `UpdateStatusLabel` izdvojen `ActiveReportList()` — jedno mesto istine za „lista aktivnog taba", koje koristi i putanja greške. **(4) Revers ambalaže (AUD-012 / FM-0029 #4/#5/#16):** `StampajReversAmbDok` je čitao `GetTableData(TBL_AMBALAZA)` **bez storno filtera**, uzimao `tipAmb` sa **prvog** reda dokumenta i sabirao količine preko **svih** tipova — dokument sa 25 letvarica + 15 plastičnih davao je jedan revers na 40 „letvarica". Dodat je `ExcludeStornirano` (reuse iz `modHelpers`), a tip ambalaže je postao deo ključa kroz nov čist seam `modIzvestaj.ReversRedPripada(rowDokID, rowDokTip, rowTipAmb, dokID, dokTip, tipAmb)` (trim + `vbTextCompare`, jer tip dolazi iz slobodnog unosa šifarnika; prazan tip je legitimna grupa, **ne** wildcard). Tip stiže sa izabranog reda pregleda preko novog gettera `KarticaDetalji_CurrentAmbTip` (uz postojeći `KarticaDetalji_CurrentAmbDok`; `ShowAmbRow` pamti kol. 2) — a `ReportAmbalazePojedinacni` red pregleda i inače grupiše po `DokumentID|TipAmbalaze`, pa je poklapanje 1:1. Bez prosleđenog tipa uzima se tip prvog reda dokumenta i sabira **samo** taj tip. Više od jedne noge po strani za isti tip (duplikat / više generacija) se prijavljuje operateru (`vbYesNo`) umesto tihog sabiranja. **(5) Cross-tab print (AUD-027 / FM-0030 #1 + FM-0029 #15):** print target panela „Detalji" je modul-stanje u `modKarticaDetalji`, nevezano za tab/listu/red — red izabran na Kartici je preživljavao prelaz na Ambalažu. `mpReports_Change` sada zove postojeći `KarticaDetalji_Clear` (isti helper koji `GenerateAmbalazeReport` već koristi; bez novog API-ja).  **(6) Review dopune (isti paket).** (a) **Matrica se razilazila sa core-om na `Kupac + Zbirni + Prosecna cena`:** kriterijum „postoji `Case` grana za taj tip" nije dovoljan — zbirni rezim salje `entitetID = ""`, a `ReportProsecnaCena` grana za Kupca ide kroz `GetPrijemniceByKupac` koji **bezuslovno** dodaje filter `KupacID = entitetID`, pa upit trazi prijemnice BEZ kupca i vraca prazno. (`OM` je imun jer grana `Case "OM", ""` eksplicitno hvata prazan ID kao „svi".) Tab je sklonjen; globalni prosek svih kupaca je **nov izvestaj** (poslovna odluka), ne UI podesavanje, i zapisan je kao otvoreno pitanje. Gate `T_E2E_ProsecnaCenaZbirniKupac` seed-uje prijemnice dva kupca u sentinel prozoru i pada **u oba smera**. Pouka: matricni `True/False` test ne dokazuje da ponudjeni tab moze da vrati podatke — za to treba E2E. (b) **Invalidacija konteksta:** `AutoRefresh` izlazi bez generisanja kad novi entitet nema nijedan izbor, a zatecene liste su ostajale kao „svez" rezultat pod novim identitetom (period ispravan, entitet pogresan — ista klasa kao AUD-024). Uveden centralni `InvalidateReportContext` na **pocetku** `AutoRefresh`-a (jedini ulaz za promenu entiteta/rezima/combo izbora): `m_genTabs.RemoveAll`, reset `m_hasGenerated`/`m_curTip`/`m_curID`/`m_curEntLabel`/`m_curEntName`/`m_genFail*`, `ClearAllReportLists`, `KarticaDetalji_Clear`, `SyncDetaljiVisibility`, `UpdateStatusLabel`. Print guard `AktivanTabGenerisan()` trazi da je **bas aktivan tab** u `m_genTabs`; naslov stampe i izbor zaglavlja kolona koriste **zapamceni identitet** (`m_curEntLabel`/`m_curEntName`/`m_curTip`) po istom principu po kom period koristi `m_cur*`. Vidljivost panela „Detalji" izdvojena u `SyncDetaljiVisibility` jer ciscenje lista okida `Change` evente koji panel mogu da pokazu na tudjem tabu. (c) **Kanonski kljuc tipa ambalaze:** pregled je grupisao po sirovom `dokIDv & "|" & tipv`, a `ReversRedPripada` poredio `Trim + vbTextCompare` — „Letvarica"/„letvarica" = dva reda pregleda, a svaki revers sabira oba. Uveden `AmbTipKljuc` (`UCase$(Trim$())`) koji koriste OBE putanje. Pouka: kad se uvodi matcher za nesto sto je vec grupisano, normalizacija mora da bude **isti simbol**, ne „ekvivalentna" logika. **(7) Review dopuna 2 (P1, deterministicki).** Kanonizacija tipa iz (6c) nije bila dovoljna: `ReversRedPripada` definise identitet kao `DokumentID + DokumentTip + TipAmbalaze`, a `ReportAmbalazePojedinacni` je grupisao samo po `DokumentID + TipAmbalaze` — nedostajao `DokumentTip` (zateceno stanje, ne regresija RF-07, ali putanje su ostale nesaglasne bas na tacki koju (6c) tvrdi da je izjednacila). Sudar je na NORMALNOJ putanji: `modOtkup.SaveOtkup` upisuje **isti `otkupID` i isti `tipAmb`** pod dva tipa dokumenta — pune gajbe primljene na OM kao `DOK_TIP_OTKUP` (`modOtkup.bas:600/603`), prazne izdate kooperantu kao `DOK_TIP_OM_IZLAZ_KOOP` (`:613/616`; komentar uz kod izricito kaze „Isti DokumentID (otkupID) -> storno otkupa hvata i ovu nogu"). Otkup 20 primljenih / 8 izdatih gajbica je davao JEDAN red pregleda sa tipom PRVOG zapisa (`Otkup`) i ref-kljucem `AMB|Otkup|<id>` -> `m_btnStampajAmb_Click` je rutirao na `ReprintOtkupniListByOtkupID`, pa revers `OM-Izlaz-Koop` **nije imao red i nije se mogao odstampati iz pregleda**. Kljuc je sada pun (`Trim$(dokTipv) & "|" & Trim$(dokIDv) & "|" & AmbTipKljuc(tipv)`); dva dokumenta = dva reda, svaki sa svojim ref-kljucem i svojom rutom stampe, UKUPNO nepromenjen. Gate: `T_E2E_AmbPregledRazdvajaTipDokumenta` (`tblAmbalaza` dodata u snapshot E2E transakcije). **Pouka:** kad se matcher i grupisanje izjednacavaju, mora se izjednaciti CEO kljuc, ne samo normalizacija njegovih delova — inace „isti kljuc" ostane tvrdnja u komentaru, a ne svojstvo koda.| Yes — AR §19 (AUD status) uz sledeći snapshot | **bez migracije**; UI/read sloj; `.frx` nedirnut i **bez novih form `WithEvents`**; izvori **ASCII-only**; **7 novih `Poruka()` ključeva** (`RPT_MSG_NIJE_OSVEZENO`, `RPT_MSG_PRVO_PRIKAZI`, `RPT_MSG_NEMA_DOSTUPNOG_IZVESTAJA`, `RPT_MSG_IZVESTAJ_NIJE_GENERISAN`, `RPT_ERR_GENERISANJE_IZVESTAJA`, `RPT_MSG_REVERS_VISE_NOGU`, `RPT_MSG_NASTAVITI_STAMPU`) -> posle importa **obavezno `EnsurePoruke`**; re-import `frmIzvestaj`(+`.frx`)/`modIzvestaj`/`modKarticaDetalji`/`modPoruke`/`modIzvestajTests`. Namerna vidljiva promena: revers za dokument sa više tipova ambalaže daje **manje** gajbica po reversu nego ranije. Gates: `RunIzvestajTests` (tvrd gate) proširen sa `T_TabMatrica` (fiksira i **postojeći pojedinačni režim**, da se meni ne suzi slučajno), `T_EntitetKod`, `T_ReversKljucPoTipu`; freshness/CleanFail/tab-meni su **operater-smoke** (UI stanje, ne računska funkcija). **Ostaje otvoreno:** FM-0029 #11 (status broji UKUPNO redove), #17 (revers bez datuma -> `Date`), #26 (redosled perioda), #9 (prazan `entitetID`); AUD-027 reprint storniranog otkupa + `FillFakturaSablon` `.UnMerge` -> RF-08 |
| v6.43 | 2026-08-06 | **RF-06 / AUD-023 — ispravnost brojki u izveštajima** (`modIzvestaj`, `modNovac`, `modHelpers`, `frmIzvestaj` prikaz). Read-model sloj, bez izmene šeme i bez ijednog novog upisa. **(1) Atribucija isplata po stanici:** `ReportSaldoOM` je isplate kooperanta vezivao za **matičnu** stanicu iz `tblKooperanti`, a ne za stanicu na kojoj je isplata izvršena — kooperant aktivan na dva OM-a je imao isplatu na pogrešnom izveštaju. Sada odlučuje `COL_NOV_OM_ID` **samog `tblNovac` reda** (isti ključ koji `ReportIsplata("OM")` već koristi → dva izveštaja se slažu); red bez `OMID`-a pada na matičnu stanicu (nepoznata istorijska stanica, alternativa bi bila da novac nestane iz svih izveštaja). Seam: `NovacRedPripadaStanici`. **(2) Kartice od početnog stanja:** `ReportKarticaKooperanta`/`ReportKarticaAmbalaze` su running saldo računali od **nule** nad period-redovima, pa je kolona „Saldo" bila neto promena perioda. Promet pre `datumOd` se sada sabira u red `IZV_POCETNO_STANJE`, kolona Saldo je završni saldo, UKUPNO zaduženje/razduženje ostaje promet perioda. Seam-ovi: `KarticaRezultatSaPocetnim`, `KarticaAmbRezultatSaPocetnim` (koriste ih i oba PDF-a i PWA `ExportKarticeToGoogle_Core` — dodatni red prolazi generički). **(3) „nema prijema":** nedostajuća prijemnica se prikazivala kao `0 kg / 0,00%` u `ReportOtkupRobaOM` i kao `100%` u `ReportManjak`. Oba sada emituju prazne brojke + oznaku `IZV_NEMA_PRIJEMA` kroz deljeni `ManjakStavka`, i takvi redovi su isključeni iz UKUPNO manjka i iz osnovice procenta. **Uz to (review nalaz):** join zbirna-prijemnica je išao po samom `BrojZbirne`, a AUD-052/RF-05 je već utvrdio da poslovni broj nije identitet - dve aktivne zbirne istog broja (različit vozač/kupac) su sabirale tuđu prijemnu količinu, pa su prijem, manjak i procenat bili pogrešni na obe putanje; `ReportManjak` je istu grešku imao i u sopstvenoj, dupliranoj agregaciji prijemnica. `modHelpers.BuildManjakDict` je prepisan na **stavka-scoped** model: `ZbirnaVlasnikKljuc` = `broj|vozac|kupac` (**ista definicija vlasnika koju koristi `modStorno.RequireJedanVlasnikPoBroju` - nije uvedena druga**), a `ZbirnaStavkaKljuc` dodaje **`Klasa`**. Klasa je obavezna jer `modAutoHladnjaca` vodi Klasu I i II kroz ceo lanac odvojeno (zasebna otpremnica/zbirna/prijemnica) uz isti broj, vozača i kupca: bez nje se `zbirnaTotal`/`prijTotal` obe klase saberu pa se taj zbir u malina modu dodeli **svakoj** otpremnici (1000/900 + 200/150 -> obe klase vide prijem 1050, UKUPNO 2100 umesto 1050). Granularnost `(BrojZbirne, Klasa)` je ista koju `modAutoHladnjaca.KeyZbrKlasa` već koristi za idempotenciju backfill-a, pa nije uveden nov pojam; `ClassOrDefault` je delegiran na novi deljeni `modHelpers.KlasaOrDefault` (jedno pravilo za praznu Klasu = I), uz indeks `#V|` (broj različitih vlasnika po broju), `#1|` (jedinstveni vlasnik), `#O|` (razrešenje po vozaču - otpremnica nema `KupacID`), `#N|` (prijemnice bez kompletnog vlasnika) i `#C|`/`#K|` (agregat po samom broju). `ReportManjak` više ne duplira agregaciju i namerno zadržava **jedan red po dokumentu** (grupiše po vlasniku, kolona 1 i dalje prikazuje `BrojZbirne`), ali prijem **sabira po klasama koje red obuhvata** - prijemnice postoje po klasi. `ReportOtkupRobaOM` čita `Klasa` otpremnice, komponuje stavka-ključ i radi srazmernu raspodelu **unutar te klase** (osnovica = kg te klase). `#V|` i dalje broji **vlasnike** (vozač+kupac, bez klase), pa dve klase istog dokumenta ne obaraju fail-closed granu. Nova čista odluka `PrijemZaZbirnu` nosi **fail-closed** pravilo: broj sa više vlasnika + nerazrešen vlasnik reda **ili** postojanje nepripisive prijemnice -> oznaka `IZV_VLASNIK_NEJASAN`, bez brojke i van UKUPNO. Broj sa tačno jednim vlasnikom koristi agregat po broju, pa starije prijemnice bez popunjenog vlasnika ne regresiraju. **(4) Uplata kupca srazmerno po vrsti:** `modNovac` je za fakturu pamtio samo vrstu **prve** stavke (`BuildVrstaFakturaCache`), pa je cela uplata išla na tu vrstu. Kes je prerastao u `BuildFakturaVrstaUdeoCache` (`FakturaID → Dictionary(vrsta → Kolicina*Cena)`), a podelu radi nova čista `RaspodeliPoUdelima`. **Zaokruživanje je finansijsko:** raspodela se računa u **celim parama metodom najvećih ostataka** (largest remainder) - svaki ključ dobije `Int` svog idealnog udela, pa višak para ide redom na najveći ostatak. Metod drži **obe** invarijante odjednom: (a) zbir delova == `ZaokruziNovac(iznos)` - bez toga bi 100/3 dalo interne delove 33,3333 koje forma prikazuje kao 33,33 -> zbir prikazanih vrsta 99,99 uz UKUPNO 100,00; (b) **nijedan deo nije negativan** - raniji oblik je poslednjem ključu davao ostatak *posle* zaokruživanja, pa kad prethodni delovi pređu cilj poslednji ode u minus (0,03 na 5 jednakih vrsta -> poslednja vrsta -0,01). Clamp na nulu je odbačen jer bi razbio invarijantu (a). Zaokruživanje je `ZaokruziNovac` (half-up; VBA `Round` je banker's i na novcu daje sistematsko odstupanje). Pare se drže u `Double`, ne `Long` - `Long` bi pukao Overflow-om preko ~21,4 miliona. Vidljiva promena: kod jednakih udela višak pare dobija **prvi** ključ umesto poslednjeg. Prazne/nulte težine -> `(Nepoznato)`, kao i pre. **(5) Eksplicitan dispatch:** `ReportProsecnaCena`/`ReportManjak`/`ReportAmbalaza`/`ReportOtkupRoba` dobili `Select Case` sa `Case Else → Empty` — zbirni mod za Kooperante/Vozače (tabovi 5/6/7 su vidljivi svim tipovima) više ne dobija globalni izveštaj pod tuđim naslovom. **(6)** `AGROHEMIJA (nerasporedjena)` ne ulazi u UKUPNO stanice (`tblMagacin` nema kolonu stanice → isti iznos se ponavljao u svakoj stanici i zbir po stanicama ga je višestruko brojao); red ostaje informativan, labela nosi „van UKUPNO". | Yes — AR §19 (AUD status) uz sledeći snapshot | **bez migracije**; read-model/prikaz; `.frx` nedirnut (`frmIzvestaj` samo kod prikaza oznake); izvori **ASCII-only**, bez novih `Poruka()` ključeva → ne treba `EnsurePoruke`; re-import `modHelpers`/`modIzvestaj`/`modNovac`/`modAutoHladnjaca`/`frmIzvestaj`/`modIzvestajTests`. **Namerne razlike u brojkama** posle importa: kartice (početno stanje), Manjak i RobaOM (UKUPNO bez redova bez prijema), saldo kupca po vrsti, saldo OM po stanici. Gates: nova assert suite `RunIzvestajTests` - **tvrd gate** (`Err.Raise` na pali assert i na internu grešku, po konvenciji uvedenoj u RF-14 za `RunSheetsJsonParserTests`/`RunMasterSyncSmokeSuite`; ranije je suite samo štampao FAIL, pa je automatizovani pozivalac mogao da ga prijavi kao PASS). Pokriva čiste seam-ove (deterministično, bez tabela) **i tri end-to-end testa nad tabelama** (dve zbirne istog broja / dva kupca / različite prijemne količine, iz ugla `ReportManjak` i `ReportOtkupRobaOM`; Klasa I+II istog dokumenta 1000/900 i 200/150 - svaka klasa dobija svoj prijem, UKUPNO 1050 a ne 2x1050, Manjak jednim redom 1200/1050/150) u `clsTransaction` sa snapshot-om i obaveznim rollback-om. Novčano zaokruživanje se proverava na nivou centa (`IzvChkEqC`) - tolerancija 0,01 bi propustila nezaokruženo 33,3333. Uz to `SmokeTest_modIzvestaj` (shape nad živim podacima) + operater uporedni pregled pre/posle. **Ostaje otvoreno:** vidljiva poruka za nevalidnu kombinaciju i sužavanje tab-matrice (RF-07); labela „OM AVANS" i header-i „Amb. (trenutno stanje)" (FM-0028 #2/#11 — UI paket); AUD-013 `MatchesFilter` `Case Else` (dokazano nedostižan iz report sloja, v. `KNOWN_ISSUES`) |
| v6.41 | 2026-06-30 | Agrohemija (`frmAgrohemija`/`modAgrohemija`/`modConfig`). **(1) Parcele-opciono:** kad je `PRACENJE_PARCELA` OFF (`IsPracenjeParcela()`, reuse iz `frmOtkup`), izdavanje robe se prihvata **bez parcele** — `UpdatePreporuka` smart-doza se preskače, validacija „Izaberite parcelu!" u `btnDodajIzlaz` je gejtovana na ON, `lstParcele` disabled (`ApplyAgroTogglesState`). `cmbKooperant_Change` restruktuiran: dug se sada prikazuje **i za kooperanta bez parcela** (ranije rani `Exit Sub` na praznoj listi) — namerno. **(2) Početni dug (migracija):** runtime dugme „Pocetni dug" (form-local `WithEvents`, `Controls.Add`, `.frx` se ne dira) + `BookPocetniDug(koop, iznos, brojDok, datum)` knjiže jednu `MAG_IZLAZ` stavku `Vrednost=iznos` (`overrideCena=iznos`, `kolicina=1`) na rezervisani virtuelni artikal `ART_POCETNI_DUG`, u `clsTransaction`, sa novim `SaveMagacin(... allowNoStock:=True)` (preskače stock-check; postojeći pozivi default `False`). Artikal se lazy-seed-uje (`EnsureArtikalPocetniDug`), izuzet iz `LoadArtikli`/`LoadArtikliUlaz` i iz `GetMagacinStanje` (nema fantomskog negativnog stanja). Dug konzistentan u `frmAgrohemija`, `GetAgrohemijaDug`, `ReportSaldoOM`/`ReportIzdavanjePoKooperantu`; reverzibilno preko storna. | Yes — AR §5.8, §13.1, §13.2 | bez migracije; re-import `frmAgrohemija`/`modAgrohemija`/`modConfig`; izvori ASCII-only, bez novih `Poruka()` ključeva → ne treba `EnsurePoruke`; **known limitation:** PWA `ExportMagacinKoop` još ne filtrira `ART_POCETNI_DUG` (KI-006 / ROADMAP); testovi `BookPocetniDug` u backlogu (ROADMAP) |
