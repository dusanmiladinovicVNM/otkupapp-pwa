...
|---|---|---|---|---|
| v6.43 | 2026-08-02 | **Pregled dokumenata (read-only).** Storno-pregled motor u `modDokumenta` generalizovan (`GetDokumentiByTip` sa `samoStornirani` + opsegom datuma); `GetStorniraniByTip` ostaje kao omotač → **bez promene ponašanja postojećeg „Pregled storniranih"**. Novi `GetAktivniByTip`/`GetAktivniGrupisano` daju aktivne dokumente u istih 12 kolona. `frmDokumenta` dobija overlay panel „Pregled dokumenata" (otkupni listovi, otpremnice, zbirne, prijemnice, fakture, keš/banka uplate i isplate, 4× revers) sa filterom tipa i opsega datuma; ulaz = novo dugme u frame-u „Lista zbirnih". Izvodi: `GetBankaImportRows` + filter traka (`Otvorene`/`Obradjene`/`Preskocene`/`Sve`) u `frmBankaImport` — do sada su se videle samo otvorene stavke. Palete ostaju u `frmPalete`, fakture u `frmFakturisanje`. | No (uz sledeći snapshot) | read-only, bez migracije i bez upisa u tabele; re-import `modDokumenta`, `modBankaMapiranje`, `frmDokumenta`(+`.frx`), `frmBankaImport`(+`.frx`); izvori ASCII-only, bez novih `Poruka()` ključeva → ne treba `EnsurePoruke`; **self-update:** `frmDokumenta` je i pre bila „tvrda" (zatečeni `m_tglIzdKoop As MSForms.ToggleButton`) pa ostaje **reinstall-only** (bootstrap `ImportAllVBA`/nov `.xlsm`), a `frmBankaImport` je bila **„meka" (self-updatable) i namerno je takva zadržana** — njene runtime kontrole su deklarisane `As Object`, NE `As MSForms.*`, jer `IsHardModuleBody` module-level `" AS MSFORMS."` rutira celu formu na reinstall (zamka #20); backward-compat pri skew-u modula: stara `frmDokumenta` zove `GetStorniraniGrupisano`/`StorniraniHeaders`, stara `frmBankaImport` zove `GetBankaImportOpen` — svi zadržani kao Public sa nepromenjenim potpisom; **known limitation:** fakture se u `frmFakturisanje` i dalje biraju kroz `cmbFaktura` (nema zasebne tabele faktura); lista faktura je dostupna kroz tip „Faktura" u novom panelu |
