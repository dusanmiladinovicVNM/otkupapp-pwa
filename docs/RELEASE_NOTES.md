...
- **Regresioni test (`modTestStorno`, `Alt+F8 → RunStornoTestSuite`):** 11 scenarija nad **pravim** tabelama u jednoj transakciji koja se **uvek poništava** (test-identitet `SVT-*`): rekalkulacija zbirne posle storna otpremnice, validacija obe zbirne pri prevezivanju, storno zbirne bez mismatch-a, ispravka zbirne prevezuje otpremnice+prijemnicu, paletne stavke dobijaju novu zbirnu, revers ispravka ne duplira saldo, revers poništenje uklanja saldo, pending ostaje vidljiv na fail, smart-trigger gate, i T11 = pun scenario ispravke otpremnice (prevezuje prijemnicu/palete, stara zbirna stornirana, context COMPLETED samo na uspeh).
