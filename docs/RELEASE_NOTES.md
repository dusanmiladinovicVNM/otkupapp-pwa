...
- **Podešavanja — do 3 računa firme za isplate (zasebna polja):** u grupi „Banka / nalozi" je jedno polje `BANKA_NALOG_RACUNI` (više računa u jednom polju, odvojeno `;`) zamenjeno **trima zasebnim textbox-ovima** — „Račun firme 1 / 2 / 3 (isplate)" — radi preglednijeg unosa. Combo **„Sa računa"** u „Banke platni nalozi" (`frmBankaExportPregled`) i dalje nudi sve unete račune (uz naziv banke po NBS prefiksu), preskačući prazna polja; ako su sva tri prazna pada na stari `;`-spisak, pa na `SELLER_ACCOUNT` (**kompatibilno unazad**). Postojeća `;`-vrednost se pri prvom otvaranju Podešavanja **automatski razbije** u tri polja (jednokratna migracija). Bez izmena na dokumentima/SEF/PWA — glavni račun ostaje `SELLER_ACCOUNT`.
