...
- **Jasna razlika DUPLI vs PONIŠTENJE:** **DUPLI/FANTOM** (višak zapisa, roba realna) tiho počisti i **oslobodi delove** (blokovi za prevezivanje, otpremnice u „čeka zbirnu") — bez pitanja. **PONIŠTENJE** (ceo tok otpada) **uvek prvo prikaže pun spisak zavisnih dokumenata** (zbirna/prijemnice/palete/blokovi) i traži svesnu potvrdu pre nego što bilo šta uradi — i kad nema lanca.
- **Regresioni test (`modTestStorno`, `Alt+F8 → RunStornoTestSuite`):** 15 scenarija nad **pravim** tabelama u jednoj transakciji koja se **uvek poništava** (test-identitet `SVT-*`): rekalkulacija zbirne posle storna otpremnice, validacija obe zbirne pri prevezivanju, storno zbirne bez mismatch-a, ispravka zbirne prevezuje otpremnice+prijemnicu, paletne stavke dobijaju novu zbirnu, revers ispravka ne duplira saldo, revers poništenje uklanja saldo, pending ostaje vidljiv na fail, smart-trigger gate, T11 = pun scenario ispravke otpremnice (prevezuje prijemnicu/palete, stara zbirna stornirana, context COMPLETED samo na uspeh), T12–T13 = revers complete traži aktivan novi revers (odbija → `MANUAL_REQUIRED` / COMPLETED + saldo samo novi), T14 = safe-stop pri više otvorenih ispravki, T15 = PONIŠTENJE uvek traži svesnu potvrdu (spisak posledica) i bez zavisnih.
...
- **Dodirnuti moduli:** `modDokumentInvariant` (nov), `modStornoContext` (nov), `modStornoFlow` (nov), `modTestStorno` (nov), `frmDokumenta` (storno UX + auto-dovršetak), `modConfig` + `modSetup` (`tblStornoVeze` šema). Reuse: `modStorno` (storno jezgra), `modDokumenta` (`ReassignOtkupToOtpremnica_TX`/`ReassignPrijemnicaToZbirna_TX`), `modAmbalaza` (saldo). `.frx` netaknut; ASCII-only; bez novih `Poruka()` ključeva.
