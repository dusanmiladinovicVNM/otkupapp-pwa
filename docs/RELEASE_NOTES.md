...
- **PIN hashing (opciono):** `Alt+F8 → EnablePinHash` čuva PIN kao SHA-256 heš (sa self-testom pre uključenja); postojeći „goli" PIN-ovi se transparentno migriraju na heš pri prvoj prijavi. Isključenje (`DisablePinHash`) ne kvari već hešovane PIN-ove.
