...
### 8.8 RF-03 post-merge review — follow-ups (PR #167, 2026-08-04)
...
| AUD-052 | P1 | Storno/ispravka i dalje polaze od **poslovnog broja**, ne od identiteta dokumenta. `RunPrijemnicaCorrection`/`RunZbirnaCorrection` primaju samo `broj`; `ScanPrijemnica` bira jedan PK preko number-only `LookupActiveID`; `StornoPrijemnicaByBroj_TX` / `StornoOtpremnicaBrojAtomic_TX` / `StornoZbirna` skupljaju **sve** aktivne redove tog broja. Broj nije globalno jedinstven (`GenerateBrojPrijemnice` racuna sekvencu po kupcu, x-deo je fiksno „1"), pa dva kupca istog dana dele `1/ddmmyy`. **Mitigacija u RF-05:** `RequireJedanVlasnikPoBroju` (modStorno, Public, fail-closed) odbija storno kad aktivni redovi broja pripadaju vise od jednog vlasnika — greska unutar transakcije, rollback, nijedan red se ne menja; ukljucen u sve tri number-only mutacije + atomic otpremnicu, pokriva SIMPLE/ISPRAVKA/DUPLI i kaskade. Test: `Test_StornoPoBrojuOdbijaDvaVlasnika`, `Test_StornoGuardNaSvimPutanjama`. **Ostaje (RF-06+):** puni identitetski storno — `OldDocID → GeneracijaID → svi redovi te generacije` kroz `Scan*`/`Run*Correction`, cime broj prestaje da bude ulaz storna; tada se guard moze relaksirati u obicnu proveru. | `modStornoFlow.bas` (Scan*/Run*Correction, `StornoOtpremnicaBrojAtomic_TX`), `modStorno.bas` (`StornoZbirna`, `Storno*ByBroj_TX`, `RequireJedanVlasnikPoBroju`) |
