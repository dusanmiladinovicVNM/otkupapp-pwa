...
| AUD-049 | P1 | **Storno celog izvoda ne postoji** — a pojedinačni storno izvodnog reda je od RF-03 zabranjen (`modStorno.ResolveNovacForStorno` / `StornoNovac_TX`), pa pogrešno mapiran red iz izvoda trenutno **nema putanju ispravke**. `modBankaMapiranje` ima samo `Map*`/`AutoMap*`/`Skip*` — nema unmap/revert; `COL_BIM_STORNIRANO` se nigde ne upisuje, a `Obradjeno="Da"` trajno zaključava staging red (`ValidateBankaImportNotProcessed`). **Odlučeno (vlasnik, ovaj tred):** izvod se stornira samo u celosti; PDF se NE uvozi ponovo → storno vraća staging redove u „za obradu" (`Obradjeno` → prazno), izvod ostaje uvezen. Identitet grupe = `BrojDokumenta` (banka uvek daje broj; `"IZVOD"` fallback uklonjen — `RequireIzvodBroj`). **Prepreka za implementaciju:** `ApplyAvansToFaktura`/`ApplyAvansToOtkup` destruktivno umanjuju `Uplata`/`Isplata` originalnog avans reda i prave split red, pa storno izvoda mora vratiti i te iznose — inače saldo ostaje pogrešan. Zaseban paket sa sopstvenim testovima. | `modBankaMapiranje.bas:1590-1650`, `modNovac.bas:640-675, 1210-1245`, `modStorno.bas` (ResolveNovacForStorno) |
