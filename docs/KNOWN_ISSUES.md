...
| AUD-049 | P2 | Agrohemija referential integrity — deferred split from RF-27/AUD-040. `ValidateMagacinInput` now verifies **artikal exists** and (for izlaz) **kooperant exists** via `FindRows`, but the **parcela↔kooperant** link is still unchecked. Deferred out of the RF-27 minimal-delta because it needs a signature change (pass `parcelaID` into the validator), `;`-list parsing (a magacin row can carry multiple `parcelaID`s), and the `PRACENJE_PARCELA` OFF branch where izlaz is parcela-optional. Fix: extend `ValidateMagacinInput` with an optional `parcelaID`, split on `;`, verify each `LookupValue(TBL_PARCELE, COL_PAR_ID, …, COL_PAR_KOOP)=kooperantID`, and skip the check when parcela tracking is off. | `modAgrohemija.bas:593-624` (ValidateMagacinInput), `frmAgrohemija.frm:537-543` (`;`-join) |
