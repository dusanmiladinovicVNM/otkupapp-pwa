...
| M6 SEF UX + banka | 🟡 1/3 | — | 2026-08 | RF-22 🟢 grana `claude/rf-22-sef-ux-lifecycle-kzzzvn` (AUD-032 zatvoren u celosti — ishod slanja, `UNKNOWN_STATUS`, pošten refresh/recovery, reset combo-a, dev makroi van Alt+F8, batch sažetak). Uz to: `SEF_UNKNOWN` je dobio izlazne tranzicije (bio slepo crevo) i **nov `RunSEFTestSuite`** — offline tvrd gate koji je §5C referencirao, a nije postojao. Review runda 2 zatvorila još tri P1: adapter za zvanični `SalesInvoiceStatus` (`Approved` ≠ `Accepted`), planer tranzicije koji pita state machine (ponovni pad SEF-a je pucao na zabranjenu tranziciju), i fail-open provera recovery-ja → whitelist. **7 novih `Poruka()` ključeva → `EnsurePoruke` obavezan.** Ostaju RF-09 (P0 AUD-007) i RF-10 |
