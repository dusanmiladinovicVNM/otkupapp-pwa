...
| M6 SEF UX + banka | ✅ gotov | v2.39.0 | 2026-08 | RF-22 🟢 grana `claude/rf-22-sef-ux-lifecycle-kzzzvn` (AUD-032 zatvoren u celosti — ishod slanja, `UNKNOWN_STATUS`, pošten refresh/recovery, reset combo-a, dev makroi van Alt+F8, batch sažetak). Uz to: `SEF_UNKNOWN` je dobio izlazne tranzicije (bio slepo crevo) i **nov `RunSEFTestSuite`** — offline tvrd gate koji je §5C referencirao, a nije postojao. Review runda 2 zatvorila još tri P1: adapter za zvanični `SalesInvoiceStatus` (`Approved` ≠ `Accepted`), planer tranzicije koji pita state machine (ponovni pad SEF-a je pucao na zabranjenu tranziciju), i fail-open provera recovery-ja → whitelist. Runda 3 još dva P1: `MISTAKE` (greška pri slanju) više ne postaje lokalno „poslato" nego `SEF_TECH_FAILED` uz retry/cancel, a `Paid/OverDue/Archived` više ne ostavljaju fakturu zaglavljenu u `SEF_SENDING`. Runda 4: `SEF_SENT → SEF_TECH_FAILED` (normalna `MISTAKE` sekvenca) + `CanSendSEFInvoice` kao zajednička kapija forme i validatora — putanja za `MISTAKE` je Cancel + ručna provera, uz jednu otvorenu odluku za vlasnika (retry istog `requestId` treba potvrditi na demo SEF-u). **7 novih `Poruka()` ključeva → `EnsurePoruke` obavezan.** RF-09 🟢 grana `claude/rf-09-banka-import-mapiranje-87nv8k` (AUD-014 + AUD-025 zatvoreni, **P0 AUD-007 preuzet ovde** — datum-parse round-trip + Level 0 kapija datuma u uvozu izvoda; dedupe ključ + broj računa; 3+ kandidata → tipizirana greška koja obara samo taj red; smer guard u `Map*`; `_Activate` više ne knjiži nego nudi dugme „Mapiraj jake ključeve (N)"; ručni Kupac dobio izbor fakture + upozorenje na avans; preview i komanda čitaju isti izvor bloka). **Nov `RunBankaImportTestSuite`.** RF-10 🟢 grana `claude/rf-10-banka-export-uo4pg8` — **AUD-026 zatvoren, milestone kompletan.** Per-blok „Isplatiti" override je preživljavao reload bez ijednog poređenja sa iznosom, pa je posle delimične isplate / vezanog avansa / storna mogao biti veći od otvorenog i takav ući u CSV nalog; klamp je izmešten u nov seam `ClampOverridesToOpen` (nestao ili zatvoren blok → brisanje, veći → spuštanje na otvoreno, **manji ostaje** jer je delimična isplata namerna), a broj usklađenih unosa ide u statusnu liniju. `GenerisiNalogeCSV` je pred upis proveravao samo TR i iznos > 0 — sada čita **svež** saldo i kroz `ValidateNalogSaldo` odbija ceo fajl kad ijedan nalog prelazi otvoreno (poruka imenuje blok: traži vs otvoreno), fail-closed i za blok kog više nema (otvoreno = 0) i za grešku pri očitavanju. Uz to batch „Primeni avans (sel.)" više ne broji no-op `True` kao primenjen avans — čita stvarni iznos iz `ByRef` parametra koji je **RF-02 već uveo** (reuse). Dve nove grupe u postojećem `RunBankaImportTestSuite` (T12 klamp, T13 odbijanje preplate), oba čisti seam testovi bez tabela. **Bez novih `Poruka()` ključeva** — `EnsurePoruke` nije potreban. |
