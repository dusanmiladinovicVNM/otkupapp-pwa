...
| 10 | Nema jednog entry pointa za punu šemu | Srednji | **Tačno** — SetupNewPC ne zove SetupPaletniListSchema/Dorade/Korisnici/Audit; redosled je na operateru | P2 | Javni `EnsureFullSchema` koji idempotentno poziva sve Ensure* redom | S |
...
| 5 | Positional insert | Visok | **Tačno** — :50-68 `rowData(0 To 17)`; redosled vezan za `EnsureStornoVezeSchema` | P2 | Column-mapped upis za recovery ledger | M |
...
| 89.17 | Schema nije u startup self-heal-u | Rizik | **Tačno** (EnsureRuntimeSchema bez Cenovnika; EnsureCenovnikSchema ručno / SetupPaletniListSchema modSetup.bas:959 / AdminEnsureEverything modAdmin.bas:261) | P3 | Dodati `EnsureCenovnikSchema` (bez MsgBox varijantu) u EnsureRuntimeSchema | S |
...
| 7 | Schema ensure pre korisničke potvrde (118.12) | — | **Tačno** — `EnsureStornoVezeSchema` `:42` pre MsgBox `:49` | P3 | Pomeri ensure posle potvrde | S |
| 8 | Nema `tblStornoZurnal` precondition (118.13) | — | **Tačno** — samo `EnsureStornoVezeSchema`; revers testovi zavise od zurnal šeme | P3 | Dodaj `EnsureStornoZurnalSchema` u preflight | S |
...
| 15 | Hidden schema prerequisite `EnsureRuntimeSchema` (120.19) | — | **Tačno** — header `:12-13` traži ručno; runner ne proverava; journal testovi sami zovu `EnsureStornoZurnalSchema :75` → nedosledno | P3 | Jedinstven preflight svih šema | S |
