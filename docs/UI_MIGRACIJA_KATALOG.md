...
| Broj novca **nije** u zajedničkom prostoru sa reversom (od 14.09.2026) | `DuplBroj` | F5/F6 **nemaju** proveru duplikata broja — broj novca nije jedinstven (izvod, split; `ARCHITECTURE_CONTRACT.md` A2 red NOV). F7 revers proverava zauzetost po nizu (stanica, dan), sa storniranima — `modBrojevi.BrojZauzetUNizu` (`KIND_REV`), ista provera kao u piscu `SaveOMUlaz_TX`; `DuplBroj` je obrisan. KOOP smerovi nemaju dodatnu proveru: isti (broj, smer, dan) na drugoj stanici je legalan od REV-IDENT-01 Faze 2b (15.09.2026) — noge jednog reversa povezuje `ReversID`; ranija provera `ReversKoopBrojDrugde` je uklonjena, a potvrde storna i „Vrati storno" imenuju stanicu i dan |
...
| Revers: **identitet reda** → `ReversID` dokumenta | `IdKolonaTipa("REVERSI")` = `AmbID` → `StornoRazlog` / `StornoOMKoopByBrDok` → `ReversIDRazresi` | četiri smera dele `KIND_REV`, a broj je jedinstven tek u nizu (stanica, dan) — ni broj ni broj + smer ne kažu koji je dokument. Smer se čita iz kliknutog reda (od 14.09.2026; ranije je ekran uzimao „prvi od četiri tipa sa aktivnim redom pod brojem", što je davalo i tuđi smer), a dokument je `ReversID` koji taj red nosi — i noga Kooperant (REV-IDENT-01 Faza 2a, 15.09.2026) |
...
| 8 **(isporučen, §27.18)** | `frmSplash`, `frmLogin`, `frmExcelMini` → faze ljuske. **Nije bio deo plana** (§27.2 ih je vodio kao „OSTAJU") i ne drži ga nijedan raniji korak: to nisu legacy forme nego forme ljuske. Posle njega ostaje **jedna forma** | `modMain.StartApp` (`frmSplash.Show`), `modAuth.PrikaziPrijavu` (`frmLogin.LoginOK` / `.Show`), `modOtkupUI.DoShowExcel` (`frmExcelMini.Show`) |
...
kupcem** — dakle **dvoklasna zbirna je JEDAN dokument na DVA reda**, sa istim
`GeneracijaID`.

**Zbog te razlike je de-duplikacija odbijena, ali i sam picker ostao nedovršen:**

- `GetAktivneZbirne` de-duplikuje po **samom broju**. To je pogrešan ključ: spojio
  bi i dva **različita** dokumenta koja anomalno dele broj, a zbir kg bi ih
  prikazao kao jedan dokument koji ne postoji. Zato **nije diran** i ostaje bez
  produkcionog pozivaoca.
- Ispravna de-duplikacija bi grupisala po **logičkom dokumentu**, ne po broju.
  Za to picker mora da nosi identitet — a nosi samo broj.
...
> obe stavke prikazuju isti tekst i daju polju istu vrednost, a writer prima samo
> broj. Izbor između dve identične stavke ne nosi identitet nigde dalje. Tvrdnja
> u testu i sabotaža koje su to zaključavale su **uklonjene**, jer bi buduću
> ispravnu de-duplikaciju proglasile regresijom.

**Verifikacija**

| Šta | Gde |
|---|---|
| Stornirana zbirna nije u ponudi, a aktivna jeste (filtar ne sme da pojede sve) | `modTest` `T_Zbirne_PickerNeNudiStornirane` (bez ijednog upisa, nad fixture-om; preduslovi se tvrde, pa test ne može tiho da prođe kao prazan) |
| Sabotaža | `zbirne-picker-nudi-stornirane` |
