...
- Bookings are **entity-relative**: `Smer` is from the row entity's view — `Ulaz` = crates into that entity, `Izlaz` = out. So otpremnica = `Stanica` `Izlaz`, otkup = `Kooperant` `Izlaz`, prijemnica = `Kupac` `Ulaz` (full received from the zbirna) / `Izlaz` (empties returned), izlaz-kupci = `Kupac` `Izlaz`, OM-ulaz = `Stanica` `Ulaz`, OM-izdavanje-kooperantu = `Kooperant` `Ulaz` (`DOK_TIP_OM_IZLAZ_KOOP` — OM hands empties to a kooperant; no vozač). The ledger is single-entry (one row per document on its primary entity).
