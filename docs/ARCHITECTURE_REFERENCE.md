...
- Bookings are **entity-relative**: `Smer` is from the row entity's view — `Ulaz` = crates into that entity, `Izlaz` = out. So otpremnica = `Stanica` `Izlaz`, otkup = `Kooperant` `Izlaz`, prijemnica = `Kupac` `Ulaz` (full received from the zbirna) / `Izlaz` (empties returned), izlaz-kupci = `Kupac` `Izlaz`, OM-ulaz = `Stanica` `Ulaz`, OM-izdavanje-kooperantu = `Kooperant` `Ulaz` **+ `Stanica` `Izlaz`** (`DOK_TIP_OM_IZLAZ_KOOP` — kooperant receives empties, OM is discharged; no vozač). The ledger is single-entry (one row per document on its primary entity) **except OM-izdavanje-kooperantu, which books both legs** (two real entities move: OM and kooperant — neither is derivable from the other's row, so a second row is required to discharge the OM).
