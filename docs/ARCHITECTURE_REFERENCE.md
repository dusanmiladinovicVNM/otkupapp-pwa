...
- after initialization, `StartApp` may hide Excel, show the splash **phase** of the shell (`modUiFaze.FazaBoot`), and hand off to the shell `frmOtkupUI` through `modOtkupUI.ShowOtkupUI`. **`frmOtkupAPP` no longer exists** — it was deleted in UI migration step 7 (`docs/UI_MIGRACIJA_KATALOG.md` §27.17), together with `frmSEF`. Since §27.18 there is **exactly one form**: splash, login and the "Excel is open" card are phases of `frmOtkupUI` (`modUiFaze`), not separate forms;
...
The printed/PDF documents (otkupni list, prijemnica, paletni list, preradni list) share one presentation style via `modDocStyle` and render onto generated worksheet templates. Templates hold no business data; they are filled from canonical tables on each print and are safe to delete/regenerate.
...
| `PrijemnicaSablon` | `FillPrijemnicaSablon` | `modPrint` | Prijemnica, full-A4 receiving document (stavke per class), redrawn each print; output via `OutputPrijemnica` per `PRIJEMNICA_PRINT_MODE` (PDF/PRINT/PREVIEW/OFF) |
...
- `DocResolveMode(mode, defMode)` — normalizes a `*_PRINT_MODE` config value to `OFF` / `PRINT` / `PREVIEW` / `PDF`; empty or unrecognized → `defMode` (the document's default: `PDF` for otkupni/grupni/izdamb/kartica/sledljivost, `OFF` for prijemnica/paletni, `PRINT` for faktura). Centralizes the per-document default that used to be implicit in each dispatcher's `Case Else`.
- `DocPrintWs(ws, mode)` — print or preview an already-filled sheet (`PREVIEW` → `PrintPreview`, else `PrintOut Copies:=1`). Lets every `Output*` dispatcher merge its `PRINT`/`PREVIEW` branches into one fill. The `PDF` branch still delegates to each document's `Export*PDF` (which owns its filename incl. folder/timestamp).
...
> Beyond the four otkup-flow templates above, the same render-onto-worksheet pattern is reused by `GrupniOtkupSablon` / `OtpremnicaSablon` / `IzdAmbSablon` and the hidden `SpecifikacijaSablon` (modOtkupBlok). As of v6.43 the formerly **manual** templates `FakturaSablon`, `KarticaSablon` and `SledljivostSablon` are also **generated in `modPrint`** (`EnsureFakturaSablon` / `EnsureKarticaSablon` / `EnsureSledljivostSablon`, layout-version marker like `PaletaSablon` — `H1` for faktura/kartica, `N1` for the 12-column landscape sledljivost) with the shared house style, and filled by `FillFakturaSablon` / `FillKarticaSablon` / `FillSledljivostSablon`. The domain modules keep their data/guards and call the modPrint renderers: `modFaktura.PrintFaktura` (business guards: single-row, storno, duplicate-`FakturaID`), `modIzvestaj.PrintKarticaPDF` (`ReportKarticaKooperanta`), `frmSledljivost.PrintTracePDF` (`TraceByZbirna` + prijemnica reconciliation). No printable template errors-if-missing anymore; all auto-generate and auto-upgrade on layout-version change. (`PrintKarticaAmbalazePDF` is a separate code-built document on `_KartAmbPrint`, intentionally untouched.)

Output mode is config-driven per document via `*_PRINT_MODE` keys (`OFF` / `PRINT` / `PREVIEW` / `PDF`, resolved through `DocResolveMode`): `OTKUP_PRINT_MODE` (otkupni + grupni), `PRIJEMNICA_PRINT_MODE`, `OM_IZDAVANJE_PRINT_MODE` (revers), `PALETA_PRINT_MODE`, plus `FAKTURA_PRINT_MODE` (default `PRINT`), `KARTICA_PRINT_MODE` and `SLEDLJIVOST_PRINT_MODE` (default `PDF`). Empty key → the document's default, so unset config preserves prior behavior.

Domain rules:

- **Prijemnica auto-print is gated to the firm's own cold storage.** `frmDokumenta.btnUnosPrij` calls `OutputPrijemnica` (output controlled by `PRIJEMNICA_PRINT_MODE`) **only when `KupacID == MALINA_DEFAULT_KUPAC`** — external-buyer prijemnice are not auto-printed; `PrintPrijemnica(prijemnicaID)` is the manual reprint for any buyer. (The `AUTO_PRIJEMNICA_HLADNJACA` toggle is unrelated — it governs auto-creation of the chain from a hladnjača *station* during otkup, not this print path.)
...
- Otkupni-list mandatory otkup-block elements: PDV-nadoknada **klauzula** (config `OTKUP_KLAUZULA`, default `OtkupKlauzulaDefault` — čl. 34 ZPDV) and **rok isplate** (config `OTKUP_ROK_ISPLATE`, default `Po dogovoru`). Config keys use strict exact-match lookup via `GetConfigValue`. The `OTKUP_KLAUZULA` text may contain placeholder tokens that `FillOtkupSablon` replaces per otkupni list at print time (case-insensitive): `{BPG}` (broj gazdinstva — differs per kooperant, so it cannot be hard-coded in the static config text), `{POLJOPRIVREDNIK}`, `{RACUN}`, `{DATUM}`, `{BROJ}`.
...
- `OtkupSablon` geometry: each primerak occupies exactly 1/3 of A4 (99 mm); the two copies fill the top two thirds and the bottom third stays blank, matching the client's pre-perforated paper (two perforations → three equal parts). `WriteOtkupCopy` uses explicit row heights (no AutoFit) and a trailing filler row that pads each copy to exactly 99 mm (`OL_THIRD_PT = 280.63 pt`); the page prints 1:1 (`Zoom = 100`, `TopMargin = 0`, no fit-to-page) so the copy boundaries land on the 99 mm / 198 mm perforations. If a printer ignores `TopMargin = 0`, the `OL_TOP_MARGIN_TRIM_PT` constant shrinks copy 1 to re-align. Sized for 1–2 stavke per copy. The packaging block is a bordered mini-table to the left of the obračun box — rows **Primljena / Izdata / Saldo ambalaze** (saldo = primljena − izdata for the document); **rok isplate** is right-aligned on the Datum row (the per-copy label row was removed and the header compacted: seller name + address/PIB-MB-Žiro row, centered title, combined poljoprivrednik+BPG+TR row). The klauzula spans the same columns as the stavke/obračun table (full content width) and its row height absorbs the freed vertical space so the signatures land just above the perforation.
...
- On save, the prijemnica is auto-printed (`OutputPrijemnica`, per `PRIJEMNICA_PRINT_MODE`) **only when `KupacID == MALINA_DEFAULT_KUPAC`** (the firm's own cold storage); see 5.12.
...
| `LICENSE_ENABLED` | Per-device license gate toggle (`YES`/`NO`, default `NO` = off). **Latch:** once a machine has activated (both `LICENSE_KEY` and `LICENSE_BOUND_PARTS` present) the gate is enforced even if this is later set back to `NO` (anti-bypass) — see `modLicense.LicenseRequired`. Owned by `modLicense`. |
...
> **Access gate (`modLicense.AccessGateOrQuit`):** licensed machine is allowed and **never sees the trial**; an unlicensed machine (no `LICENSE_KEY`) gets the trial when `TRIAL_ENABLED=YES`, otherwise falls to the license gate. An **already-activated** machine forces the gate regardless of `LICENSE_ENABLED` (latch via `modLicense.LicenseRequired`), so setting the flag back to `NO` cannot bypass licensing post-activation. `modLicense` and `modTrial` are a pair (mutual references) and must both be imported.
